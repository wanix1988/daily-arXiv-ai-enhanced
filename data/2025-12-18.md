...
TL;DR: 本研究构建了SkyCap数据集，通过存档匹配和配准将光学SkySat和SAR Capella Space场景结合，用于SAR高分辨率变化检测，并发现预处理与预训练统计的对齐对模型性能影响显著。


<details>
  <summary>Details</summary>
Motivation: 当前用于线性基础设施监控的变化检测需要高分辨率数据和稳定的观测频率，但光学和雷达传感器各有优缺点。光学VHR图像可解释性高但易受云遮挡影响，而SAR则不受天气影响但难以标注。

Method: 通过SkySat和Capella数据存档匹配和配准创建了SkyCap双时相VHR光学与SAR图像数据集，利用光学到SAR的标签转换学习SAR幅度变化检测标签，无需人工专家标注，并对SAR特定的基础模型（FMs）和光学FMs在SkyCap数据集上进行了预训练和基准测试。

Result: 实验结果表明，MTP（ViT-B+RVSA）光学模型配合dB+Z-score预处理在F1$_c$指标上表现最佳，达到45.06，优于直接在Capella数据上预训练的SAR特定FMs，显示了预处理对齐预训练统计对模型性能的重要影响，并且光学模型在光学变化检测中的排名不一定直接应用于SAR幅度变化检测。

Conclusion: 研究创新性地展示了基础模型在VHR SAR幅度变化检测领域的应用，并强调了预处理与预训练数据集统计的对齐对模型性能的影响。
...
TL;DR: 本研究提出了SocialNav-MoE，一种基于Mixture-of-Experts的高效视觉语言模型，用于社交适应的导航。该模型利用强化学习微调和语义相似性奖励来平衡导航准确性与效率。


<details>
  <summary>Details</summary>
Motivation: 在人类共存的环境下，机器人的安全性和社会遵守同样重要。尽管较大的视觉语言模型有潜力实现社交适应导航，但它们的计算开销大，不适合在资源受限的机器人平台上实时部署。

Method: 我们研究了小型视觉语言模型的有效性，并提出了SocialNav-MoE，这是一种利用Mixture-of-Experts架构的模型，结合了强化学习微调（RLFT）和语义相似性奖励（SSR）功能。此外，研究了不同小型语言模型类型（Phi、Qwen、StableLM）、路由策略以及视觉编码器（CLIP、SigLIP，冻结状态 vs. 微调状态）的有效性。

Result: 实验表明，SocialNav-MoE 在导航准确性和效率之间取得了良好的平衡，提出的语义相似性奖励（SSR）比硬级和字符级奖励更有效。

Conclusion: 该研究为社交适应导航提供了一种新的解决方案，有望推动机器人技术在人机共存环境中的应用。
...
TL;DR: 该研究提出了一种模块化的专家系统流水线，能够将包含歌词的简谱乐谱自动转换为可读的MusicXML和MIDI格式，无需大规模标注数据。系统结合了传统的计算机视觉技术和无监督深度学习模块，用于页面级和乐句级别的特征提取。实验表明，系统在旋律和歌词识别上均取得了高度精准的结果。


<details>
  <summary>Details</summary>
Motivation: 现有的光学音乐识别研究大多集中于西方五线谱，而简谱及其丰富的歌词资源则相对未被充分利用。本研究旨在填补这一空白，通过开发一种无需大量标注数据的自动转换系统，为简谱及其歌词的数字化提供解决方案。

Method: 该方法采用自上而下的专家系统设计，结合传统计算机视觉技术（如短句相关性和骨架分析）与无监督的深度学习模块，对乐谱进行页面级和乐句级别的特征提取，从而在保持高可解释性的同时提高识别准确性。

Result: 在《中国民间歌曲集成》数据集上，该系统成功大规模转录了一部包含超过5000首无伴奏旋律曲目和1400余首带歌词的乐谱，分别包含超过30万和10万音符。实验结果显示，系统在旋律识别（音符级F1得分：0.951）以及歌词对齐识别（字符级F1得分：0.931）方面的精度均较高。

Conclusion: 该研究提出的系统为简谱及其歌词的大规模数字化提供了有效解决方案，证明了在无需大量标注数据的条件下，利用混合策略可以在保持高可解释性的同时实现高识别精度。
...
TL;DR: AquaDiff是一种基于扩散的水下图像增强框架，旨在修正色差同时保持结构和感知保真度。该方法结合了色彩先验引导的颜色补偿策略与条件扩散过程，使用残差密集块和多分辨率注意力机制捕捉全局颜色上下文和局部细节，并引入新的跨域一致性损失来综合要求。


<details>
  <summary>Details</summary>
Motivation: 由于水体中的光吸收和散射，水下图像受到了严重影响，导致色彩失真、对比度低和细节损失，阻碍了基于视觉的水下应用。因此，提出了AquaDiff框架来解决这些挑战。

Method: AquaDiff框架通过结合色彩先验引导的颜色补偿策略和条件扩散过程，以及增强去噪残差密集块和多分辨率注意力机制，综合色彩上下文和局部细节。此外，还引入了一种新的跨域一致性损失来增强像素水平准确性、感知相似性、结构完整性和频域保真度。

Result: 在多个具有挑战性的水下基准测试中，与传统、CNN、GAN和扩散型方法相比，AquaDiff实现了良好的色差校正效果，并且在多种水下条件下实现了竞争力的整体图像质量。

Conclusion: 实验结果表明，AquaDiff框架在色差修正和整体图像质量方面优于现有方法。
...
TL;DR: 该研究通过将精确的皮肤乳突线文本描述与传统图像识别技术结合，提供了动物个体识别的新方法，该方法能够显著提高跨模态检索的准确性，并解决了数据稀缺的问题。


<details>
  <summary>Details</summary>
Motivation: 当前动物个体识别主要依赖图像，但存在局限性，尤其是当数据稀缺时。因此，研究者试图引入一种新的方法，通过结合文本描述和图像识别来克服传统图像识别的局限性。

Method: 研究采用了包含84,264个手动标注的细节特征的数据集，覆盖了185只东北虎的3,355张图片，开发了一种文本与图像协同生成的管道，生成了具有皮肤乳突线文本描述的虚拟个体，并进行了跨模态检索基准测试。

Result: 这种方法在跨模态检索中表现出了显著的优势，可以大幅提高准确性，并缓解了数据稀缺的问题。

Conclusion: 研究者的路径提供了超越纯视觉限制的方法，通过提供可验证的人文匹配，使得文本到视觉的身份恢复成为可能，这标志着在身份识别中可解释性的提升，以及生态监测中描述方式统一的语义驱动联合。
...
TL;DR: 本文提出了一种新颖的任务——Vibe Blending，旨在生成具备连贯性和意义的图像混合体，揭示不同图像之间的共享属性。Vibe Space是一个层次图流形，能够学习特征空间中的低维测地线，实现概念之间的平滑和语义一致的过渡。利用包含人类评估、LLM推理和几何路径难度评分的认知启发式框架，该方法生成的结果比当前方法更具有创造力和连贯性。


<details>
  <summary>Details</summary>
Motivation: 当前方法在识别和穿越不同概念间非线性的路径方面存在局限，无法生成具备高创造力和一致性的图像混合体。

Method: 本文提出了Vibe Space，一种层次图流形，能学习特征空间（如CLIP）中的低维测地线，从而实现不同概念间的平滑过渡与语义一致性。

Result: 通过结合人类评估、LLM推理和几何路径难度评分的认知框架，本方法生成的图像混合体相比现有方法在创造性和连贯性方面表现更优。

Conclusion: 本文的方法能够有效地生成更具创意和连贯性的图像混合体，为未来相关研究提供了新的思路与技术基础。
...
TL;DR: 研究通过PANDA-PLUS-Bench基准数据集评估了多个基础模型在前列腺癌Gleason分级任务中的性能，发现模型在区分组织特异性特征和幻象特征方面存在显著差异，提示特定组织训练可能增强生物学特征的捕获能力。


<details>
  <summary>Details</summary>
Motivation: 当前的人工智能基础模型在前列腺癌Gleason分级领域可能存在学习幻象特征而忽略了可推广的生物学特征的问题，这限制了其在临床环境中的实用价值。因此，研究引入了一个专门用于量化这一问题的基准数据集，以提高模型在临床重要上下文中的鲁棒性。

Method: 研究构建了一个由九个来自不同患者的整张切片图像组成的基准数据集，每个图像都包含不同的Gleason模式，并进行了八种不同的数据增强操作。然后，研究使用这个基准数据集对七种基础模型进行评估，以确定它们区分生物学信号和切片级别的混淆因素的能力。

Result: 研究结果表明，不同模型在鲁棒性方面存在显著差异。Virchow2在大模型中滑动级别的编码最低（81.0%），但交叉滑动准确性第二低（47.2%）。尤其值得注意的是，专门针对前列腺组织训练的HistoEncoder，显示了最高的交叉滑动准确性（59.7%）和最强的滑动级别编码（90.3%）。所有的模型在内部滑动和跨滑动之间的准确性差异都有所存在，范围从19.9个百分点到26.9个百分点。

Conclusion: 研究数据表明，专门针对组织训练可能增强生物学特征的捕获能力和滑片特异性签名。同时，研究提供了一个开源的Google Colab笔记本，使研究人员能够使用标准化的度量标准对其他基础模型进行评估。PANDA-PLUS-Bench通过提供一个专门构建的资源，填补了基础模型评估方面的一个重要空白，在临床重要的Gleason分级上下文中提高了模型的鲁棒性。
...
TL;DR: 本文提出了一种适应性后处理技术，用于改进大规模预训练模型生成的胶质瘤分割质量，显著提高了各种脑肿瘤分割挑战任务的效果。


<details>
  <summary>Details</summary>
Motivation: 强调了对于胶质瘤这种高致命性的成人脑部肿瘤，准确的多参数MRI肿瘤分割对于手术规划、放射治疗和疾病监测至关重要。传统的深度学习模型虽然提高了自动化分割的准确性，但在大规模预训练模型普遍存在的泛化不良和系统性误差问题，以及不平等的GPU资源获取和大规模模型训练的环境成本，迫使研究者探索新的技术。本文旨在提供一种高效的后处理策略，以提高大型预训练模型的分割质量。

Method: 本文提出了一种适应性后处理方法，用于优化不同类型的预训练模型生成的胶质瘤分割结果。该方法通过改进的各种ClosedBrain竞赛任务，展示了该技术的有效性，特别是在发育中国家的胶质瘤分割任务中取得了显著效果。

Result: 该方法在BraTS 2025分割挑战任务中表现出色，特别是在针对撒哈拉以南非洲地区任务中的排名提高了14.9%，在成人胶质瘤挑战任务中的排名提高了0.9%。

Conclusion: 本文提出的适应性后处理技术有助于胶质瘤分割研究从复杂模型架构转向高效的、与临床需求相匹配的后处理策略，这些策略既精确又公平且可持续，在数据量、计算能力和环境影响方面都有更好的表现。
...
TL;DR: TalkVerse 是一个大规模、开放的单人音频驱动视频生成语料库，包含2.3百万个同步音频视频片段，总时长6300小时，用于公平可重复比较不同方法。研究人员使用 TalkVerse 开发了一个可复现的5B DiT基线模型，实现流畅的分钟级生成并降低推理成本10倍。


<details>
  <summary>Details</summary>
Motivation: 目前最先进的系统依赖于封闭数据或计算密集型模型，而 TalkVerse 旨在提供一个大规模、开放的数据集，以便进行公平和可重复的比较，降低研究门槛。

Method: 使用 TalkVerse，研究团队训练了一个基于 Wan2.2-5B 的5B DiT基线模型。模型利用视频 VAE 和滑动窗口机制实现分钟级生成，通过控制隐空间噪声实现了零样本视频配音。此外，还引入了一个基于视觉和听觉线索的 MLLM 导演来重新编写提示。

Result: 实验结果显示，基于 TalkVerse 的 5B DiT 基线模型能够实现流畅的分钟级生成，唇型同步和视觉质量与 14B Wan-S2V 模型相当，但推理成本降低了10倍。此外，模型支持通过控制隐空间噪声实现零样本视频配音。

Conclusion: 研究团队开放了 TalkVerse 的数据集、训练食谱和5B模型的检查点，以促进音频驱动的人类视频生成领域的研究工作。
...
TL;DR: PC-GRPO 提出了一个无需标注和外部验证的监督框架，通过自监督的拼图环境增强视觉推理，缓解了奖励稀疏和平坦奖励的难题，并通过动态难度课程来提高推理质量、稳定性和下游准确性。


<details>
  <summary>Details</summary>
Motivation: 近年来，RL 方法如 Outcome-supervised GRPO 对 VLM 中的链式思维推理有了显著提升，但仍面临标注依赖、奖励稀疏和平行推理逻辑不一致等问题。

Method: PC-GRPO 使用三个自监督的拼图环境（PatchFit、Rotation、Jigsaw）取代标签，引入难度感知课程，在训练过程中监控推理-答案一致性以提高最终性能。

Result: PC-GRPO 在不同基准测试和不同模型下显著提高了推理质量和下游准确性，验证了改进后的 VLM 的有效性和可解释性。

Conclusion: PC-GRPO 是一个可扩展的、可验证的和可解释的 RL 后训练方法，对 VLM 中的自动推理问题提出了实用的解决方案。
...
TL;DR: 本文提出了一种三模态身份识别框架，利用多任务学习独立处理语音、面部和手势模态，并通过跨注意力和门控融合机制促进模态间的交互。该系统动态适应模态缺失或低质量数据，实现了在单模态或双模态条件下具有高准确性的身份识别。


<details>
  <summary>Details</summary>
Motivation: 实世界条件下模态数据常常缺失或劣化，为此提出了一种鲁棒的三模态识别框架，能够在不同模态条件下保持高识别准确性。

Method: 该方法利用多任务学习处理每个模态，然后通过跨注意力和门控融合机制促进多模态间的交互。此外，采用基于置信度的融合策略，适应模态缺失或低质量数据。

Result: 在CANDOR数据集上，本文提出的三模态系统在单模态和双模态条件下的识别准确率分别达到99.18%和99.92%，超越了传统的单模态和延迟融合方法。

Conclusion: 该研究提出了一种三模态身份识别系统，具有良好的鲁棒性和较高的识别准确性，为实世界条件下的身份识别提供了新的解决方案。
...
TL;DR: PMMD提出了一种方法，它通过多视图参考、姿态图和文本提示生成人体的光逼真图像。该方法通过多模态编码器减少跨模态差异，提高身份保真度，并设计了ResCVA模块和跨模态融合模块来增强细节同时保持全局结构。


<details>
  <summary>Details</summary>
Motivation: 目前的人体图像生成方法存在遮挡、服装风格漂移和姿态错位的问题，这些问题限制了虚拟试衣、图像编辑及数字人脸创建等应用的效果。

Method: PMMD采用了一个扩散框架，结合多视图参考、姿态图和文本提示进行图像合成。多模态编码器联合建模视觉视图、姿态特征和语义描述。设计了ResCVA模块和跨模态融合模块来处理图像细节和语义信息。

Result: 在DeepFashion多模态数据集上的实验表明，PMMD在一致性、细节保持和可控性方面优于代表性的基线方法。

Conclusion: PMMD通过综合多视图参考和多模态信息，提出了一种用于生成光逼真人体图像的新方法，能够满足虚拟试衣、图像编辑和数字人脸创建等应用的需求。
...
TL;DR: Uni-Parser 是一种工业级文档解析引擎，专为科学文献和专利设计，具备高吞吐量、稳定准确性和成本效益。它采用模块化多专家架构，支持跨模态对齐，并优化了云环境部署，实现每秒20页PDF的处理速度。


<details>
  <summary>Details</summary>
Motivation: 开发Uni-Parser旨在应对大规模文档数据解析需求，特别是科学文献和专利领域的处理，提供性能优异的解决方案。

Method: Uni-Parser采用模块化和松耦合的多专家架构，集成了自适应GPU负载均衡、分布式推理、动态模块编排和可配置模式等功能，支持整体或特定模态的解析。

Result: Uni-Parser在8 x NVIDIA RTX 4090D GPU上实现了每秒20页PDF的处理速度，支持跨云大规模部署，并能高效处理数十亿页文档。

Conclusion: Uni-Parser为科学文献检索、摘要生成、化学结构和活性数据提取等下游应用提供了高效的成本效益解决方案，支持大规模语料库构建，进而培训下一代大语言模型和AI4Science模型。
...
TL;DR: 本研究全面评估了Nano Banana Pro在14个低级视觉任务上的性能，发现其在主观视觉质量上表现出众，但在量化指标上逊色于专门模型，强调了对于高保真度的专业模型实现的难度。


<details>
  <summary>Details</summary>
Motivation: 随着文本到图像生成模型的迅速发展，研究者们开始探索这些模型在传统低级视觉任务的潜力。尽管市场上已有多个商业产品，但它们作为通用型解决方案的能力仍不明确。因此，此研究旨在评估Nano Banana Pro在零样本条件下的表现，以确定其作为低级视觉任务全能选手的可能性。

Method: 研究者们对Nano Banana Pro进行了零样本评估，涉及14个不同类型的低级视觉任务，这些任务涵盖了40个不同的数据集。评估过程中，他们仅使用简单的文本提示，未进行Fine-Tuning。通过对比Nano Banana Pro与当前领先的专业模型，研究者分析了它们在主观视觉质量和量化指标上的差异。

Result: 研究结果表明，Nano Banana Pro在主观视觉质量上表现突出，能够生成超出现有专业模型的高频细节。然而，在传统的参考基量化指标方面，它明显落后。这主要归因于生成模型固有的随机性，这种随机性使得维护与传统度量要求一致的像素级准确性变得困难。

Conclusion: 报告指出，Nano Banana Pro在低级视觉任务上具有零样本竞争能力，但在达到专业模型的高保真度方面仍面临重大挑战。
...
### [24] [3DProxyImg: Controllable 3D-Aware Animation Synthesis from Single Image via 2D-3D Aligned Proxy Embedding](https://arxiv.org/abs/2512.15126)
...
### [25] [Explainable Action Form Assessment by Exploiting Multimodal Chain-of-Thoughts Reasoning](https://arxiv.org/abs/2512.15153)
...
### [26] [EagleVision: A Dual-Stage Framework with BEV-grounding-based Chain-of-Thought for Spatial Intelligence](https://arxiv.org/abs/2512.15160)
...
### [27] [Criticality Metrics for Relevance Classification in Safety Evaluation of Object Detection in Automated Driving](https://arxiv.org/abs/2512.15181)
...
### [28] [Robust and Calibrated Detection of Authentic Multimedia Content](https://arxiv.org/abs/2512.15182)
...
### [29] [ERIENet: An Efficient RAW Image Enhancement Network under Low-Light Environment](https://arxiv.org/abs/2512.15186)
...
### [36] [SynthSeg-Agents: Multi-Agent Synthetic Data Generation for Zero-Shot Weakly Supervised Semantic Segmentation](https://arxiv.org/abs/2512.15310)
...
### [37] [KD360-VoxelBEV: LiDAR and 360-degree Camera Cross Modality Knowledge Distillation for Bird's-Eye-View Segmentation](https://arxiv.org/abs/2512.15311)
...
### [38] [Automated Motion Artifact Check for MRI (AutoMAC-MRI): An Interpretable Framework for Motion Artifact Detection and Severity Assessment](https://arxiv.org/abs/2512.15315)
...
### [39] [Prototypical Learning Guided Context-Aware Segmentation Network for Few-Shot Anomaly Detection](https://arxiv.org/abs/2512.15319)
...
### [40] [VTCBench: Can Vision-Language Models Understand Long Context with Vision-Text Compression?](https://arxiv.org/abs/2512.15649)
...
TL;DR: TIMAR是一种因果建模框架，用于3D对话头部生成，它融合了每轮对话中的多模态信息，并运用轮次级因果注意力累积对话历史，预测连续的3D头部动态。


<details>
  <summary>Details</summary>
Motivation: 现有的对话建模框架在处理说话和倾听的交互时存在问题，或者依赖于非因果的全序列建模，这阻碍了时间上的连贯性。此研究旨在解决此问题。

Method: TIMAR使用因果Masked AutoRegression（MA）模型，通过在每轮对话中融合多模态信息来构建对话的交织音频-视觉上下文，应用轮次级因果注意力来积累对话历史，同时利用轻量级扩散头预测连续的3D头部动态。

Result: 在DualTalk基准测试上，TIMAR的Frechet距离和MSE分别降低了15-30%，并且类似的改进也适用于新数据。

Conclusion: TIMAR框架在3D对话头生成中表现出优越的性能和连贯性，能够捕捉同步性和表达性变异。
...
TL;DR: 本文提出了一种名为Pro-GRPO的新颖动态框架，通过早期终止聚类奖励的轨迹来减少计算开销，同时利用样本多样性来优化，适用于流形和扩散模型。


<details>
  <summary>Details</summary>
Motivation: 当前的GRPO技术在大群体规模和高计算成本之间存在冲突，本文通过实证研究揭示了轨迹聚类奖励现象，并设计了Optimal Variance Filtering（OVF）和Pro-GRPO框架来解决这个问题。

Method: 本文提出了一种新型动态框架Pro-GRPO，利用早期终止聚类奖励的轨迹来降低计算成本，同时采用扩展并修剪策略以最大化轨迹多样性。

Result: 实验证明，Pro-GRPO框架在扩散和流形模型上的普遍性和有效性。

Conclusion: Pro-GRPO框架有效缓解了群体规模和计算成本之间的冲突，能够高效地提高生成模型的性能。
...
TL;DR: 该研究提出了一种专为桥梁的3D语义分割设计的新数据集，并分析了不同传感器引起的领域差距。评估了三种最先进的3D深度学习模型，并发现模型在指定任务上的表现较为稳定，但传感器差异可能导致约11.4%的mIoU下降。


<details>
  <summary>Details</summary>
Motivation: 为了满足基础设施检测和维护的迫切需求，该研究旨在提供一个用于桥梁3D语义分割的新数据集，以促进结构健康监测的准确性和自动化。

Method: 研究人员构建了一个包含不同国家桥梁高分辨率3D扫描数据集，附带详细的语义标签。通过分析三种不同先进的3D深度学习模型在新数据集上的性能，并研究不同传感器的数据以衡量传感器差异带来的领域差距。

Result: 三种模型在特定任务上的表现都较为稳健，但传感器差异可能会导致高达11.4%的mIoU降低。

Conclusion: 研究结果表明，虽然所有模型在此特定任务上都表现出了较高的性能，但传感器差异仍然是一个值得关注的问题，可能会对模型的整体性能产生负面影响。
...
TL;DR: 该研究提出了在篮球广播视频中预测哪支球队将在投篮后获得球权的任务，构建了一个包含10万条视频剪辑、300多小时脚本和超过2000个手动标注的篮板事件的新数据集，利用先进的深度学习技术进行了基准测试，展示了新的挑战和机遇。


<details>
  <summary>Details</summary>
Motivation: 虽然计算机视觉和视频理解已经在体育分析中取得了显著进展，但在体育视频中预测即将发生的行为（如篮板）这一任务却受到了较少的关注。

Method: 该研究构建了一个新数据集，包含10万条篮球视频剪辑，为反弹预测任务提供了一个全面的基准，并探索了两个辅助任务：反弹分类和反弹框定，展示了通过预测团队对篮板前的控制来实现即时自动广播和赛后分析工具，以支持决策。

Result: 该方法使用最先进的深度学习技术进行基准测试，展示了对篮板预测的新应用，为动态多智能体体育场景的预测建模提供了重要见解。

Conclusion: 该研究通过揭示反弹预测的可能性及其固有的挑战，为开发实时自动广播和赛后分析工具提供重要信息。
...
### [48] [IC-Effect: Precise and Efficient Video Effects Editing via In-Context Learning](https://arxiv.org/abs/2512.15635)
*Yuanhang Li,Yiren Song,Junzhe Bai,Xinran Liang,Hu Yang,Libiao Jin,Qi Mao*

Main category: cs.CV

TL;DR: IC-Effect是一种指导指令、基于DiT的框架，用于少量样本的视频VFX编辑。它从有限的配对数据中高效学习效果模式，同时严格保持空间和时间一致性。


<details>
  <summary>Details</summary>
Motivation: 视频VFX编辑具有挑战性，需要无缝融合效果与背景，保持背景不变，并从有限的数据中高效学习效果模式。

Method: IC-Effect框架利用源视频作为清洁的上下文条件，利用DiT模型的上下文学习能力，通过两阶段训练策略实现精确的背景保留和自然效果注入。引入了时空稀疏标记化，提高了效率。

Result: IC-Effect在大量实验中展示了高质量、可控制和时间一致的VFX编辑效果，释放了视频创作的新可能性。

Conclusion: IC-Effect为视频VFX编辑提供了一种新的解决方案，促进了视频创作领域的发展。
...
TL;DR: 研究发现，保持标记独立性并使用故意浅层架构比增加模型规模更适合在多路数据中进行自我监督的表征学习。尽管结构紧凑，新型浅层Channel-Independent Model (CIM-S) 在表征能力上明显优于早期融合的CNN模型。


<details>
  <summary>Details</summary>
Motivation: 目前大多数深学习模型在多路组织成像数据中应用了早期通道融合，假设标记之间具有共享结构。本文研究是否可以通过保留标记独立性并结合故意浅层的设计，为多路数据中的自我监督表征学习提供更合适的归纳偏差。

Method: 使用了Hodgkin淋巴瘤CODEX数据集，包含145,000个细胞和49个标记。对比标准的早期融合CNN和通道分离架构，包括标记感知基线和新型浅层Channel-Independent Model (CIM-S)。

Result: 尽管参数量小，CIM-S优于早期融合模型。在对比预训练和线性评估中，CIM-S表现出更强的表征能力，并且不受数据增强设置的影响。结果表明，轻量的通道独立架构可以在多路表征学习中匹配或超越早期融合CNN和基础模型。

Conclusion: 研究表明，相较于增加模型规模，保持标记独立性和采用故意浅层的架构对于多路数据中的自我监督表征学习更加合适。CIM-S模型在表征能力上具有竞争力，并且提供了未来研究的空间。
...
TL;DR: 该研究提出了3D-Mirage基准和评估框架，旨在量化和解决单目深度模型在感知模糊的二维结构上生成虚假三维结构的问题。通过引入Grounded Self-Distillation策略，模型可以在保持背景知识的同时，精确约束幻觉区域的二维性。


<details>
  <summary>Details</summary>
Motivation: 单目深度模型通过学习大规模语义先验获得了出色的泛化能力，但同时也带来了虚假的三维结构问题，尤其是面对几何上平坦但感知上有歧义的输入时。因此，该研究旨在通过详细的评估框架和技术来诊断和缓解这个问题。

Method: 该研究的方法包括：1) 创建3D-Mirage基准，涵盖真实世界的幻觉场景，附带精确的二维区域标注和限定上下文裁剪。2) 提出一个基于Laplacian的评价框架，包括变异复合评分（DCS）和混淆复合评分（CCS），分别用于评估无意义非平坦和上下文不稳定性。3) 引入Grounded Self-Distillation，通过精心设计的蒸馏方法强化二维性约束，同时通过冻结教师模型保护背景知识。

Result: 研究结果表明，通过3D-Mirage基准和提出的评价框架可以有效量化和减少3D Mirage问题。Grounded Self-Distillation策略在保持背景知识的同时，能够精确约束幻觉区域的二维性，提高了模型的结构和上下文鲁棒性。

Conclusion: 研究为单目深度模型评估提供了一套重要的诊断和缓解工具，推动了性能更优、更鲁棒的单目深度估计技术的发展。未来工作将进一步完善评估框架，以适应更多样化的复杂场景。
...
### [51] [Evaluation of deep learning architectures for wildlife object detection: A comparative study of ResNet and Inception](https://arxiv.org/abs/2512.15480)
...
### [52] [RUMPL: Ray-Based Transformers for Universal Multi-View 2D to 3D Human Pose Lifting](https://arxiv.org/abs/2512.15488)
...
### [53] [The LUMirage: An independent evaluation of zero-shot performance in the LUMIR challenge](https://arxiv.org/abs/2512.15505)
...
### [54] [Off The Grid: Detection of Primitives for Feed-Forward 3D Gaussian Splatting](https://arxiv.org/abs/2512.15508)
...
### [55] [VAAS: Vision-Attention Anomaly Scoring for Image Manipulation Detection in Digital Forensics](https://arxiv.org/abs/2512.15512)
*Opeyemi Bamigbade,Mark Scanlon,John Sheppard*

Main category: cs.CV

TL;DR: VAAS 提出了一种新颖的双模块框架，结合 Vision Transformers 进行全局图像注意力异常估计和 SegFormer 进行局部一致性评分，旨在更准确地度量图像伪造的异常强度并提高可视化解释性。


<details>
  <summary>Details</summary>
Motivation: 鉴于现代生成模型可以生成难以检测的篡改图像，现有的方法缺乏量化篡改严重性的手段。因此，VAAS 旨在通过将全局注意力与局部一致性评分相结合，为图像篡改提供可解释的、连续的异常评分。

Method: VAAS 引入了将 Vision Transformers 与 SegFormer 结合使用的方法，前者用于全局图像注意力异常估计，后者用于局部一致性评分，从而实现对图像篡改位置和程度的量化评估。

Result: VAAS 在 DF2023 和 CASIA v2.0 数据集上的评估显示，其在 F1 和 IoU 性能方面表现出色，并通过注意力引导的异常图增强了视觉解释性。

Conclusion: VAAS 通过结合定量检测和人类可理解的推理，为图像完整性评估提供了一种透明且可靠的方法。
...
### [56] [DeX-Portrait: Disentangled and Expressive Portrait Animation via Explicit and Latent Motion Representations](https://arxiv.org/abs/2512.15524)
*Yuxiang Shi,Zhe Li,Yanwen Wang,Hao Zhu,Xun Cao,Ligang Liu*

Main category: cs.CV

TL;DR: 本文提出了一种名为DeX-Portrait的新方法，通过将姿势和表情信号分离驱动，实现从单一源图像和驱动视频生成表达性强的肖像动画。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散模型在进行肖像动画生成时未能实现对头部姿态和面部表情的高保真分离控制，从而限制了应用场景。

Method: 提出了一种将姿势表示为显式全局变换，表情表示为隐含的潜在代码的方法。设计了一个强大的运动训练器学习提取精确和分离的驱动信号。通过双分支条件机制将姿势变换注入扩散模型，并通过交叉注意力机制将表情潜在代码引入。最后，设计了一个渐进混合分类器无引导，使身份一致性更加忠实。

Result: 实验结果显示，本文方法在动画质量和分离可控性方面优于现有先进技术。

Conclusion: 本文通过分离类方法提高肖像动画的质量。
...
### [57] [EmoCaliber: Advancing Reliable Visual Emotion Comprehension via Confidence Verbalization and Calibration](https://arxiv.org/abs/2512.15528)
*Daiqing Wu,Dongbao Yang,Can Ma. Yu Zhou*

Main category: cs.CV

TL;DR: 本文提出了一种增强的视觉情感理解模型EmoCaliber，通过引入三个训练阶段来赋予多模态大语言模型（MLLMs）表达自信的能力，从而提高情感预测和置信度估计的可靠性。


<details>
  <summary>Details</summary>
Motivation: 当前的MLLMs在视觉情感理解（VEC）任务中通常一次性输出单一的情感标签，而忽视了情感感知的主观性，这可能导致多种合理解释被忽略。因此，提出让MLLMs表达对自己情感预测的信心。

Method: 该方法通过一个三个阶段的训练框架来构建EmoCaliber，逐步赋予MLLMs结构化推理、自信表达和信心校准的能力。

Result: EmoCaliber在统一分类的基准VECBench上表现出在情感预测和置信度估计方面的全面优越性。

Conclusion: 该研究展示了增强信心表达在视觉情感理解中的有效性，并为更可靠的视觉情感理解系统奠定了基础。
...
### [58] [An Efficient and Effective Encoder Model for Vision and Language Tasks in the Remote Sensing Domain](https://arxiv.org/abs/2512.15531)
*João Daniel Silva,Joao Magalhaes,Devis Tuia,Bruno Martins*

Main category: cs.CV

TL;DR: 该研究探索了使用编码器架构进行多任务学习，并开发了一种GeoMELT模型，能够有效处理来自遥感图像的文本生成和跨模态检索任务，同时保持参数量较少。


<details>
  <summary>Details</summary>
Motivation: LVLMs在多个计算机视觉和自然语言处理任务上表现出巨大潜力，但由于模型参数众多，导致训练和推理成本高昂。现有的参数高效适应技术虽有进展，但仍然不适合大多数机构。因此，研究探索了一种编码器仅架构以减轻这些负担。

Method: 该研究使用编码器仅架构，开发了一种GeoMELT模型，旨在处理复杂的多任务场景，如从遥感图像生成文本和跨模态检索。

Result: GeoMELT模型在多个基准测试中表现出高效性和有效性，能够提高多任务处理的紧凑性和性能。

Conclusion: 研究验证了编码器仅架构在处理多个遥感任务时的有效性，提供了能够优化多任务学习效率的方法。
...
### [59] [BLANKET: Anonymizing Faces in Infant Video Recordings](https://arxiv.org/abs/2512.15542)
*Ditmar Hadera,Jan Cech,Miroslav Purkrabek,Matej Hoffmann*

Main category: cs.CV

TL;DR: BLANKET是一种用于婴儿视频数据 anonymization 的新方法，能够在不破坏关键面部特征的前提下实现匿名化。


<details>
  <summary>Details</summary>
Motivation: 随着视频数据中涉及人类主体，特别是婴儿的使用越来越多，确保其伦理使用变得尤为重要。BLANKET 的提出旨在解决在保持婴儿关键面部特征的同时实现匿名化的问题。

Method: BLANKET 方法包含两个阶段。首先，通过使用扩散模型生成与原始身份兼容的新随机面孔，实现局部区域的 inpainting。其次，通过具有时间一致性的 face swapping 将新身份无缝地集成到每个视频帧中，并进行真实的表情转换。

Result: 该方法已在婴儿短视频数据集上进行评估，并与流行的方法 DeepPrivacy2 进行了比较。关键评估指标包括去识别程度、面部特征保留情况、对下游任务（如人体姿态估计）的影响以及存在的 artifacts 含量。结果显示，BLANKET 在所有其他指标上都优于 DeepPrivacy2。

Conclusion: BLANKET 方法为婴儿视频数据的匿名化提供了一种有效且实用的方法，代码已发布开源，方便使用者下载和使用。
...
### [60] [GRAN-TED: Generating Robust, Aligned, and Nuanced Text Embedding for Diffusion Models](https://arxiv.org/abs/2512.15560)
*Bozhou Li,Sihan Yang,Yushuo Guan,Ruichuan An,Xinlong Chen,Yang Shi,Pengfei Wan,Wentao Zhang,Yuanxing zhang*

Main category: cs.CV

TL;DR: 本文提出了一种名为GRAN-TED的方法，该方法通过一个高效的文本基准TED-6K来评估文本编码器的质量，该基准使用轻量级适配器和统一标准进行标准化。该方法还包括两阶段训练过程，以提高视觉表示和抽取更细微和强大的文本特征。经验证，此编码器在多种任务上均表现出优越性能。


<details>
  <summary>Details</summary>
Motivation: 文本编码器在文本到图像和文本到视频生成模型中至关重要，但其开发受限于缺乏有效的评估框架和预训练语言模型难以适应视觉合成的任务。

Method: 本文的方法包括开发一个无图像的基准TED-6K和两阶段训练。首先是从多模态大型语言模型进行微调以改善视觉表示，其次是一步步加权提取更具体和强大的文本特征。

Result: 作者在TED-6K基准测试上验证了该方法的有效性，并且GRAN-TED编码器在文本到图像和文本到视频生成任务中取得了最先进的性能。

Conclusion: 本研究提出了一种新的评价框架和文本编码器训练方法，提高了生成模型中文本到图像和视频任务的质量。
...
### [61] [On the Effectiveness of Textual Prompting with Lightweight Fine-Tuning for SAM3 Remote Sensing Segmentation](https://arxiv.org/abs/2512.15564)
*Roni Blushtein-Livnon,Osher Rafaeli,David Ioffe,Amir Boger,Karen Sandberg Esquenazi,Tal Svoray*

Main category: cs.CV

TL;DR: 该研究评估了SAM3在遥感图像中的表现，通过文本来生成掩码，并在不同监督水平下进行轻量级微调。结合语义和几何线索产生了最佳结果，尽管纯文本提示在不规则目标上表现较弱。


<details>
  <summary>Details</summary>
Motivation: 遥感图像分割面临标注数据有限及与基础模型训练数据间的差距，因此需要在有限监督下有效适应。研究希望通过使用SAM3概念驱动框架，通过对文本提示和轻量级微调的评估，找到适应方法。

Method: 研究采用了SAM3概念驱动框架，对比了文本、几何和混合提示策略，并在不同监督水平下进行轻量级微调。评估集中在语义和几何线索的组合使用上，特别关注在不规则形状目标上的表现。

Result: 研究结果表明，结合语义和几何线索的策略在所有目标和指标上都表现最佳。纯文本提示表现最差，特别是在不规则形状的目标上，反映出SAM3的文本表示与遥感图像之间语义对齐的不足。即使轻度微调也能在几何结构良好且视觉显著的目标上提供实用的性能-努力权衡。

Conclusion: 随着监督水平的增加，性能提升逐渐减缓，适度的几何标注努力足以实现有效适应。结果表明，不完全准确的分割结果和边界误差仍是遥感任务中的常见错误模式，尤其是在不规则且不常见的目标上。
...
### [62] [MoonSeg3R: Monocular Online Zero-Shot Segment Anything in 3D with Reconstructive Foundation Priors](https://arxiv.org/abs/2512.15577)
*Zhipeng Du,Duolikun Danier,Jan Eric Lenssen,Hakan Bilen*

Main category: cs.CV

TL;DR: MoonSeg3R 是一种新颖的方法，能够实现实时单目 3D 实例分割，通过利用 CUT3R 重建基础模型提供可靠的几何先验，结合自监督查询精炼模块和 3D 查询索引记忆来增强跨帧融合。


<details>
  <summary>Details</summary>
Motivation: 现有的零样本单目 3D 实例分割方法依赖于布置好的 RGB-D 序列，当面对未布置的 RGB 流数据时，无法正常工作。论文致力于解决该问题。

Method: MoonSeg3R 引入了三个核心模块：自监督查询精炼模块、3D 查询索引记忆模块和状态分布标记。该方法利用了 CUT3R 提供的几何先验，通过这些模块增强了跨帧的融合。

Result: 在 ScanNet200 和 SceneNN 数据集上的实验表明，MoonSeg3R 是首个能够实现在线单目 3D 分割的方法，并且其性能与基于 RGB-D 的最先进技术相当。

Conclusion: MoonSeg3R 为单目 3D 实例分割领域带来了一种新的范式转变，提供了一种有效解决现实世界中未布置 RGB 流问题的全新方法。
...
### [63] [IMKD: Intensity-Aware Multi-Level Knowledge Distillation for Camera-Radar Fusion](https://arxiv.org/abs/2512.15581)
*Shashank Mishra,Karan Patil,Didier Stricker,Jason Rambach*

Main category: cs.CV

TL;DR: IMKD 是一种多级知识蒸馏框架，通过雷达相机融合增强3D目标检测性能，它既保留了各传感器的独特特性，又强化了它们的互补优势。


<details>
  <summary>Details</summary>
Motivation: 现有的知识蒸馏方法通常直接传递模态特异性特征，可能导致特征失真，进而削弱各传感器的优势。IMKD 设计旨在解决这一问题。

Method: IMKD 采用了三级、强度感知的知识蒸馏策略，包括：1) 雷达对激光雷达强度感知特征蒸馏，增强雷达表示的细微结构线索；2) 激光雷达对融合特征强度导向蒸馏，强调融合层面有用几何和深度信息，促进模态间的互补而非强迫模态对齐；3) 雷达相机强度导向融合机制，促进特征对齐和校准。

Result: IMKD 在 nuScenes 基准测试中达到 67.0% NDS 和 61.0% mAP，优于所有先前的知识蒸馏雷达相机融合方法。

Conclusion: IMKD 提供了使用雷达相机融合实现高性能雷达-相机3D目标检测的新方法，且结果表现更佳。
...
### [64] [FlexAvatar: Learning Complete 3D Head Avatars with Partial Supervision](https://arxiv.org/abs/2512.15599)
*Tobias Kirschstein,Simon Giebenhain,Matthias Nießner*

Main category: cs.CV

TL;DR: FlexAvatar 是一种基于单张图片生成高质量 3D 头像的方法，通过引入可学习的数据源令牌和偏置接收机来统一训练单视角和多视角数据集，从而解决单视角训练导致的部分3D重建问题。


<details>
  <summary>Details</summary>
Motivation: Single-view 下的 3D 头像重建通常会导致信息不完整，该研究旨在通过共同利用单视角和多视角数据集的长处来改进这一问题。

Method: FlexAvatar 使用了一种基于Transformer的 3D 画像动画模型，并引入了可以学习的数据源令牌（称为偏置接收机），以助力不同视角之间的统一训练。

Result: 实验结果表明，与许多现有方法相比，FlexAvatar 生成的 3D 头像更加完整且具有真实的面部动画。

Conclusion: FlexAvatar 在单视角及少量视角下生成高质量 3D 头像具有显著优势，展示了其在头像动画领域的潜力。
...
### [65] [Qwen-Image-Layered: Towards Inherent Editability via Layer Decomposition](https://arxiv.org/abs/2512.15603)
...
### [66] [Robust Multi-view Camera Calibration from Dense Matches](https://arxiv.org/abs/2512.15608)
...
### [67] [Persistent feature reconstruction of resident space objects (RSOs) within inverse synthetic aperture radar (ISAR) images](https://arxiv.org/abs/2512.15618)
...
### [68] [OccSTeP: Benchmarking 4D Occupancy Spatio-Temporal Persistence](https://arxiv.org/abs/2512.15621)
...
### [69] [Towards Physically-Based Sky-Modeling For Image Based Lighting](https://arxiv.org/abs/2512.15632)
*Ian J. Maquignaz*
...
### [70] [InpaintDPO: Mitigating Spatial Relationship Hallucinations in Foreground-conditioned Inpainting via Diverse Preference Optimization](https://arxiv.org/abs/2512.15644)
...
### [71] [Hard Labels In! Rethinking the Role of Hard Labels in Mitigating Local Semantic Drift](https://arxiv.org/abs/2512.15647)
...
### [72] [Stylized Synthetic Augmentation further improves Corruption Robustness](https://arxiv.org/abs/2512.15675)
...
### [73] [Skyra: AI-Generated Video Detection via Grounded Artifact Reasoning](https://arxiv.org/abs/2512.15693)
...
### [74] [VLIC: Vision-Language Models As Perceptual Judges for Human-Aligned Image Compression](https://arxiv.org/abs/2512.15701)
...
### [75] [End-to-End Training for Autoregressive Video Diffusion via Self-Resampling](https://arxiv.org/abs/2512.15702)
...
### [76] [GateFusion: Hierarchical Gated Cross-Modal Fusion for Active Speaker Detection](https://arxiv.org/abs/2512.15707)
...
### [77] [Multi-View Foundation Models](https://arxiv.org/abs/2512.15708)
...
### [78] [DiffusionVL: Translating Any Autoregressive Models into Diffusion Vision Language Models](https://arxiv.org/abs/2512.15713)
...
### [79] [In Pursuit of Pixel Supervision for Visual Pre-training](https://arxiv.org/abs/2512.15715)
...
### [80] [Incentives or Ontology? A Structural Rebuttal to OpenAI's Hallucination Thesis](https://arxiv.org/abs/2512.14801)
*Richard Ackermann,Simeon Emanuilov*

Main category: cs.CL

TL;DR: 本文反驳了大语言模型幻觉的主要原因是评估激励不适当的观点，认为幻觉是变压器模型架构上的固有属性，而不是优化失败。研究表明，通过改变激励、提示或微调是无法消除幻觉的，唯一的解决办法是引入外部的真理验证和避党工委件。


<details>
  <summary>Details</summary>
Motivation: 作者希望纠正将幻觉归因于激励机制问题的观点，强调幻觉是模型架构的固有属性，以此来促进更负责任的AI系统的开发。

Method: 论文通过回顾先前关于架构幻觉的研究，并使用许可仲裁器进行实证实验，展示了幻觉无法通过改变激励、提示或微调来解决。

Result: 实验证明，幻觉只能通过提供模型缺乏的外部真理验证和避 Hospitable模块来消除，而非通过激励机制、提示或微调的调整。

Conclusion: 本文得出结论，幻觉是生成架构的结构属性，为了实现可靠的人工智能，需要区分语言流畅性和知识责任性的混合系统。
...
### [81] [T5Gemma 2: Seeing, Reading, and Understanding Longer](https://arxiv.org/abs/2512.14856)
...
TL;DR: T5Gemma 2 是一种轻量级开放编码器-解码器模型，具备多语言、跨模态和长上下文处理能力。通过 UniLM 2 预训练的编码器-解码器模型配方以及在文本和跨模态上的改进，模型在长上下文建模上表现优异。


<details>
  <summary>Details</summary>
Motivation: 为了克服T5Gemma在处理跨模态和长上下文方面的能力限制，T5Gemma 2通过引入两种方法来提高效率，分别是共享嵌入和联合注意力机制，以便更好地适应各种架构和模态。

Method: T5Gemma 2 采用了一种新的适应策略，即利用 UniLM 2 将预训练的解码器模型转换为编码器-解码器模型，并在此基础上扩展了跨模态能力。同时，该方法还包括了共享嵌入、联合注意力机制等技术。

Result: 实验证明，T5Gemma 2 在各种架构和模态下具有较高的普遍性，并在长上下文建模方面表现出色。此外，T5Gemma 2 的后训练性能显著优于其前身 Gemma 3。

Conclusion: T5Gemma 2 是一个具有借鉴意义的新一代轻量级编码器-解码器模型，适用于多种语言和跨模态应用场景，具有广阔的应用前景。
...
### [82] [Integrating Large Language Models and Knowledge Graphs to Capture Political Viewpoints in News Media](https://arxiv.org/abs/2512.14887)
*Massimiliano Fadda,Enrico Motta,Francesco Osborne,Diego Reforgiato Recupero,Angelo Salatino*

Main category: cs.CL

TL;DR: 本文通过改进现有的管道，利用大型语言模型和从Wikidata获取的语义描述来增强观点分类和声明表示，从而提高了分类性能。


<details>
  <summary>Details</summary>
Motivation: 理解新闻来源对民主社会中政治和社会话语的影响，以及是否提供了公正和公平的公众辩论的叙述。

Method: 通过调整大型语言模型进行观点分类，并结合从Wikidata获得的关于相关行为者的语义描述，以改进声明表示。

Result: 实验证明，即使在使用能够处理长输入的大型语言模型时，这两种机制的结合也能获得最佳结果。

Conclusion: 通过此改进的管道，可以更准确地理解和分析新闻媒体中关于特定话题的观点和声明。
...
### [83] [DrugRAG: Enhancing Pharmacy LLM Performance Through A Novel Retrieval-Augmented Generation Pipeline](https://arxiv.org/abs/2512.14896)
...
### [84] [Multiscale Aggregated Hierarchical Attention (MAHA): A Game Theoretic and Optimization Driven Approach to Efficient Contextual Modeling in Large Language Models](https://arxiv.org/abs/2512.14925)
*Caner Erden*
...
### [85] [Parameter Efficient Multimodal Instruction Tuning for Romanian Vision Language Models](https://arxiv.org/abs/2512.14926)
...
### [86] [Evaluating Large Language Models on Multimodal Chemistry Olympiad Exams](https://arxiv.org/abs/2512.14989)
*Yiming Cui,Xin Yao,Yuxuan Qin,Xin Li,Shijin Wang,Guoping Hu*

Main category: cs.CL

TL;DR: 研究系统评估了40种多模态大语言模型在化学问题上的表现，发现许多模型在不同模态融合方面存在困难，链式思维提示能提升模型的准确性和视觉对接能力。


<details>
  <summary>Details</summary>
Motivation: 评估现有大模型在复杂科学情境下的综合思维能力，为改进这些模型提供实践指导。

Method: 使用了由实际化学奥林匹克题目组成的全面基准测试，评估模型的多模态融合和因果推理能力。

Result: 许多多模态大语言模型在视觉与文本融合上表现不佳，甚至在移除图片后模型的准确率提升，表明视觉和语言整合的偏差；链式思维提示显著提升模型准确性和对视觉信息的理解。

Conclusion: 当前多模态大语言模型在化学科学推理方面存在明显不足，需进一步研究和开发以提高模型的科学推理能力和解释性。
...
### [87] [DASH: Dialogue-Aware Similarity and Handshake Recognition for Topic Segmentation in Public-Channel Conversations](https://arxiv.org/abs/2512.15042)
*Sijin Sun,Liangbin Zhao,Ming Deng,Xiuju Fu*

Main category: cs.CL

TL;DR: 提出了一种基于LLM的新型对话主题分割框架DASH-DTS，通过对话握手识别、相似性引导的示例选择以及生成选择性的正负样本来实现有解释性的主题分割，并在真实海事VHF对话数据集VHF-Dial和标准基准上取得了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 传统方法在处理非正式对话和隐式转换时存在局限性，为了改善对任务导向公共频道通信的理解，如海事VHF对话，提出DASH-DTS框架。

Method: 该框架的核心贡献包括：1）通过对话握手识别检测话题转换；2）通过相似性引导的示例选择增强上下文；3）生成选择性的正负样本以提高模型的判别能力和稳健性。

Result: DASH-DTS在真实海事VHF对话数据集VHF-Dial和标准基准上取得了SOTA性能，并提供了可解释的分割依据和置信度评分。

Conclusion: 通过DASH-DTS框架，为稳定监控和决策支持在操作对话中的应用奠定了坚实基础。
...
### [88] [SGM: Safety Glasses for Multimodal Large Language Models via Neuron-Level Detoxification](https://arxiv.org/abs/2512.15052)
*Hongbo Wang,MaungMaung AprilPyone,Isao Echizen*

Main category: cs.CL

TL;DR: SGM（安全干预模型）作为一种白盒神经元级别干预方法，能够通过选择性地重新校准有毒的专业神经元，以减少有害的跨模态激活，从而在不更新参数的情况下减轻多模态生成模型中的毒性。实验表明，即使在对抗性触发条件下，SGM也能大幅降低毒性，同时保持流畅性和多模态推理能力。


<details>
  <summary>Details</summary>
Motivation: 由于大型语言模型在预训练过程中从未经严格筛选的文本数据集中继承了有害的、有偏见的和不适合公开的内容，尤其是在面对各种对抗性触发的情况下，现有的、不透明且无法在训练过程外进行毒性抑制的方法已经无法满足需求。因此，提出了一种名为SGM（安全干预模型）的新方法，以实现对有毒神经元的选择性重新校准，确保生成内容的安全性。

Method: SGM通过一种带有专家加权软抑制机制的方法，在神经元级别对有毒的专家神经元进行选择性重新校准。这种方法能够在不更新模型参数的情况下，减轻有害的跨模态激活，进而控制生成内容的毒性。此外，SGM还被修改为与现有去毒方法结合使用（SGM*），以进一步提高安全性。

Result: 在开源多模态大型语言模型上的实验结果表明，SGM在标准和对抗性条件下都能有效地减轻毒性。与未处理模型相比，毒性率从48.2%降低至2.5%。此外，SGM还能够保持良好的流畅性和跨模态推理能力，使得其更具解释性和低成本。

Conclusion: SGM为多模态生成中控制有害内容提供了有效的解决策略。通过在神经元级别对有害专家神经元进行选择性重新校准，SGM不但能在标准和对抗性条件下有效降低毒性，还能与其他安全措施结合使用，进一步提升安全性，并具备解释性。
...
### [89] [The Meta-Prompting Protocol: Orchestrating LLMs via Adversarial Feedback Loops](https://arxiv.org/abs/2512.15053)
*Fanzhe Fu*

Main category: cs.CL

TL;DR: 本文提出了一种名为Meta-Prompting Protocol的理论框架，旨在将大型语言模型（LLMs）从随机聊天界面转变为可靠软件组件。该框架通过生成器、审计员和优化器三个组件协作，使用自然语言指令和文本批评来优化模型，减少了幻觉现象并避免了模型崩溃。


<details>
  <summary>Details</summary>
Motivation: 当前的提示工程方法难以满足关键任务应用所需的确定性保证。因此，需要一种能够提供更强可靠性和自我优化能力的理论框架。

Method: 引入了Meta-Prompting Protocol，其中包含一个由生成器、审计员和优化器组成的三重拓扑结构。通过将自然语言指令视为语义计算图中的可微变量，并利用文本批评作为梯度，来优化模型。

Result: 通过使用声明式编程范式(DSPy)和自动文本微分(TextGrad)方法，证明了这种方法的理论可行性，并为概率计算时代下的可观察软件工程奠定了基础。

Conclusion: 该论文展示了如何通过一种新的理论框架和方法，实现LLMs从互动聊天转变到可靠的软件组件，从而提升了模型的稳定性和可调控性。
...
### [90] [Beyond Majority Voting: Towards Fine-grained and More Reliable Reward Signal for Test-Time Reinforcement Learning](https://arxiv.org/abs/2512.15146)
*Weiqin Wang,Yile Wang,Kehao Chen,Hui Huang*

Main category: cs.CL

TL;DR: SCOPE 提出了一种分组特定的逐步置信度加权伪标签估计框架，通过结合模型置信度和动态分组策略，解决了多数投票策略带来的确认偏见和稀疏奖励问题，实验表明该方法在多个基准上优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 为了改善大规模语言模型的推理能力，SCOPE 框架通过降低对标注数据的依赖，采用逐步置信度加权伪标签估计方法，并结合模型信心和动态子组划分，旨在提高模型性能。

Method: SCOPE 框架通过引入逐步置信机制改进伪标签生成，并根据推理质量和探索多样性动态划分候选输出池。每个子组通过重复抽样形成局部共识，提供多样化的监督目标以促进更广泛的探索。

Result: 实验结果表明，SCOPE 框架在多个模型和基准上优于现有基线，特别在具有挑战性的 AIME 2025 和 AMC 数据集上分别取得了 13.1% 和 8.1% 的相对改进。

Conclusion: SCOPE 提供了一种有效的解决方案，通过集成模型置信度和动态子组划分策略，改善了大规模语言模型的推理性能。
...
### [91] [Rakuten Data Release: A Large-Scale and Long-Term Reviews Corpus for Hotel Domain](https://arxiv.org/abs/2512.15151)
*Yuki Nakayama,Koki Hikichi,Yun Ching Liu,Yu Hirate*

Main category: cs.CL

TL;DR: 论文介绍了包含730万条评论的大规模数据库，涵盖了从2009年至2024年的16年间的数据，分析了2019年至2024年间的数据漂移现象。


<details>
  <summary>Details</summary>
Motivation: 由于Rakuten Travel Reviews的数据量庞大且时间跨度长，因此开发这样一个数据库可以为旅游行业提供详细的用户反馈，有助于理解用户需求和旅游市场趋势。

Method: 使用统计方法来分析数据漂移现象，并将数据拆分为详细字段如评论文本、回复、匿名用户ID、日期、住宿ID等进行研究。

Result: 展示了数据库的统计信息，并通过统计方法提供了2019年至2024年间数据漂移的见解。

Conclusion: 论文通过大规模的旅游评论数据库，揭示了不同年份间的用户偏好变化和市场趋势，为旅游行业改进服务提供了重要参考。
...
### [92] [MCP-SafetyBench: A Benchmark for Safety Evaluation of Large Language Models with Real-World MCP Servers](https://arxiv.org/abs/2512.15163)
...
### [93] [From NLG Evaluation to Modern Student Assessment in the Era of ChatGPT: The Great Misalignment Problem and Pedagogical Multi-Factor Assessment (P-MFA)](https://arxiv.org/abs/2512.15183)
...
### [94] [RFKG-CoT: Relation-Driven Adaptive Hop-count Selection and Few-Shot Path Guidance for Knowledge-Aware QA](https://arxiv.org/abs/2512.15219)
...
### [95] [Yes-MT's Submission to the Low-Resource Indic Language Translation Shared Task in WMT 2024](https://arxiv.org/abs/2512.15226)
...
### [96] [FAME: Fictional Actors for Multilingual Erasure](https://arxiv.org/abs/2512.15235)
...
### [97] [The Moralization Corpus: Frame-Based Annotation and Analysis of Moralizing Speech Acts across Diverse Text Genres](https://arxiv.org/abs/2512.15248)
...
### [98] [SynGP500: A Clinically-Grounded Synthetic Dataset of Australian General Practice Medical Notes](https://arxiv.org/abs/2512.15259)
...
### [99] [Well Begun, Half Done: Reinforcement Learning with Prefix Optimization for LLM Reasoning](https://arxiv.org/abs/2512.15274)
*Yiliu Sun,Zicheng Zhao,Yang Wei,Yanfang Zhang,Chen Gong*

Main category: cs.CL

TL;DR: 本文提出了一种名为PPPO的新颖RLVR方法，通过重点优化生成输出的前缀部分，有效提高了强化学习在大型语言模型中的推理能力，同时减少了对低收益令牌的优化。


<details>
  <summary>Details</summary>
Motivation: 当前的RLVR方法在训练过程中忽视了哪些生成令牌（例如前缀令牌）实际上对推理有贡献，导致了低收益令牌的优化影响最终效果。

Method: PPPO通过结合路径依赖理论，在LLM推理过程中聚焦于前缀推理过程，引入了两种训练策略：逐步前缀保留和延续积累奖励。

Result: 实验结果表明，PPPO在各种推理任务中表现出色，相对于只训练26.17%的令牌，准确率提高了18.02%。

Conclusion: PPPO在提高大型语言模型推理能力方面表现出色，并且该方法在减少不必要的计算开销的同时提升了整体性能。
...
### [100] [Evaluating LLMs for Zeolite Synthesis Event Extraction (ZSEE): A Systematic Analysis of Prompting Strategies](https://arxiv.org/abs/2512.15312)
*Charan Prakash Rathore,Saumi Ray,Dhruv Kumar*

Main category: cs.CL

TL;DR: 本研究评估了多种提示策略在大型语言模型执行特定领域任务（从沸石合成实验中提取结构化信息）上的效果，发现尽管这些模型在高阶分类任务上表现良好，但在细粒度提取任务上表现较弱，且高级提示策略的改进有限。


<details>
  <summary>Details</summary>
Motivation: 随着材料科学的发展，从实验程序中自动提取有用信息变得越来越重要。本文旨在评价大型语言模型（LLMs）在沸石合成领域的具体应用效果，探索有效的提示策略，为利用LLMs进行科学信息提取提供参考。

Method: 研究人员针对沸石合成这一特定领域，设置了四个关键子任务：事件类型分类、触发文本识别、论元角色提取和论元文本提取。使用ZSEE数据集中的1,530个注释句子，评估了四种提示策略在六个先进的大模型（Gemma-3-12b-it、GPT-5-mini、O4-mini、Claude-Haiku-3.5、DeepSeek推理和非推理）上的表现。

Result: 实验结果显示，在事件类型分类上，模型表现出色，F1值在80-90%之间；但在细粒度抽取任务上，表现较为一般，特别是论元角色和论元文本抽取任务的F1值为50-65%。GPT-5-mini对提示高度敏感，F1值波动较大。高级提示策略相较于零样本策略提供的改善有限。

Conclusion: 研究结果表明，虽然LLMs在高层次理解上取得成效，但在实验参数精准提取方面，仍需依赖专门针对该领域的模型，提供了科学信息提取的定量基准。
...
### [101] [Why Your Academic Field Is Everywhere at Once: A Case Study of Arabic Linguistics](https://arxiv.org/abs/2512.15328)
*Ayman Eddakrouri,Amani Ramadan*

Main category: cs.CL

TL;DR: 本研究使用布罗克斯的分类分散度（Δ）测量方法，分析了2019年至2025年阿拉伯应用语言学研究的主题结构，发现主题分散度极低，显示出该领域的高度异质性。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于应用布罗克斯的分类分散度测量方法来分析阿拉伯应用语言学的主题结构，旨在更好地理解和评估这一领域的学术分布。

Method: 研究方法包括收集2019年至2025年间阿拉伯应用语言学领域的1,564篇出版物（按八大门类分类），采用布罗克斯公式计算主题分散度，并探讨各子领域的贡献。

Result: 研究结果显示，主题分散度为Δ = 0.194，显示出显著的分散性，反传统地揭示了一个高度异质性的领域的存在。研究还发现计算语言学在非霸权地位下的主导作用，并确认了与其他子领域的均衡关系。

Conclusion: 本研究澄清了布罗克斯公式的正确应用，展示了其在领域特征描述中的实用性，并提供了一种可复制的文献计量学方法，用于评估跨领域学科结构。
...
### [102] [Adversarial versification in portuguese as a jailbreak operator in LLMs](https://arxiv.org/abs/2512.15353)
*Joao Queiroz*
...
### [103] [Dual-Density Inference for Efficient Language Model Reasoning](https://arxiv.org/abs/2512.15358)
...
### [104] [ORACLE: Time-Dependent Recursive Summary Graphs for Foresight on News Data Using LLMs](https://arxiv.org/abs/2512.15397)
...
### [105] [Toward expert-level motivational interviewing for health behavior improvement with LLMs](https://arxiv.org/abs/2512.15446)
...
### [106] [When a Nation Speaks: Machine Learning and NLP in People's Sentiment Analysis During Bangladesh's 2024 Mass Uprising](https://arxiv.org/abs/2512.15547)
...
### [107] [CTkvr: KV Cache Retrieval for Long-Context LLMs via Centroid then Token Indexing](https://arxiv.org/abs/2512.15550)
...
### [108] [Learning inflection classes using Adaptive Resonance Theory](https://arxiv.org/abs/2512.15551)
...
### [109] [From Data to Dialogue: Unlocking Language for All](https://arxiv.org/abs/2512.15552)
...
### [110] [An Empirical Study on Chinese Character Decomposition in Multiword Expression-Aware Neural Machine Translation](https://arxiv.org/abs/2512.15556)
...
### [111] [Bolmo: Byteifying the Next Generation of Language Models](https://arxiv.org/abs/2512.15586)
...
### [112] [You Never Know a Person, You Only Know Their Defenses: Detecting Levels of Psychological Defense Mechanisms in Supportive Conversations](https://arxiv.org/abs/2512.15601)
...
### [113] [Evaluating Metrics for Safety with LLM-as-Judges](https://arxiv.org/abs/2512.15617)
...
### [114] [How Much is Too Much? Exploring LoRA Rank Trade-offs for Retaining Knowledge and Domain Robustness](https://arxiv.org/abs/2512.15634)
...
### [115] [PPSEBM: An Energy-Based Model with Progressive Parameter Selection for Continual Learning](https://arxiv.org/abs/2512.15658)
...
### [116] [Activation Oracles: Training and Evaluating LLMs as General-Purpose Activation Explainers](https://arxiv.org/abs/2512.15674)
...
TL;DR: 本文通过将自我注意和残差流解释为近似的向量符号架构（VSA），提出了一种统一的视角，解释了基于Transformer的语言模型在稳定符号操作任务上的脆弱性。作者提出了VSA启发式的架构偏置和训练目标，并设计了度量标准来衡量VSA的相似性和逻辑组合性。


<details>
  <summary>Details</summary>
Motivation: 研究指出，尽管基于Transformer的语言模型在推理任务上表现出色，但在需要稳定符号操作的任务上仍然很脆弱。作者旨在通过引入VSA的视角，来理解这种现象，并提出改进方案以增强模型的逻辑可靠性。

Method: 作者将自我注意和残差流解释为近似的向量符号架构，从而建立了Transformer内部机制与链式思维、程序推理和内存辅助工具使用之间的联系。

Result: 作者解释了特征失败模式，如变量混淆和逻辑相关提示的一致性问题，并提出了一种基于VSA的架构偏置和训练目标。此外，作者还提出了衡量VSA相似性和逻辑组合性的度量标准。

Conclusion: 该研究认为，将注意力解释为软向量符号计算为开发更可解释且逻辑可靠的推理系统提供了一种有原则的方法。
...
TL;DR: 本研究通过系统地注入结构化配置知识，增强了LLM在Terraform领域的IaC生成能力，显著提高了技术验证成功率，并揭示了实现用户意图的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在生成符合意图的IaC代码方面表现不佳，研究旨在通过注入结构化配置知识，提升LLM在具体领域如Terraform中的IaC生成能力。

Method: 研究通过增强现有的IaC-Eval基准，加入云模拟和自动化错误分析，并开发了一种新型的LLM辅助IaC代码生成错误分类法。随后实现了从简单检索增强生成到更复杂的基于图的检索增强生成等技术。

Result: 实验结果显示，尽管基线LLM的整体成功率仅为27.1%，但通过注入结构化配置知识，技术验证成功率提高到75.3%，整体成功率提高到62.6%。尽管技术正确性有所提升，但实现用户意图则相对受限，存在“正确性-一致性差距”。

Conclusion: 研究建议未来工作进一步探索如何增强LLM的意图一致性，使其不仅仅成为高效的“程序员”，也能更好地成为“架构师”。
...
### [120] [LADY: Linear Attention for Autonomous Driving Efficiency without Transformers](https://arxiv.org/abs/2512.15038)
...
### [121] [A Clustering-Based Variable Ordering Framework for Relaxed Decision Diagrams for Maximum Weighted Independent Set Problem](https://arxiv.org/abs/2512.15198)
*Mohsen Nafar,Michael Römer,Lin Xie*

Main category: cs.AI

TL;DR: 该论文提出了一种通过集群重构变量排序的新框架来提高离散优化问题中松弛决策图的紧度。


<details>
  <summary>Details</summary>
Motivation: 现有的动态变量排序启发式方法虽然有效，但在整个变量集上进行全局评估时，会导致计算开销增加。

Method: 论文介绍了基于聚类的框架，首先将变量分成集群，然后利用这种结构分解来指导排序过程，显著减少了启发式搜索的空间。

Result: 提出了两种策略 - Cluster-to-Cluster和Pick-and-Sort，并介绍了两种不同策略下DD大小增长的理论结果，最终使提出的基于DD的分支定界算法在最大加权独立集问题上的基准实例中成本降低。

Conclusion: 新方法在解决最大加权独立集问题时，与标准动态变量排序基准相比，具有持续的成本降低效果。
...
TL;DR: 本研究使用2025年韩国高考地球科学I部分来评估最先进大语言模型的多模态科学推理能力和认知局限，指出模型在非结构化输入、感知与认知差距、计算与概念差异以及过程幻想等方面存在缺陷，提出了设计‘抗AI问题’的方法来确保评估的公平性。


<details>
  <summary>Details</summary>
Motivation: 随着学生利用AI完成作业的情况增加，研究关注点转向了如何通过设计问题来检测和区分真正的学生能力与AI生成的内容，以维护学术诚信和评估的有效性。

Method: 研究设计了三种实验条件：全文输入、单个题目输入和优化的多模态输入，通过定量和定性的分析来评估模型在不同类型数据结构下的表现。

Result: 研究结果表明，非结构化输入导致显著的性能下降，即使在优化条件下，模型也表现出根本性的推理错误。定性分析显示出‘感知错误’和‘感知认知差距’、‘计算概念差异’，以及‘过程幻想’三个主要问题。

Conclusion: 研究结论指出，通过利用AI的这些认知缺陷，教育者可以设计‘抗AI问题’来区分真实的学生能力与AI生成的内容，促进考试公平性。
...
### [123] [Bilateral Spatial Reasoning about Street Networks: Graph-based RAG with Qualitative Spatial Representations](https://arxiv.org/abs/2512.15388)
...
### [124] [Outer-Learning Framework for Playing Multi-Player Trick-Taking Card Games: A Case Study in Skat](https://arxiv.org/abs/2512.15435)
*Stefan Edelkamp*
...
### [125] [Intent-Driven UAM Rescheduling](https://arxiv.org/abs/2512.15462)
...
### [126] [Nemotron-Math: Efficient Long-Context Distillation of Mathematical Reasoning from Multi-Mode Supervision](https://arxiv.org/abs/2512.15489)
...
### [127] [Evaluating Large Language Models in Scientific Discovery](https://arxiv.org/abs/2512.15567)
...
### [128] [A Decision-Theoretic Approach for Managing Misalignment](https://arxiv.org/abs/2512.15584)
...
TL;DR: CAGE 提供了一种新的框架，通过引入一个因果属性图来逐步改进语言模型的上下文解释，该图保留了因果关系和行随机性，从而提高了上下文解释的真实性和准确性。


<details>
  <summary>Details</summary>
Motivation: 当前上下文归因方法产生的解释不完整，因为它们直接将生成的标记与提示联系起来，忽略了代间影响。CAGE 的引入旨在解决这个不足，通过构建保留因果关系和行随机性的属性图来解释语言模型的生成过程。

Method: CAGE 使用了一个属性图，这是一个有向图，量化了每一生成步骤如何受到提示和所有先前生成步骤的影响。通过沿着图中的路径进行中间贡献的边际化计算上下文归因。

Result: CAGE 在多个模型、数据集、度量标准和方法上进行测试，显示出其改善上下文归因真实性的显著效果，平均提高了40%。

Conclusion: CAGE 通过引入一个更全面的属性图框架，解决了当前上下文归因方法的不足，有效提高了语言模型决策解释的真实性和可靠性。
...
### [131] [FAME: FPGA Acceleration of Secure Matrix Multiplication with Homomorphic Encryption](https://arxiv.org/abs/2512.15515)
*Zhihan Xu,Rajgopal Kannan,Viktor K. Prasanna*

Main category: cs.AR

TL;DR: 本文提出了一种基于FPGA的高效HE矩阵乘法（HE MM）实现，通过减少外部存储器流量和内存需求，显著提高了性能。


<details>
  <summary>Details</summary>
Motivation: 对加密数据进行安全计算的需求日益增长，尤其是云计算中的隐私问题。然而，现有的HE矩阵乘法（HE MM）由于高计算成本，特别是矩阵相乘操作，其实际部署受到了阻碍。

Method: 文中首先建立了成本模型来评估给定HE参数和输入矩阵尺寸时的片上内存需求。然后设计了一种用于同态线性变换（HLT）的新型数据路径，以减少外部存储器流量和片上内存需求，并引入了FAME FPGA加速器。

Result: FAME在Alveo U280 FPGA上实现了HE MM，并在各种矩阵大小和形状上对其实现进行了评估。结果显示，与其他基于CPU的实现相比，FAME的速度平均提高了221倍，证明了其在大规模连续HE MM和真实负载中的可扩展性和实用性。

Conclusion: 该研究为HE MM的实际部署提供了一个有效的解决方案，其高效的设计和广泛的应用范围使其适用于多种HE参数集。
