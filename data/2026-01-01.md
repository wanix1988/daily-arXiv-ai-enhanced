...
TL;DR: 通过利用高保真合成先验和动态矢量低秩适应，该研究在高反射环境下实现了准确的单目深度估计，显著提升了在薄手术工具和透明表面的边界处的性能。


<details>
  <summary>Details</summary>
Motivation: 准确的单目深度估计对于机器人外科手术至关重要，但在具有镜面反射和充满流体的内窥镜环境中，现有的自监督方法常由于边界坍塌等问题性能不佳。

Method: 作者利用Depth Anything V2的高保真合成先验来捕捉薄结构的精确几何细节，并通过Dynamic Vector Low-Rank Adaptation (DV-LORA) 进行医疗领域的高效适应，减少参数预算同时弥合合成与真实环境的差距。

Result: 在SCARED数据集上，本研究提出的方法准确性达到98.1%，相比现有基线，Squared Relative Error减少了超过17%，显示出在不良手术照明条件下的优异鲁棒性。

Conclusion: 该研究工作展示了在高反射环境中单目深度估计的新水准，为解决现有方法在精细结构上的挑战提供了新的见解。
...
TL;DR: 本文提出了一种基于视频的评估pipeline，无需额外硬件，通过计算机视觉模型提取2D骨架、视线向量和运动轨迹，量化心理运动流畅性、情境意识和团队合作等任务特定指标，以支持具体领域的行动反馈，并扩展到3D数据分析。


<details>
  <summary>Details</summary>
Motivation: 当前的自动评估方法在客观评估认知、心理运动和团队合作技能方面仍面临挑战，传统的评估手段依赖昂贵的传感器或主观的人类观察，限制了评估的规模和准确性。因此，本文旨在改进这一过程以实现更加客观和高效的人体行为评估。

Method: 本文通过计算机视觉模型从训练视频中提取人体2D骨架、视线向量和运动轨迹来开发任务特定的评估指标。这些指标通过加权组合进入扩展的认知任务分析（CTA）层次结构，配备互动仪表板，以提供直观且易于理解的反馈，适用于GIFT（通用智能辅导框架）和Gamemaster。

Result: 本文实现了通过视频数据进行自动评估，提供了适用于具体领域的量化指标，能够捕捉个体和团队的表现，并能在交互式仪表板上为After Action Reviews提供洞察与反馈。

Conclusion: 本文表明，基于视频的评估方法展示了在合成训练环境中进行大规模、准确的认知和心理运动技能评估的潜力，虽然还存在一些局限性，但通过进一步研究可以改良方法并拓展到3D视频数据。
...
TL;DR: PFP是一种深度学习结构，用于压缩长视频至短语境，并在预训练阶段保持视帧的高频细节。该模型便于作为视频自回归模型的记忆编码器，有助于降低上下文成本并保持较低的建模误差。


<details>
  <summary>Details</summary>
Motivation: 现有技术中的视频压缩方法通常是将长时间序列数据压缩成有限长度的序列，但可能会丢失短时间尺度内的信息。PFP旨在实现高效的长视频压缩，同时保留高频率信息，便于记忆和后续模型的使用。

Method: PFP包含一个预训练阶段，在该阶段中，模型学习从任意时间点上抽取帧并保持其视觉属性，之后模型通过调整可以在保持一定解真度的条件下实现长视频的记忆。

Result: PFP能够将20秒的视频压缩至约5k长度的语境中，随机抽取的帧具有感知上可识别的外观。经验证，这种方法可以被视为高效的、可微的记忆编码器，支持知识的记忆。

Conclusion: 通过PFP模型，可以实现高效的视频上下文压缩，同时保持高帧率信息，使模型能够更好地支持自回归视频建模。
...
TL;DR: 本文提出了一种深度学习驱动的管道，将儿童0.2至2岁之间的T1加权MRI转换为合成CT (sCT)，以预测详细的颅骨分割，生成缝合概率热图，并直接从热图中进行缝合分割。这种方法生成的颅骨sCT在视觉上与实际CT一致，并解决了MRI对骨和缝合体描绘有限的问题。


<details>
  <summary>Details</summary>
Motivation: 儿童CT成像存在电离辐射风险，尤其是在没有严重异常的情况下。MRI可以提供辐射自由的成像，但由于其不能显示颅缝、估计头骨密度或评估颅骨穹窿生长，因此无法替代CT。为此，本文提出了一种基于深度学习的方法，将MRI图像转化为合成CT，旨在解决CT和MRI之间的局限性。

Method: 本文利用了高效的域特定变分自编码器，将儿童T1加权MRI图像转化为合成CT。该方法包括以下步骤：1) 预处理MRI数据；2) 利用变分自编码器进行图像转换；3) 预测颅骨分割，并生成缝合概率热图；4) 根据热图直接进行缝合分割。此外，使用两个一边检验 (TOST) 证实了颅骨和缝合分割从sCT与实际CT的等效性。

Result: 该方法生成的sCT在结构相似性指数（99%）和弗雷歇丁基恩距离（1.01）方面与实际CT相近；颅骨分割的平均Dice系数为85%，而缝合分割的Dice系数为80%。通过TOST分析，验证了sCT和实际CT之间的颅骨与缝合分割的等效性。

Conclusion: 本文提出的方法开创性地使在sCT上进行缝合分割成为可能，从而填补了无侵入性颅部评估中存在的空白。此方法可以用于临床环境，提供了一种非侵入性的评估儿童颅部和颅缝的方法。
...
TL;DR: 本文提出了一种新型的元引导多模态学习(MGML)框架，该框架包括元参数化自适应模态融合和一致性正则化模块。该方法在多种模态缺失的情况下，能够有效整合信息，同时增强分割性能并提高泛化能力。在BraTS2020和BraTS2023公开数据集上，该方法相比之前的多种先进方法，显示了优越的性能。


<details>
  <summary>Details</summary>
Motivation: 在临床实践中，多模态MRI数据经常不完整，寻找有效利用这些不完整信息的方法，对脑肿瘤分割尤为重要。

Method: 该方法包括两个组成部分：元参数化自适应模态融合 (Meta-AMF) 和一致正则化模块。Meta-AMF 能够在不同输入条件下有效整合多种模态信息，并基于可用模态生成适应性软标签监督信号以促进更一致的多模态融合。一致性正则化模块则进一步提升分割性能并增强整体框架的鲁棒性和泛化能力。

Result: 在BraTS2020和BraTS2023数据集上进行了大量实验，结果表明该方法相较于多种之前的方法表现更优。特别是在BraTS2020数据集上，对于15种模态缺失组合的平均Dice分数分别达到87.55、79.36和62.67，分别对应肿瘤区（WT）、肿瘤核心（TC）和增强肿瘤（ET）。

Conclusion: 该研究提出的方法展示了在多模态不完整的情况下有效分割脑肿瘤的潜力，并在公开数据集上的实验证明了其优越性。
...
TL;DR: 本文提出了一种利用AI驱动的框架进行显微吻合手术过程中的自动化动作分割及技能评估。该框架在边缘计算平台上高效运行，包含对象尖端追踪、动作分割及监督分类三个模块，实验结果显示其在动作分割和技能评估方面均表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有的显微手术评估方法依赖于专家监督或视频审查，这些方法存在主观性、评估差异大和耗时长的问题。因此，需要一种客观、实时、可扩展的自动评估方法来改进显微手术培训。

Method: 该方法采用基于YOLO和DeepSORT的对象尖端追踪和定位模块，结合自相似矩阵进行无监督动作边界检测和分割；监督分类模块用于评估手术手势熟练度。

Result: 在包含58个专家评估显微吻合手术视频的数据集上进行的实验验证表明，该方法在动作分割准确率为92.4%、总体技能分类准确率为85.5%，能够提供客观的实时反馈，启用标准的数据驱动培训。

Conclusion: 该研究提出的方法有望为高风险手术环境中的手术技能评估提供标准化的解决方案，推动手术技能培训的标准化和数据驱动型发展。
...
### [7] [U-Net-Like Spiking Neural Networks for Single Image Dehazing](https://arxiv.org/abs/2512.23950)
...
TL;DR: 本文研究了文本到视频（T2V）生成模型在对抗攻击方面的脆弱性，通过两种不同的攻击目标和两种攻击方法，在多个模型上进行了实验，发现即使是小的提示修改也会导致语义和时间动态的明显下降。


<details>
  <summary>Details</summary>
Motivation: 当前文本到视频模型的设计虽取得了显著成果，但在对抗攻击方面的不足仍然没有得到充分研究。研究者的动机是为了解决这一问题，提供一种新的视角来增强模型的鲁棒性。

Method: 研究者提出了两种攻击方法：一种是T2VAttack-S，通过贪婪搜索替换关键单词；另一种是T2VAttack-I，通过迭代插入优化的单词，这些方法针对语义和时间动态进行了两种新的攻击目标的研究。

Result: 通过对几个最先进的T2V模型（ModelScope, CogVideoX, Open-Sora, HunyuanVideo）的实验，研究者表明即使是轻微的提示修改也会影响模型的语义和时间动态，揭示了当前T2V模型的关键漏洞。

Conclusion: 研究者得出结论，T2V模型在对抗攻击方面存在明显的脆弱性，特别是在语义和时间动态方面，这为提升未来T2V模型的安全性提供了重要的启示。
...
TL;DR: 本文提出了一种利用图像和相机姿态重建场景的方法，通过融合全局静态点云和逐帧动态点云，结合4D可变形高斯框架重建场景，并使用视频扩散模型进一步优化图像质量，实现了在新型态视图上的有效视图外推。


<details>
  <summary>Details</summary>
Motivation: 传统的视图外推方法依赖于昂贵的传感器或人工标注的数据，本文的方法仅需图像和相机姿态就能实现视图外推，大幅降低了成本和标注需求。

Method: 首先，文章利用图像和相机姿态估计全局静态点云和逐帧动态点云，并将这些点云融合成统一表示；然后使用4D可变形高斯框架重建场景；接着，通过训练的4D高斯模型生成降级和伪图像，用于训练视频扩散模型；最后，通过扩散模型逐步优化生成的图像，并将优化结果反馈用于模型进一步训练，直到达到目标视点。

Result: 与基线方法相比，本文的方法能够生成更高质量的在新型态视图上的图像。

Conclusion: 本文提出了一种基于图像和相机姿态的新方法，通过融合点云和4D可变形高斯框架实现场景重建，并利用扩散模型优化图像生成效果。
...
TL;DR: 该研究利用深度学习方法分析遥感图像，通过学习预测地面在无变化情况下的外观，能够以前所未有的敏感度检测出不可见变化，尤其适用于地震等突发事件后的地表变化检测。


<details>
  <summary>Details</summary>
Motivation: 遥感图像变化检测面临气象噪声、季节波动和传感器伪影的复杂影响，而传统的检测方法难以有效应对。本研究旨在提供一种基于深度学习的方法，以更精细地捕捉地表变化，特别是在突发事件后的快速响应中。

Method: 研究团队开发了一种修复模型，基于SATLAS基础模型，用于从之前的遥感图像序列中预测并重构最后一帧图像，从而揭示传统方法未能识别的变化。

Result: 方法在2023土耳其-叙利亚地震序列触发的地表破裂检测中表现出色，特别是在特佩汗地区发现了新的裂缝特征。检测灵敏度和特异性显著优于时间中位数和Reed-Xiaoli异常检测器。

Conclusion: 该研究表明，基于深度学习的预测模型在地表变化检测中具有巨大潜力，能实现自动化、全球规模的变化监测，尤其适用于利用免费的多光谱遥感数据。
...
TL;DR: 该研究提出了GASeg，一种利用拓扑信息结合几何与外观特征的语义分割方法。通过差异化的盒计数模块量化多尺度拓扑统计数据，并引入对抗性策略TopoAug以增强模型学习稳定结构表示。实验结果显示GASeg在多个基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 传统的自监督语义分割方法在面对外观模糊性时表现不佳，本文研究则指出这是由于过度依赖不稳定的外观特征如阴影、眩光和局部纹理。为解决这一问题，本文提出GASeg框架来结合几何信息和拓扑稳定信息。

Method: GASeg的核心是一个差异化的盒计数(Differentiable Box-Counting, DBC)模块，用于量化几何和外观特征的多尺度拓扑统计数据。此外，TopoAug引入了对抗性策略，通过形态学运算模拟现实世界中的不确定性。最后通过一种多目标损失函数(GALoss)来强制几何和外观特征之间的跨模态对齐。

Result: 在COCO-Stuff, Cityscapes和PASCAL等四个基准测试上，GASeg表现出优越的性能，验证了通过拓扑信息结合几何和外观特征的方法的有效性。

Conclusion: 本文提出了一种创新的GASeg框架，通过结合几何和拓扑结构信息，提高了自监督语义分割的稳定性与准确性。
...
TL;DR: 本文提出了一个新颖的工作，利用RPO期间捕捉的一系列图像来从头构建未知目标航天器的3D结构。该方法通过将太阳位置的先验知识纳入训练管道，提高了3DGS渲染的光度质量，从而在动态光照条件下有效学习3D结构。


<details>
  <summary>Details</summary>
Motivation: 鉴于在空间探测中的RPO过程中，光照条件变化快速且是动态的，传统的3D结构重建方法假设静态场景，难以适应这种变化。本文旨在通过引入太阳位置的先验知识，提升基于3D Gaussian Splatting (3DGS)模型的几何和光度准确性，从而更好地适应动态光照条件。

Method: 该方法首先将目标航天器的几何形状和外观表示为3DGS模型。然后，在训练3DGS模型时，引入了太阳位置的信息作为先验知识，以改进光照条件下的光度质量。最后，通过光度优化实现相机姿态估计。

Result: 实验结果表明，这种方法能够在快速变化的光照条件下有效学习3D结构，并准确反映全局阴影和自遮挡。

Conclusion: 本文提出的方法能够有效适应RPO过程中动态光照条件的变化，通过先验知识的引入提升了3DGS模型的光度精度，为后续的姿态估计任务提供了准确实用的几何模型。
...
TL;DR: 本文提出了一种名为Conditioned Velocity Correction (CVC)的方法，通过重新定义速度在跨分布转换中的作用，提高流基编辑的结构保真度和语义一致性。CVC通过分两支路径进化潜空间：一条保持结构一致的路径，另一条受语义引导的路径控制向目标分布的偏移。此外，通过后验一致更新调整条件速度场，确保稳定且可解释的潜空间动态。


<details>
  <summary>Details</summary>
Motivation: 现有方法在进行图像分布转换时，会出现速度累积误差，导致语义不一致性和结构保真度丧失。CVC旨在解决这一问题，通过引入两种视角的速度转换机制，确保潜空间的稳定性和可解释性。

Method: CVC通过分解潜空间演化为两个分支：一个是保持结构一致性的分支；另一个是语义引导的分支。此外，使用Empirical Bayes Inference和Tweedie校正提出一个后验一致的更新机制，以补偿在时间上的绝对速度误差。

Result: 实验结果表明CVC在不同任务中能提供较好的重建保真度、更好的语义对齐和更可靠的编辑行为。

Conclusion: CVC为流基编辑提供了一种新的视角，并通过精确纠正速度误差，实现了在保持结构保真度的同时增强语义一致性。
...
TL;DR: 该研究提出了适应性的虚拟试穿（fit-aware VTON），开发了FitControler插件，可无缝集成到现代VTON模型中以实现定制化的 fit 控制。这通过fit-aware布局生成器和多尺度fit注入器来实现。


<details>
  <summary>Details</summary>
Motivation: 现有技术主要关注服装细节的逼真渲染，但忽略了在塑造整体风格方面起到关键作用的服装合身性。

Method: 研究首先引入了一种适应性的布局生成器，可以根据去服装化处理后的表示重新绘制身体-服装布局。然后使用多尺度fit注入器传递布局线索，实现基于布局的虚拟试穿。

Result: 研究构建了fit-aware VTON数据集Fit4Men，包含13,000个不同合身的男体-服装对。同时引入了两种尺合一致性评估指标。实验结果表明，FitControler可以与各种VTON模型兼容，实现准确的合身控制。

Conclusion: 研究成果展示了适应性虚拟试穿的可能性，并且该方法能够与多种VTON模型集成，实现精准的合身控制。
...
TL;DR: 该研究提出了一种结构引导的分配原则，用于2D高斯散点图表示，通过将图像结构与表示能力和量化精度耦合，同时保持快速解码，提高了表示能力和RD性能。


<details>
  <summary>Details</summary>
Motivation: 现有2D高斯散点图（2DGS）管道在低比特率下未能高效利用资源，因此提出了一种新的策略来改进其率失真性能。

Method: 首先，提出了一种结构引导的初始化方法，根据自然图像在空间上的结构先验对2D高斯分布进行初始化。其次，在量化感知微调期间，提出了自适应的协方差参数量化，为复杂区域的小尺度高斯分配更高精度，而在其他地方分配较低精度。最后，引入了几何一致性的正则化方法，将高斯的朝向与局部梯度方向对齐，以更好地保留结构细节。

Result: 实验结果表明，与基线GSImage相比，在Kodak上节省了43.44%的BD率，在DIV2K上节省了29.91%。

Conclusion: 该方法在保持超过1000 FPS解码速度的同时，显著提高了2DGS的表示能力和RD性能。
...
TL;DR: MF-RSVLM是一种多特征融合的遥感视觉-语言模型，它能够有效提取和融合视觉特征，改善小且复杂的遥感场景结构捕捉，通过循环视觉特征注入方案减轻视觉遗忘，实验表明其在多种遥感任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有遥感视觉-语言模型在处理遥感图像时存在精细视觉特征提取不足和视觉遗忘问题，为了克服这些问题，研究提出了一种多特征融合的远程 sensing 视觉-语言模型（MF-RSVLM）。该模型通过学习多尺度视觉表示并结合全局上下文与局部细节，提高对遥感场景中小而复杂的结构捕捉。

Method: MF-RSVLM采用多特征融合的方法，通过学习多尺度视觉表示和结合全局上下文与局部细节来提高对遥感场景中小而复杂的结构捕捉。通过循环视觉特征注入方案确保语言模型保持在视觉证据的基础上，减少视觉遗忘。该模型在多种遥感任务中进行了广泛实验验证。

Result: 实验证明，MF-RSVLM在遥感分类、图像描述以及VQA任务上取得了最先进的或竞争性表现。

Conclusion: MF-RSVLM在遥感视觉-语言模型领域中表现出色，有助于提高遥感图像处理的准确性，为该领域提供了新的解决方案。
...
TL;DR: 本文提出了一种基于强化学习的可训练各向异性扩散框架，通过深度Q学习学习顺序选择扩散动作以适应不同的图像结构，从而改进传统方法，适用于去除三种常见噪声类型。


<details>
  <summary>Details</summary>
Motivation: 传统的各向异性扩散方法在复杂图像结构下表现不佳，本文旨在通过引入机器学习方法提升其性能。

Method: 本文提出了一个基于强化学习的可训练各向异性扩散框架。通过将去噪过程模型化为由深度Q学习学习顺序选择的朴素扩散操作序列，生成了一种有效的基于扩散的图像去噪器。

Result: 实验表明，该去噪器在去除三种常见噪声类型方面优于现有的基于扩散的方法，并能与代表性的深度CNN方法竞争。

Conclusion: 本文提出的方法在低水平视觉中的图像去噪任务上表现出色，具有较强的适应不同类型图像结构的能力。
...
TL;DR: 本文提出了一种新颖的鲁棒跨模态学习框架NIRNL，该框架通过跨模态边界保持（CMP）调整正负样本对的相对距离，通过邻域意识实例细化（NIR）识别纯子集、难例子集和噪声子集，制定不同的优化策略，最大化利用所有可用数据，减少错误传播，实验表明，NIRNL在三个基准数据集上达到最新性能，特别是在高噪声率下表现尤为出色。


<details>
  <summary>Details</summary>
Motivation: CMR模型在处理多模态数据时，由于大规模高质量标注数据的收集成本高昂且包含噪声，影响了模型检索性能。为此，需要研发出能够同时满足模型性能、标签校准可靠性及数据利用率的鲁棒CMR方法。

Method: 本文提出了NIRNL框架：首先采用CMP调整正负样本对的相对距离；接着使用NIR识别三类样本集，并为其设计不同优化策略；最后利用这些方法最大化数据利用，减少错误传播。

Result: NIRNL在三个基准数据集上表现优秀，特别是在噪声水平较高的情况下，展现了出色的鲁棒性。

Conclusion: NIRNL框架通过多阶段优化策略显著提升了鲁棒跨模态学习的性能，是处理含有噪声标注数据的有效方法。
...
TL;DR: 提出了一种结合平衡层次对比损失和解耦学习策略的检测Transformer（DETR）框架，解决了层级标签分布不均和类别间语义关系学习干扰的问题。


<details>
  <summary>Details</summary>
Motivation: 改进细粒度检测性能，解决现有方法中存在的层次标签数据分布不均及类别间语义关系学习干扰问题。

Method: 提出平衡层次对比损失并结合解耦学习策略，利用检测Transformer框架，引入学习类原型，均衡各层级类别的梯度贡献，分离对象查询用于分类和定位，实现任务特定的特征提取和优化。

Result: 在三个带有层次注释的细粒度数据集上进行的实验表明，该方法优于现有最优方法。

Conclusion: 该研究提出了一种新颖的方法，能够改善细粒度检测的表现。
...
TL;DR: RainFusion2.0 提出了一个用于视频和图像生成模型的硬件高效、低开销的稀疏注意力机制，通过块均值作为稀疏掩码预测代表，时空感知的 token 重新排列，以及一种专为视频生成场景设计的第一帧吸收机制，实现 80% 的稀疏度和视频质量不损失的情况下加速达到 1.5~1.8 倍的速度，并且其效果在不同硬件平台下具有良好的通用性。


<details>
  <summary>Details</summary>
Motivation: 现有的稀疏注意力方法存在硬件通用性差和预测开销大两大问题，RainFusion2.0 的提出旨在解决这些问题，同时提高图像和视频生成模型的计算效率并保持高质量。

Method: RainFusion2.0 采用了块均值作为稀疏掩码预测代表，实施了时空感知的 token 重新排列，并引入了专为视频生成设计的第一帧吸收机制，以降低计算开销并提高硬件效率。

Result: 实验结果显示，RainFusion2.0 在保证视频质量不降低的情况下，通过稀疏化达到 80% 的稀疏度，并能实现 1.5~1.8 倍的端到端加速，而且在不同硬件平台上的性能表现良好。

Conclusion: RainFusion2.0 提供了一种硬件高效且低开销的稀疏注意力机制，显著加速了视频和图像的生成模型，同时保证了生成质量和平台通用性。
...
TL;DR: 该研究提出了一种新的框架D$^2$VLM，它通过分拆学习过程并引入证据标记和新颖的因子偏好优化算法，改进了事件级别的视频理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有的视频语言模型在事件级别的时间定位精度上仍存在不足，同时，现有的模型也没有清晰地将时间定位和文本响应这两个任务进行逻辑分层。

Method: 研究者提出了一种名为D$^2$VLM的新框架，该框架采取了‘先定位后回答带有证据参考’的范式，并引入了证据标记以强调事件级别的视觉语义捕捉。同时，研究者引入了新型的因子偏好优化（FPO）算法，以更明确地将概率性时间定位建模纳入优化目标，从而同时优化时间定位和文本响应。

Result: 实验结果表明，该方法在各项任务上取得了显著的优势。

Conclusion: 该研究通过引入新的学习框架和算法，显著提高了事件级别的视频理解能力，并公开了源代码。
...
### [25] [Enhancing LLM-Based Neural Network Generation: Few-Shot Prompting and Efficient Validation for Automated Architecture Design](https://arxiv.org/abs/2512.24120)
...
### [28] [Bayesian Self-Distillation for Image Classification](https://arxiv.org/abs/2512.24162)
...
### [32] [PointRAFT: 3D deep learning for high-throughput prediction of potato tuber weight from partial point clouds](https://arxiv.org/abs/2512.24193)
...
TL;DR: CorGi和CorGi+是针对扩散变换器（DiT）的训练无损推理加速框架，通过缓存低贡献度的变换块并在后续步骤中重用它们来减少冗余计算，同时保持生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散变换器模型由于迭代去噪过程和大容量带来的高推理成本以及步骤间的大量重复计算，需要一种无需训练的加速方法。

Method: CorGi通过选择性重用扩散变换器中各步骤间低贡献度转变块的输出来减少冗余计算；CorGi+在文本转图像任务中利用每个块的交叉注意力图来识别关键标记，并应用部分注意力更新以保护重要对象的细节。

Result: CorGi和CorGi+分别在最先进的扩散变换器模型上实现了约2.0倍的平均加速，同时保持了高质量的生成。

Conclusion: 该研究提出的CorGi和CorGi+策略是首个无需训练的扩散模型推理加速方案，显著提高了扩散变换模型的推理效率。
...
TL;DR: 一种名为注意力精炼模块（ARM）的轻量化学习模块能够在不进行训练的情况下从CLIP的内部特征中提取并精炼出丰富的细节，这显著改进了开放词汇语义分割（OVSS）的表现。


<details>
  <summary>Details</summary>
Motivation: 现有的训练免费方法要么依赖于费时的外部基础模型，要么采用静态先验，这些方法在计算成本或效果上存在局限性。

Method: 提出的ARM模块通过自适应地融合不同层次的特征，使用语义导向的跨注意力块从较深的特征中选择并精炼富含细节的浅层特征，然后通过一个自注意力块进一步处理。

Result: ARM能够在多种基准测试上持续提升基础模型的表现，且几乎不增加推理负担。

Conclusion: ARM提供了一种高效且有效的方法来实现训练免费的开放词汇语义分割。
...
TL;DR: Mirage 是一种用于驾驶场景中逼真且一致的资产编辑的一步式视频扩散模型，通过结合从预训练的2D编码器注入到3D解码器中的时空无关的潜变量，以及分阶段的数据对齐策略，保证了空间细节的恢复和临时因果结构的维持。


<details>
  <summary>Details</summary>
Motivation: 现有的视频对象编辑方法难以同时保持高质量的视觉保真度和时间连贯性，本文提出了一种新的方法来解决这一问题，以改善视觉效果和时间一致性。

Method: 该方法利用了一种文本到视频的扩散模型，并结合从预训练的2D编码器注入到3D解码器中的时空无关的潜变量来恢复细节，同时保持时间上的因果关系。此外，还提出了一种包含粗略3D对齐和精细2D微调的两阶段数据对齐策略，以提高对齐度和提供清晰的监督。

Result: 实验证明，Mirage 在多种编辑场景下达到了高逼真度和时间一致性。此外，它还可以适用于其他视频到视频的转换任务，为未来的研究提供了一个可靠的基准。

Conclusion: Mirage 提供了一种新的方法，能够有效解决视频对象编辑中的时间连贯性和视觉保真度问题，具有广泛的应用前景。
...
### [44] [SenseNova-MARS: Empowering Multimodal Agentic Reasoning and Search via Reinforcement Learning](https://arxiv.org/abs/2512.24330)
...
### [45] [Spatial-aware Vision Language Model for Autonomous Driving](https://arxiv.org/abs/2512.24331)
...
### [46] [The Mechanics of CNN Filtering with Rectification](https://arxiv.org/abs/2512.24338)
...
### [48] [Forging Spatial Intelligence: A Roadmap of Multi-Modal Data Pre-Training for Autonomous Systems](https://arxiv.org/abs/2512.24385)
*Song Wang,Lingdong Kong,Xiaolu Liu,Hao Shi,Wentong Li,Jianke Zhu,Steven C. H. Hoi*

Main category: cs.CV

TL;DR: 本文提出了一个多模态预训练框架，旨在创建从多种车载传感器数据中获得的统一理解。该框架从单一模态基础模型到复杂的统一框架涵盖了多种预训练范式，提出了面向未来的通用多模态基础模型，有助于提升3D物体检测和语义占用预测等高级任务。


<details>
  <summary>Details</summary>
Motivation: 随着自主系统的快速发展，特别是自动驾驶车辆和无人机，如何从多模态车载传感器数据中构建真正的空间智能成为了一个迫切的需求。虽然基础模型在单一模态中表现出色，但在多种传感器如摄像头和激光雷达的数据整合方面仍面临挑战。

Method: 该论文提出了一个多模态预训练框架，分析了传感特性和学习策略的相互作用，提出了一个综合的预训练范式分类，包括单一模态基准和高级综合框架。此框架还通过整合文本输入和占用表示，提高了开放世界感知与规划的能力。

Result: 通过该框架，研究团队成功地实现了从单一模态到多模态基础模型的迁移学习，提高了3D物体检测和语义占用预测等高级任务的能力。此外，论文还探讨了面向未来的通用多模态基础模型的发展方向，克服了计算效率和模型可扩展性等关键瓶颈。

Conclusion: 该工作为未来的多模态基础模型研究铺平了道路，提出了实现全面空间智能的有效方法，并强调了对计算效率和模型规模性的关注。
...
TL;DR: DyStream 提出了一种实时生成实时讲话头视频的方法，通过流匹配的自回归模型和因果编码增强模块，实现了超低延迟的 flows 和高质量的唇部同步效果。


<details>
  <summary>Details</summary>
Motivation: 传统方法由于需要非因果上下文窗口，导致了高延迟，无法提供即时非言语反馈。DyStream 方法通过结合流匹配的自回归框架和因果编码增强模块，解决了这个问题。

Method: DyStream 使用了一个自回归的框架，引入了基于流匹配的头端来实现概率建模，并通过因果编码增强模块结合短时未来上下文以确保低延迟。

Result: 实验结果显示，DyStream 方法能够在每帧 34 毫秒内生成视频，确保整个系统延迟不超过 100 毫秒，并且在 HDTF 数据集上实现了离线和在线唇部同步评分分别为 8.13 和 7.61 的最佳效果。

Conclusion: DyStream 方法通过独特的设计提高了生成实时讲话头视频的质量和效率，是一种有效的解决高延迟问题的方案。
...
TL;DR: 本研究提出了一种新的基于AI的自动化评估框架，用于微吻合手术表现的评估。该系统通过视频变换器架构结合时空注意力机制，实现了手术视频中的精准动作识别和详细运动分析，最终在实验验证中取得了高准确率的评估结果。


<details>
  <summary>Details</summary>
Motivation: 现有的评估手术技能的方法依赖于专家监督，主要问题包括主观性强、一致性差及向低收入国家普及困难。

Method: 该研究采用了一种基于TimeSformer的视频变换器架构，并结合了层次时间注意力和加权空间注意力机制来实现对微吻合手术视频中精细动作的识别。同时，通过YOLO方法完成了对象检测与跟踪，来详细分析手术器械的运动。

Result: 在验证实验中，系统实现了87.7%的帧级动作分割精度，并通过后期处理将精度提高到93.62%；各技能方面的平均分类准确率达到76%，显示出系统在重复专家评估方面的高一致性。

Conclusion: 该系统的评估结果显示了其提供客观、一致且可解释反馈的潜力，有助于实现标准化、数据驱动的外科手术教育训练。
...
TL;DR: 通过引入类似以前工作中的框架，并结合Discrete Wavelet Transform (DWT)，研究Vision Transformer (ViT)编码器中的表示学习是否遵循组合性原则。


<details>
  <summary>Details</summary>
Motivation: 当前大部分关于Transformer模型的研究集中在语言任务上，本文旨在通过测试Vision Transformer (ViT)编码器的组合性来深入理解其在视觉任务中的表示学习。

Method: 使用Discrete Wavelet Transform (DWT)来获取输入依赖的基本单元，进而评估ViT编码器中的组合性。通过检查组合表示是否能够再现原始图像表示，来实证测试组合性在表示空间中的实现程度。

Result: 发现单一级别DWT分解的基本单元在潜在空间中的组合近乎完美，为探讨ViT如何组织信息提供了一个新的视角。

Conclusion: 该研究展示了通过组合基本单元可以获得表示空间中的组合性，为ViT架构的理解提供了新的见解。
...
### [53] [F2IDiff: Real-world Image Super-resolution using Feature to Image Diffusion Foundation Model](https://arxiv.org/abs/2512.24473)
...
### [54] [Using Large Language Models To Translate Machine Results To Human Results](https://arxiv.org/abs/2512.24518)
...
### [55] [Hierarchical Vector-Quantized Latents for Perceptual Low-Resolution Video Compression](https://arxiv.org/abs/2512.24547)
...
### [56] [PhyGDPO: Physics-Aware Groupwise Direct Preference Optimization for Physically Consistent Text-to-Video Generation](https://arxiv.org/abs/2512.24551)
...
### [57] [OCP-LS: An Efficient Algorithm for Visual Localization](https://arxiv.org/abs/2512.24552)
...
### [58] [Renormalization Group Guided Tensor Network Structure Search](https://arxiv.org/abs/2512.24663)
...
### [66] [LLHA-Net: A Hierarchical Attention Network for Two-View Correspondence Learning](https://arxiv.org/abs/2512.24620)
...
### [67] [FireRescue: A UAV-Based Dataset and Enhanced YOLO Model for Object Detection in Fire Rescue Scenes](https://arxiv.org/abs/2512.24622)
...
### [69] [From Sequential to Spatial: Reordering Autoregression for Efficient Visual Generation](https://arxiv.org/abs/2512.24639)
...
TL;DR: DarkEQA 是一个用于评估视觉语言模型在多级低光条件下感知能力的开放源代码基准。该基准通过控制衰减来评估第一人称观测的问答能力，同时调整视觉退化以模拟物理光照下降和传感器噪声，并通过ISP启发的渲染流程。所评估的模型在这些具有挑战性的视觉条件下展示了性能局限。


<details>
  <summary>Details</summary>
Motivation: 现有基准多在理想、光线充足的条件下评测视觉语言模型的能力，而实际应用中需要在各种视觉退化条件下，特别是低光、夜间或黑暗环境下的稳健运行能力。因此，提出了 DarkEQA 来填补这一空白。

Method: DarkEQA 所用的方法包括：在模拟的物理光照下降和传感器噪声的线性RAW空间中建模视觉退化，以及一个ISP启发的渲染流程，来系统地调整视觉衰减。接着评估一系列先进的视觉语言模型和低光图像增强模型在这些多级低光条件下的表现。

Result: 研究发现，视觉语言模型在低光条件下具有明显的局限性。通过 DarkEQA 的评估，可以更加准确地识别和分析这些模型在不利光照条件下的具体表现缺陷。

Conclusion: DarkEQA 作为一个新的基准，能够提供有意义的洞见，帮助改进视觉语言模型在低光条件下的性能。随着代码和数据集的发布，其他研究者可以在这些光照条件下进一步优化模型。
...
### [72] [EchoFoley: Event-Centric Hierarchical Control for Video Grounded Creative Sound Generation](https://arxiv.org/abs/2512.24731)
*Bingxuan Li,Yiming Cui,Yicheng He,Yiwei Wang,Shu Zhang,Longyin Wen,Yulei Niu*

Main category: cs.CV

TL;DR: EchoFoley提出了一个新颖的任务，旨在解决VT2A中的三个关键限制，通过引入EchoFoley-6k数据集和EchoVidia框架，实现了更精细的控制和更好的感知质量。


<details>
  <summary>Details</summary>
Motivation: EchoFoley旨在通过解决视觉和文本调节失衡、缺乏隔音细则定义以及弱指令理解和遵循的问题，改进视频文本音频转换。

Method: EchoFoley定义了一个新的视频导向声音生成任务，涉及事件级地方控制和层次语义控制。EchoVidia框架则是一种以声音事件为中心的行为生成方法，包含慢速-快速思维策略。

Result: EchoVidia显著提高了可控性和感知质量，相比最近的VT2A模型，在可控性上提高了40.7%，在感知质量上提高了12.5%。

Conclusion: EchoFoley-6k为大规模专业编写的基准数据集，EchoVidia框架展示了在VT2A领域取得的重大进展。
...
### [73] [Splatwizard: A Benchmark Toolkit for 3D Gaussian Splatting Compression](https://arxiv.org/abs/2512.24742)
...
TL;DR: 介绍了一个名为Splatwizard的统一基准工具包，旨在评估3D Gaussian Splatting压缩模型的效果。


<details>
  <summary>Details</summary>
Motivation: 现有基准工具缺乏对3DGS压缩技术特性的全面评估方法，因此需要设计一个统一的基准工具来解决这个问题。

Method: Splatwizard提供了一个易于使用的框架，可以实现新的3DGS压缩模型，并集成了一个管道来自动化计算关键性能指标。

Result: 使用Splatwizard可以方便地实现新的3DGS压缩模型，并评估其渲染速度、压缩率、几何精度等。

Conclusion: 该工具包的开发将有助于推动3DGS技术的进一步研究和发展。
...
### [74] [UniC-Lift: Unified 3D Instance Segmentation via Contrastive Learning](https://arxiv.org/abs/2512.24763)
*Ankit Dhiman,Srinath R,Jaswanth Reddy,Lokesh R Boregowda,Venkatesh Babu Radhakrishnan*

Main category: cs.CV

TL;DR: 该研究提出了一种统一的框架，将现有的两阶段方法合并，通过引入可学习的特征嵌入进行高效率的实例标签生成，结合新的三元组损失改进了形体边界处理。该方法在ScanNet、Replica3D和Messy-Rooms数据集上优于基线。


<details>
  <summary>Details</summary>
Motivation: 提出一种统一的框架以解决现有方法在多视图2D分割到3D难以保持一致性的挑战。现有方法多为两阶段分割，需后期补救步骤，或依赖于对比学习和先验的聚类，稳定性差且耗时。

Method: 新框架结合了实例分割和可学习特征嵌入，使用新颖的“嵌入到标签”过程生成实例标签，并采用线性层和三元组损失稳定边缘处理。

Result: 方法在ScanNet、Replica3D和Messy-Rooms数据集上表现出优于基线的性能。

Conclusion: 该方法的有效整合优化步骤和新颖处理边缘策略显著提升了新视图合成的质量和鲁棒性。
...
TL;DR: SpaceTimePilot 是一种视频扩散模型，能够独立控制时间和空间，通过时间嵌入机制实现出视频的运动序列控制，同时提出一种时空分离策略和数据集提升模型效果。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型难以同时控制时间和空间，SpaceTimePilot旨在解决这一问题，提供更加灵活的场景重构能力。

Method: SpaceTimePilot通过时间嵌入机制在扩散模型中引入动画时间编码，结合时空数据集训练策略，实现对时间和空间的独立控制。

Result: 实验表明，SpaceTimePilot在现实世界和合成数据上的表现优于现有方法，展示了清晰的时空分离效果。

Conclusion: SpaceTimePilot有效地分离了时间和空间，实现了对生成视频的精准控制，并贡献了首个时空全面覆盖的合成数据集CamxTime。
...
### [76] [Nonlinear Noise2Noise for Efficient Monte Carlo Denoiser Training](https://arxiv.org/abs/2512.24794)
*Andrew Tinits,Stephen Mann*

Main category: cs.CV

TL;DR: 该研究提出了一种改进的Noise2Noise方法，允许对HDR图像进行无噪声训练，同时减少非线性变换带来的偏差。


<details>
  <summary>Details</summary>
Motivation: 当前的Noise2Noise方法在处理HDR图像时存在困难，主要因为非线性变换会引入偏差。研究者希望通过改进该方法，减轻非线性变换对模型性能的影响。

Method: 研究者开发了一种理论框架来分析特定非线性函数对噪声图像的影响，并描述了一个具有最小偏差的非线性函数类别。他们还探索了特定的损失函数和色调映射函数组合，能够在减少异常值影响的同时，引入最少的偏差。

Result: 研究者将改进的方法应用于基于机器学习的Monte Carlo去噪器，发现可以使用这种方法仅用噪声数据训练出接近原实现结果的去噪器，而无需使用高样本数的参考图像。

Conclusion: 该研究为处理复杂噪声图像（如HDR图像）时改进Noise2Noise方法提供了理论支持和实证结果，从而扩大了该方法的应用范围。
...
### [77] [CropTrack: A Tracking with Re-Identification Framework for Precision Agriculture](https://arxiv.org/abs/2512.24838)
...
### [78] [VLN-MME: Diagnosing MLLMs as Language-guided Visual Navigation agents](https://arxiv.org/abs/2512.24851)
...
### [79] [OFL-SAM2: Prompt SAM2 with Online Few-shot Learner for Efficient Medical Image Segmentation](https://arxiv.org/abs/2512.24861)
...
### [80] [FinMMDocR: Benchmarking Financial Multimodal Reasoning with Scenario Awareness, Document Understanding, and Multi-Step Computation](https://arxiv.org/abs/2512.24903)
...
### [81] [Semi-Supervised Diversity-Aware Domain Adaptation for 3D Object detection](https://arxiv.org/abs/2512.24922)
...
### [82] [ProDM: Synthetic Reality-driven Property-aware Progressive Diffusion Model for Coronary Calcium Motion Correction in Non-gated Chest CT](https://arxiv.org/abs/2512.24948)
...
### [83] [VIPER: Process-aware Evaluation for Generative Video Reasoning](https://arxiv.org/abs/2512.24952)
...
### [84] [Bi-C2R: Bidirectional Continual Compatible Representation for Re-indexing Free Lifelong Person Re-identification](https://arxiv.org/abs/2512.25000)
...
### [85] [FoundationSLAM: Unleashing the Power of Depth Foundation Models for End-to-End Dense Visual SLAM](https://arxiv.org/abs/2512.25008)
...
### [86] [From Inpainting to Editing: A Self-Bootstrapping Framework for Context-Rich Visual Dubbing](https://arxiv.org/abs/2512.25066)
...
### [87] [FineTec: Fine-Grained Action Recognition Under Temporal Corruption via Skeleton Decomposition and Sequence Completion](https://arxiv.org/abs/2512.25067)
...
### [88] [Edit3r: Instant 3D Scene Editing from Sparse Unposed Images](https://arxiv.org/abs/2512.25071)
...
### [89] [GaMO: Geometry-aware Multi-view Diffusion Outpainting for Sparse-View 3D Reconstruction](https://arxiv.org/abs/2512.25073)
...
TL;DR: 本文介绍了一个名为CAT的框架，用于评估和可视化大型语言模型（LLMs）在可控输入变化下的准确性和响应一致性之间的相互作用。通过CAR曲线和CORE指标，展现了准确性和一致性的权衡关系。


<details>
  <summary>Details</summary>
Motivation: 当前对LLMs的评估主要集中在准确性或基准得分上，而CAT框架的引入旨在通过引入准确性和一致性之间的关系来提供更全面的评估，特别适用于高风险的实际应用场景。

Method: CAT框架采用了CAR曲线和CORE指数来衡量模型在不同一致性要求下的性能。CAR曲线展示准确性如何随着一致性要求的增加而变化，而CORE指数则作为全球指标合并了CAR曲线的面积和形状，量化了准确性和一致性的权衡。

Result: 应用CAT框架于多种通用和特定领域的大语言模型，在多个选择题基准上展示了其成果。此外，还提出了将其扩展至长文本开放性评价的策略。

Conclusion: CAT框架提供了一种新的评估LTXMs一致性和准确性的方法，有助于更全面地理解和评估这些模型在实际应用场景中的表现。
...
### [91] [PharmaShip: An Entity-Centric, Reading-Order-Supervised Benchmark for Chinese Pharmaceutical Shipping Documents](https://arxiv.org/abs/2512.23714)
*Tingwei Xie,Tianyi Zhou,Yonghong Song*

Main category: cs.CL

TL;DR: PharmaShip 提供了一个针对含噪 OCR 和异构模板的预训练文本布局模型的现实世界中文药品运输数据集，涵盖了序列实体识别、关系提取和阅读顺序预测三类任务，通过一个实体为中心的评估协议来进行基准测试，并公开了数据以便于研究。


<details>
  <summary>Details</summary>
Motivation: 为了在医学领域内提供一个用于安全文档理解的标准化基准测试数据集，尤其是在有噪音光学字符识别（OCR）和不一致模板的情况下。

Method: 设计并提供了 PharmaShip 数据集，包含了序列实体识别、关系提取和阅读顺序预测任务，并采用了实体为中心的评估标准来评估模型性能。

Result: 实验结果表明，像素和显式几何提供了互补的诱导偏置，单独使用两者都不足以获得最佳预测结果。引入了基于阅读顺序的正则化，有效提升了序列实体识别和实体链接的效果。而且，较大的位置覆盖率能够稳定预测跨页内容，减少剪辑伪影。

Conclusion: PharmaShip 是一个安全关键文档理解在药品领域的受控可重复基准测试数据集，并强调序列感知约束作为一种可转移偏置对于结构建模的重要作用。
...
### [92] [Noise-Driven Persona Formation in Reflexive Neural Language Generation](https://arxiv.org/abs/2512.23716)
*Toshiyuki Shigemura*

Main category: cs.CL

TL;DR: 本研究提出了一种名为Luca-Noise Reflex Protocol（LN-RP）的计算框架，通过在初始生成状态下注入随机噪音种子，发现大型语言模型中非线性语言行为过渡，并揭示了三种具有不同熵签名的稳定人模式。


<details>
  <summary>Details</summary>
Motivation: 该研究旨在探索外部噪音源如何影响大型语言模型中的反射性生成动态，从而识别新的非线性语言行为模式。

Method: 通过向初始生成状态注入不同随机噪音种子，研究者分析了152个生成周期内语言行为的变化，从而揭示不同的人格模式。

Result: 研究结果发现，外部噪音可以有效引发生成动态的相变，且不同的人格模式之间存在显著差异（p < 0.01），表明这些模式具有高度一致的人格保留。

Conclusion: 该研究提出了一种可重复的方法来研究反射性生成、涌现行为和大语言模型中的远距离语言连贯性。
...
### [93] [HarmTransform: Transforming Explicit Harmful Queries into Stealthy via Multi-Agent Debate](https://arxiv.org/abs/2512.23717)
*Shenzhe Zhu*
...
### [94] [Emergent World Beliefs: Exploring Transformers in Stochastic Games](https://arxiv.org/abs/2512.23722)
...
### [95] [When in Doubt, Deliberate: Confidence-Based Routing to Expert Debate for Sexism Detection](https://arxiv.org/abs/2512.23732)
*Anwar Alajmi,Gabriele Pergola*

Main category: cs.CL

TL;DR: 本文提出了一种双阶段框架，该框架通过目标训练程序和选择性推理将监督适应稀少和嘈杂的数据，以及在模棱两可或边缘情况下的联合处理。这种方法在多个基准测试中取得了最先进的成果。


<details>
  <summary>Details</summary>
Motivation: 随着在线性别歧视内容以更为微妙和上下文相关的形式出现，传统检测方法难以察觉，这导致注释数据标注稀少、类别不平衡以及噪声监督，这些都使得模型难以识别隐晦且未充分代表的形式。因此，有必要设计一种方法，明确处理数据和模型预测中的代表性不足、噪声和概念模糊问题。

Method: 提出了一种双阶段框架，包括目标导向的训练过程和选择性的推理过程。训练过程中使用了类别平衡的焦点损失、类别感知批量处理和后验阈值校准来缓解标签不平衡和监督的噪声问题。在推理阶段，通过动态路由机制直接分类高置信度情况或将不确定实例升级到协作专家判断模块，该模块通过法官模型引导多个化身并综合他们的推理。

Result: 该方法在多种基准测试中表现出色，分别在EXIST 2025任务1.1上的F1得分提高了2.72%，在EDOS任务A和B上分别提高了4.48%、1.30%。这一改进突显了新方法的有效性。

Conclusion: 该研究通过改进训练和推理相结合的方法，显著提高了检测在线性别歧视内容的能力。
...
### [96] [Entropy-Aware Speculative Decoding Toward Improved LLM Reasoning](https://arxiv.org/abs/2512.23765)
*Tiancheng Su,Meicong Zhang,Guoxiu He*

Main category: cs.CL

TL;DR: EASD（熵感知推测解码）通过引入基于熵的动态惩罚机制，在推测模型和目标模型之间引入不确定性衡量，从而提高推测解码的性能，使其有可能超越目标模型的固有表现。


<details>
  <summary>Details</summary>
Motivation: 当前的推测解码（SD）由于推测模型与目标模型之间的过度对齐，其性能受限于目标模型。为了克服这一限制，提出EASD作为一种无需训练的增强方法。

Method: EASD在标准SD的基础上，引入了一种基于熵的动态惩罚机制。它通过每次解码步骤中的采样分布的熵来量化模型不确定性，在预测结果具有高熵并且预测结果有重叠时，拒绝相应候选令牌并由目标模型重新采样。

Result: 在多个推理基准测试中，EASD展示了相对于现有SD方法的持续优势，并在大多数情况下超过了目标模型本身。而且EASD的效率与SD相当。

Conclusion: EASD提供了一种有效的方法，增强了推测解码技术，使其能够提升模型推理性能并超越传统方法及目标模型的实际性能。
...
### [97] [MiMo-Audio: Audio Language Models are Few-Shot Learners](https://arxiv.org/abs/2512.23808)
...
TL;DR: 研究通过大规模预训练音频模型MiMo-Audio，发现其在多种音频任务上实现了强大的泛化能力。经过指令微调后，MiMo-Audio展示了出色的理解和生成音频内容的能力。


<details>
  <summary>Details</summary>
Motivation: 借鉴GPT-3的成功，探索在音频数据上应用大规模预训练模型，以实现类似人类的学习能力。

Method: 大规模预训练MiMo-Audio模型，并通过对比实验展示其在多种音频任务上的泛化能力。在预训练之后，引入指令微调以增强模型在特定任务上的表现。

Result: 研究发现，MiMo-Audio-7B-Base在多个开源音频基准测试中取得了SOTA性能，并展示了强大的少样本学习能力。指令微调后的MiMo-Audio-7B-Instruct在多个音频理解与生成任务上达到了或接近封闭源模型的表现。

Conclusion: 大规模音频预训练模型能显著提升模型在各种音频任务上的表现，并且指令微调进一步增强了模型的特定任务处理能力。
...
### [98] [Explaining News Bias Detection: A Comparative SHAP Analysis of Transformer Model Decision Mechanisms](https://arxiv.org/abs/2512.23835)
*Himel Ghosh*

Main category: cs.CL

TL;DR: 该研究通过对比分析两种基于变压器的偏见检测模型（调优的偏见检测器模型和领域适应的预训练RoBERTa模型），揭示了它们在处理评价性语言时的不同方式，指出模型内部证据和预测正确性之间的不匹配导致了虚假阳性现象。


<details>
  <summary>Details</summary>
Motivation: 鉴于自动化偏见检测在新闻分析和媒体问责中的广泛应用，但人们对模型决策过程和失败原因了解甚少，本研究旨在通过对比分析两种模型，提高偏见检测系统的可解释性。

Method: 研究使用SHAP方法解释模型的决策，主要分析了两种模型在正确和错误预测中的词级归因，并考察了它们在处理评价性语言时的不同策略。

Result: 对比发现，两种模型都侧重于处理评价性语言，但模型间的归因方式有显著差异。调优的偏见检测器模型内部证据对虚假阳性现象的处理不如真实阳性的处理强，导致对中性新闻内容的系统性误报率高。相比之下，领域适应的模型归因更符合预测结果，且比调优模型产生的虚假阳性率减少了63%。模型错误主要由语境层面的歧义而非明确的偏见线索引起。

Conclusion: 研究强调了偏见检测系统需要具备解释性的评估，并指出架构和训练选择对模型的可靠性和在新闻环境中的适用性至关重要。
...
### [99] [Retrieval Augmented Question Answering: When Should LLMs Admit Ignorance?](https://arxiv.org/abs/2512.23836)
*Dingmin Wang,Ji Ma,Shankar Kumar*

Main category: cs.CL

TL;DR: 本文探讨了利用大语言模型（LLMs）进行检索增强问答，并设计了一种自适应提示策略来平衡相关和不相关信息的使用，实验表明该策略能与标准提示策略获得相似的效果，且使用更少的标记。


<details>
  <summary>Details</summary>
Motivation: 长上下文窗口在大规模语言模型中的成功应用激发了对利用广泛上下文进行检索增强生成的兴趣。当前方法中，虽然长上下文便于整合目标知识，但也引入了大量无关信息，干扰模型生成过程并降低其性能。

Method: 本文设计了一种自适应提示策略，将检索到的信息分成较小的部分，并顺序请求LLMs回答问题，通过调整分块大小在相关信息的利用与减少无关信息之间寻求权衡。

Result: 在三个开放领域问答数据集上的实验结果表明，自适应策略能够达到标准提示策略的性能，同时减少标记使用量。进一步分析指出，当面对不足信息时，模型更倾向于生成不准确的答案而不是拒绝响应。

Conclusion: 本文提出的方法在不需要大量额外标记的情况下提高了LLMs的性能，并揭示了增强LLMs处理不足信息能力的重要性。
...
### [100] [Adversarial Lens: Exploiting Attention Layers to Generate Adversarial Examples for Evaluation](https://arxiv.org/abs/2512.23837)
*Kaustubh Dhole*

Main category: cs.CL

TL;DR: 该研究利用中间注意力层的标记分布生成对抗性示例，这些示例在保持语义相似性的前提下，可以有效降低LLM在下游评估任务中的性能，但某些层和位置的标记替换可能导致语法退化。


<details>
  <summary>Details</summary>
Motivation: 鉴于机械可解释性进步表明中间注意层储存在每个标记级别的假设，并逐步细化以达到最终输出，该研究旨在探索利用这些内部标记预测来直接生成对抗性示例的可能性。其目的是测试这些内部预测生成的变化是否能够有效地扮演对抗性扰动的角色，而在不会剧烈改变输入语义的情况下损害模型性能。

Method: 研究使用了LLaMA-3.1-Instruct-8B模型进行生成和评估，并从中间注意层提取标记分布，基于这些标记分布生成对抗性扰动。

Result: 实验证明，基于注意力层的对抗性示例可以在不显著改变原始输入的前提下，显著降低评估模型的表现。然而，某些层和位置的标记替换在引入有效对抗性扰动的同时，也可能导致语法上的退化。

Conclusion: 研究表明，虽然中间层的标记可以直接用于生成对抗性示例，改善了对抗性攻击的内部一致性，但在实践中还存在语法一致性的挑战。这暗示了一种折中的策略可能适用于利用中间层信息提高对抗性攻击的效果，同时减轻对语法完整性的潜在损害。
...
### [101] [Integrating Domain Knowledge for Financial QA: A Multi-Retriever RAG Approach with LLMs](https://arxiv.org/abs/2512.23848)
*Yukun Zhang,Stefan Elbl Droguett,Samyak Jain*

Main category: cs.CL

TL;DR: 本研究通过使用多检索器Retrieve Augmented Generators (RAG)系统，结合领域特定训练与最新的大语言模型，显著提升了金融数值推理问答任务的表现，尤其是在小模型和少量示例情况下，强调了外部知识获取与幻觉风险之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 鉴于现有的大型语言模型在处理涉及金融专业知识和复杂多步骤数值推理的问答任务时存在局限，本研究旨在提高模型的性能，以更好地理解和回答这类问题。

Method: 研究实施了一个多检索器RAG系统，结合了外部领域知识检索和内部问题上下文检索。同时，利用SecBERT编码器进行领域特定训练，并采用最新的大规模语言模型进行模型生成。

Result: 结果显示，领域特定训练大大提高了神经符号模型的表现，超过了基线FinQA的顶级模型。基于提示的大型语言模型生成器也取得了最先进的性能，尽管依然低于人类专家的性能。研究还发现，对于较小的模型和少量示例，外部知识的获取与幻觉的减少之间存在权衡。

Conclusion: 研究确认了最新大规模语言模型在数值推理方面的能力提升，特别是在少量示范学习的情况下表现出色。然而，也指出了模型性能与外部知识获取和幻觉风险之间的持续权衡问题。
...
### [102] [Disentangling Learning from Judgment: Representation Learning for Open Response Analytics](https://arxiv.org/abs/2512.23941)
*Conrad Borchers,Manit Patel,Seiyon M. Lee,Anthony F. Botelho*

Main category: cs.CL

TL;DR: 研究提出了一种新的分析框架，通过分离内容信号和评分者倾向，使其更加透明和可审计。该方法在数学作业数据上取得了良好的效果，能够更好地反映学生的真实理解程度。


<details>
  <summary>Details</summary>
Motivation: 现有的自动评分方法往往将学生的回答与其教师评分混淆，使得评分过程难以捉摸和审计。因此，研究提出了一种新的分析框架来解决这一问题。

Method: 该方法采用未识别的学生数学回答数据，将教师历史看作动态先验，并从句子嵌入中提取文本表示。同时采用中性化和残差化方法来抵消提示和教师的混淆因素。引入了时空验证的线性模型来量化每个信号的贡献，并采用了投影表面模型进行定性检查。

Result: 实验结果显示，教师先验在分数预测中起着重要作用；结合内容嵌入后表现最佳（AUC ≈ 0.815），而仅使用内容模型则是随机猜测概率之上但表现较弱（AUC ≈ 0.626）。通过调整评分者效应，可以更精确地保留有意义的嵌入维度，揭示那些支持理解而非学生回答表面差异的案例。

Conclusion: 该研究提供了一种实际的流水线方法，能够将嵌入从简单的特征转化为学习分析，帮助教师和研究者反思评分实践是否与学生推理和学习的证据相一致。
...
### [103] [Improving Multi-step RAG with Hypergraph-based Memory for Long-Context Complex Relational Modeling](https://arxiv.org/abs/2512.23959)
*Chulun Zhou,Chunkang Zhang,Guoxin Yu,Fandong Meng,Jie Zhou,Wai Lam,Mo Yu*

Main category: cs.CL

TL;DR: 本文提出了一种基于超图的记忆机制HGMem，显著增强了多步检索增强生成(RAG)的能力，特别是在需要全局理解和长时间推理的任务上。


<details>
  <summary>Details</summary>
Motivation: 当前的RAG系统大多依赖静态的记忆模块，虽然可以积累孤立的细节信息，但在更高阶的关联推理以及全局理解方面存在一定局限性。

Method: 该研究提出了一种基于超图的记忆机制，该机制能够动态地捕捉复杂的交互关系，使得模型能够在推理过程中形成逻辑连贯的复杂知识结构，从而提升长期推理和全局理解能力。

Result: 在多项针对全球理解和长期推理的任务设计的数据集上，与强基线系统相比，使用HGMem的多步RAG表现更好。

Conclusion: 研究结论表明，与传统的基于静态存储的记忆机制相比，基于超图的记忆机制HGMem能显著提升多步RAG系统在全局理解和长期推理任务上的性能。
...
### [104] [Efficient Context Scaling with LongCat ZigZag Attention](https://arxiv.org/abs/2512.23966)
...
TL;DR: LoZA是一种稀疏注意力机制，旨在将现有的全注意力模型转换为具有有限计算预算的稀疏版本，在处理大语境时可以显著加快速度。


<details>
  <summary>Details</summary>
Motivation: 当前的全注意力模型在处理长上下文时面临计算效率低下的问题，而LoZA通过引入残差路径和降采样机制，使模型能够更快地处理长序列信息，提高模型在大规模文本处理任务中的性能。

Method: LoZA通过引入残差路径和降采样机制，将输入分为多个堆叠的稀疏注意力层，减少注意力机制的计算量。具体的实现步骤包括：首先，将输入进行降采样并通过残差路径连接，然后依次进行多个稀疏注意力层的处理，最后通过叠加得到最终的注意力输出。

Result: 通过应用LoZA到LongCat-Flash，我们成功地创建了LongCat-Flash-Exp，该模型能够在中训练阶段处理高达100万条令牌的长上下文，有效地提高了长期推理和远期智能的能力。

Conclusion: LoZA展示了在保持模型性能的同时显著降低计算成本的潜力，为处理大规模文本带来了新的解决方案。
...
### [105] [CEC-Zero: Zero-Supervision Character Error Correction with Self-Generated Rewards](https://arxiv.org/abs/2512.23971)
...
### [106] [Fantastic Reasoning Behaviors and Where to Find Them: Unsupervised Discovery of the Reasoning Process](https://arxiv.org/abs/2512.23988)
*Zhenyu Zhang,Shujian Zhang,John Lambert,Wenxuan Zhou,Zhangyang Wang,Mingqing Chen,Andrew Hard,Rajiv Mathews,Lun Wang*

Main category: cs.CL

TL;DR: 该研究提出了一种无监督框架RISE，用于发现推理向量，这些向量表示激活空间中的方向，对应于不同的推理行为。通过分析链式思考片段，RISE可以识别出‘反思’和‘回溯’等行为，并通过干预向量实现对推理行为的控制。


<details>
  <summary>Details</summary>
Motivation: 先前采用有监督方式依赖人类定义的概念（如过度思考、反思）来分析语言模型的推理，但这种方法难以捕捉所有潜在推理行为，难以定义在标记空间中的行为。RISE通过无监督方法提出了解决方案，旨在揭示模型在推理过程中的内在机制。

Method: RISE框架首先将推理过程拆分为句子级别的步骤，然后使用稀疏自动编码器(SAEs)在步骤级激活上进行训练，从而发现与可解释行为（如反思、回溯）对应的分离特征。研究使用可视化和聚类分析来展示这些行为在解码器列空间中的分离区域。

Result: RISE分析显示，特定的推理行为可在解码器列空间中占据分离的区域。通过干预SAE产生的向量，可以有控制地放大或抑制特定的推理行为，而不需重新训练模型。此外，SAE还揭示了推理轨迹的结构属性，例如响应长度，且能够发现超越人类监督的新行为。

Conclusion: RISE通过无监督的隐空间发现技术，不仅揭示了语言模型推理行为背后的不同机制，还提供了控制这些行为的手段。该研究为理解与调控大型语言模型的推理过程提供了新的视角和技术路径。
...
### [107] [WISE: Web Information Satire and Fakeness Evaluation](https://arxiv.org/abs/2512.24000)
*Gaurab Chhetri,Subasish Das,Tausif Islam Chowdhury*

Main category: cs.CL

TL;DR: 该研究开发了WISE框架，并对8种轻量级Transformer模型及两种基础模型进行了评估。评估使用了平衡的数据集和多种评价指标，结果显示MiniLM在准确率上领先，而RoBERTa-base在ROC-AUC上表现最佳。


<details>
  <summary>Details</summary>
Motivation: 由于假新闻和讽刺报道在语言特征上有重叠且意图不同，区分它们造成了独特的挑战。因此，研究设计了一种评估框架，以期在实际资源有限的环境中提升假信息检测系统的效能。

Method: 研究构建了WISE框架并使用了平衡的数据集，包括2万个标注为假新闻或讽刺报道的样本。使用了五折分层交叉验证，评估了多个模型，在多项指标如准确率、精确率、召回率、F1分数等上进行了比较。

Result: 实验结果显示，MiniLM在准确性方面达到了87.58%，而RoBERTa-base在ROC-AUC方面达到95.42%，高效率模型DistilBERT则提供了良好的效能与耗时权衡。

Conclusion: 研究表明轻量级模型能够达到或超越基础模型的性能，这对于在实际资源受限环境下部署假信息检测系统具有指导意义。
...
### [108] [iCLP: Large Language Model Reasoning with Implicit Cognition Latent Planning](https://arxiv.org/abs/2512.24014)
*Sijia Chen,Di Niu*

Main category: cs.CL

TL;DR: 该研究提出了一种名为iCLP的新框架，帮助大型语言模型生成隐式计划，从而提高问题解决过程中的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在执行准确和高效的推理步骤时依然存在挑战，特别是由于幻觉和任务特定问题的高度多样性。

Method: iCLP框架首先从现有的逐步推理路径中提取明示计划，然后通过带有代码簿的向量量化自编码器学习这些计划的离散表示。最后，通过对隐式计划和对应的推理步骤微调大型语言模型，模型在推理过程中学会进行隐式计划。

Result: 在数学推理和代码生成任务上，iCLP表现出了显著的准确性和效率提升，并展示了强大的跨域泛化，同时保持了逐步推理的可解释性。

Conclusion: iCLP框架通过使用隐式计划提高了大型语言模型在问题解决中的性能，并展示了该方法在实际应用中的潜力。
...
### [109] [Beyond Hallucinations: A Composite Score for Measuring Reliability in Open-Source Large Language Models](https://arxiv.org/abs/2512.24058)
*Rohit Kumar Salla,Manoj Saravanan,Shrikar Reddy Kota*

Main category: cs.CL

TL;DR: 提出了一个名为CRS的新框架来全面评估LLMs的可靠性，并通过实验证明了其优越性。


<details>
  <summary>Details</summary>
Motivation: 在决策关键领域（如医疗、法律和金融）中，我们亟需一个统一的框架来全面评估LLMs的可靠性，而现有评价方法往往只关注单一方面，未能提供综合的表现评估。

Method: 研究引入了CRS框架，该框架通过集成模型的校准性、稳健性和不确定性量化三个维度，旨在形成一个单一的可解释性度量标准。在十个主流开源LLMs上进行了跨五个问答数据集的实验，评估了其在基线、扰动和校准方法下的表现。

Result: CRS提供了稳定且可解释的模型排名，并发现了单一指标无法发现的隐藏故障模式，强调了最可靠的系统在准确性、稳健性和校准不确定性之间的平衡。

Conclusion: 研究展示了CRS框架的潜力，为评估LLMs的综合性能提供了一种新的方法，尤其适用于决策关键的应用场景。
...
### [110] [HY-MT1.5 Technical Report](https://arxiv.org/abs/2512.24092)
*Mao Zheng,Zheng Li,Tao Chen,Mingyang Song,Di Wang*

Main category: cs.CL

TL;DR: HY-MT1.5系列模型通过一套综合训练框架展示了卓越的参数效率，在多个翻译任务上显著超越了大型开源模型和主流商用API。


<details>
  <summary>Details</summary>
Motivation: 模型发展的动机是在保持高性能的同时，大幅提升模型的参数效率，以促进更广泛的应用场景。

Method: HY-MT1.5系列模型采用了一种全面的训练框架，该框架包括通用和翻译定向的预训练、受监督的微调、基于策略的蒸馏和强化学习。

Result: HY-MT1.5-1.8B在标准的中英和其他任务中表现优异，尤其在参数规模上显示了高效的性能。而HY-MT1.5-7B创造了同一参数规模的新记录，并在某些小众语言和WMT25基准中超越了大型同类模型。

Conclusion: 这两个模型在广泛的翻译任务和特定的应用场景下提供了极具竞争力的解决方案，证明了它们的实用性和价值。
...
### [111] [Training a Huggingface Model on AWS Sagemaker (Without Tears)](https://arxiv.org/abs/2512.24098)
*Liling Tan*

Main category: cs.CL

TL;DR: 本文旨在通过汇集研究人员所需的关键信息，帮助他们从零开始使用AWS SageMaker训练Hugging Face模型，从而简化云平台的采用。


<details>
  <summary>Details</summary>
Motivation: 当前云平台的学习曲线陡峭，且现有文档存在知识缺口，使得研究人员难以从零开始使用AWS SageMaker训练Hugging Face模型。

Method: 文章通过演示和汇总所需的信息来帮助研究人员。

Result: 文章提供的信息可能使研究人员更容易理解并成功使用AWS SageMaker。

Conclusion: 该论文成功地提供了研究人员所需的关键信息，有助于云平台的普及和技术的广泛应用。
...
### [112] [Activation Steering for Masked Diffusion Language Models](https://arxiv.org/abs/2512.24143)
...
### [113] [Large Emotional World Model](https://arxiv.org/abs/2512.24149)
...
### [114] [Training Report of TeleChat3-MoE](https://arxiv.org/abs/2512.24157)
...
### [115] [MedKGI: Iterative Differential Diagnosis with Medical Knowledge Graphs and Information-Guided Inquiring](https://arxiv.org/abs/2512.24181)
...
### [116] [LAILA: A Large Trait-Based Dataset for Arabic Automated Essay Scoring](https://arxiv.org/abs/2512.24235)
...
### [117] [Tracing the Flow of Knowledge From Science to Technology Using Deep Learning](https://arxiv.org/abs/2512.24259)
...
### [118] [Joint Selection for Large-Scale Pre-Training Data via Policy Gradient-based Mask Learning](https://arxiv.org/abs/2512.24265)
...
### [119] [Automated Analysis of Sustainability Reports: Using Large Language Models for the Extraction and Prediction of EU Taxonomy-Compliant KPIs](https://arxiv.org/abs/2512.24289)
...
### [120] [Figure It Out: Improving the Frontier of Reasoning with Active Visual Thinking](https://arxiv.org/abs/2512.24297)
...
### [121] [QianfanHuijin Technical Report: A Novel Multi-Stage Training Paradigm for Finance Industrial LLMs](https://arxiv.org/abs/2512.24314)
...
### [122] [Skim-Aware Contrastive Learning for Efficient Document Representation](https://arxiv.org/abs/2512.24373)
*Waheed Ahmed Abro,Zied Bouraoui*

Main category: cs.CL

TL;DR: 本文提出了一种新的自监督对比学习框架，通过随机遮蔽文档的一部分，并使用基于自然语言推理的对比目标来增强长文档的表示，从而获得更丰富且计算更高效的表示。


<details>
  <summary>Details</summary>
Motivation: 现有基于变换器的模型虽然在词级和句级任务中表现出色，但在长文档表示，尤其是在法律和医学领域，仍有改进空间。本文旨在克服现有模型的局限性。

Method: 本文提出的方法包括随机遮蔽文档的某一部分，并使用基于自然语言推理的对比目标来构建有监督学习框架。

Result: 该方法在法律和医学文本上的实验结果显示了显著的准确性和效率提升。

Conclusion: 此方法通过模仿人类的阅读策略，提出了一个新的自监督对比学习框架，提高了长文档的表示性能。
...
### [123] [Comparing Approaches to Automatic Summarization in Less-Resourced Languages](https://arxiv.org/abs/2512.24410)
*Chester Palen-Michel,Constantine Lignos*

Main category: cs.CL

TL;DR: 本文对比了多种摘要生成方法，包括零样本提示大模型和小模型、微调较小的模型（如mT5）、多语言迁移微调以及LLM翻译流水线方法。实验使用五种不同的评估指标，发现不同大小的LLM在类似参数规模下的性能存在差异，多语言微调mT5基线在大多数指标上优于其他方法，但在较少资源的语言上LLM作为裁判不够可靠。


<details>
  <summary>Details</summary>
Motivation: 较少资源语言的自动文本摘要性能相对较弱，本文旨在研究不同方法在这些语言中的有效性，以提高其性能。

Method: 本文采用了零样本提示、模型微调（包括多语言迁移微调）、以及LLM翻译流水线等方法进行自动文本摘要，并通过多种评估指标进行比较。

Result: 研究发现，不同大小的LLM在相同参数量级下表现出差异性；多语言微调的mT5模型在大多数评估指标上获得了较好的效果；而LLM作为评判标准在较少资源的语言上表现不稳定。

Conclusion: 本文介绍了多种自动摘要方法在较少资源语言上的应用效果，并强调了多语言微调mT5模型的有效性，但同时也指出在复杂或较少资源语言上评估LLM表现的局限性。
...
### [124] [Cleaning English Abstracts of Scientific Publications](https://arxiv.org/abs/2512.24459)
*Michael E. Rose,Nils A. Herrmann,Sebastian Erhardt*

Main category: cs.CL

TL;DR: 该研究介绍了一个开源的、易于集成的语言模型，用于自动识别并清除科学论文摘要中的冗余信息，从而提高摘要的质量和相似度评估的准确性。


<details>
  <summary>Details</summary>
Motivation: 当前，科学论文摘要中时常包含额外信息，这会干扰基于文档相似性和文本嵌入的下游分析。因此，研究需要开发一种能够减少这些干扰信息的工具。

Method: 研究团队开发了一个基于语言模型的工具，能够自动识别并移除摘要中的出版社版权声明、部分标题、作者注释、注册信息以及元数据等冗余内容。

Result: 实验结果表明，该模型不仅保守地移除了这些冗余信息，而且提高了清洗后摘要之间的相似性排名，并增强了标准化摘要长度的文本嵌入的信息量。

Conclusion: 这项研究提供了一种有效的方法来清理科学论文中的摘要，从而为更准确的数据分析提供了支持。
...
### [125] [IELTS Writing Revision Platform with Automated Essay Scoring and Adaptive Feedback](https://arxiv.org/abs/2512.24460)
*Titas Ramancauskas,Kotryna Ramancauske*

Main category: cs.CL

TL;DR: 该论文提出了一种辅助IELTS写作考试候选人的修订平台，通过迭代的设计方法与基于Transformer的模型实现了自动化评分和适应性反馈，在早期验证了规则基础方法的局限性，并最终展示了在适应性反馈方面的显著改进。


<details>
  <summary>Details</summary>
Motivation: 传统的IELTS准备方法缺乏个性化的反馈，为了提供针对性更强的支持，该平台引入了自动评分系统和适应性反馈，并在迭代的设计过程中逐步优化。

Method: 研究采用了迭代的设计基于研究(DBR)方法，首先通过规则基础的方法建立模型，发现其局限性后改用基于DistilBERT的Transformer模型，并结合回归头实现自适应反馈。

Result: 通过DistilBERT模型的引入，平台在评分准确性上取得了显著提升，同时适应性反馈也证明了其在提高成绩方面的有效性，尽管不同修订策略的效果有所差异。此外，自动化反馈更适合作为人机交流的补充。

Conclusion: 实验结果显示，自动化反馈机制能够显著提高考生的考试分数，但表面级别的纠正比激进的结构性干预更可靠。未来研究需要进一步探索更高分数段的评估和与真实考生及官方评分员的验证。
...
### [126] [Paragraph Segmentation Revisited: Towards a Standard Task for Structuring Speech](https://arxiv.org/abs/2512.24517)
...
### [127] [Safe in the Future, Dangerous in the Past: Dissecting Temporal and Linguistic Vulnerabilities in LLMs](https://arxiv.org/abs/2512.24556)
...
### [128] [HaluNet: Multi-Granular Uncertainty Modeling for Efficient Hallucination Detection in LLM Question Answering](https://arxiv.org/abs/2512.24562)
...
### [129] [Korean Canonical Legal Benchmark: Toward Knowledge-Independent Evaluation of LLMs' Legal Reasoning Capabilities](https://arxiv.org/abs/2512.24572)
...
### [130] [Understanding and Steering the Cognitive Behaviors of Reasoning Models at Test-Time](https://arxiv.org/abs/2512.24574)
...
### [131] [Youtu-LLM: Unlocking the Native Agentic Potential for Lightweight Large Language Models](https://arxiv.org/abs/2512.24618)
...
### [132] [Do Large Language Models Know What They Are Capable Of?](https://arxiv.org/abs/2512.24661)
...
### [133] [R-Debater: Retrieval-Augmented Debate Generation through Argumentative Memory](https://arxiv.org/abs/2512.24684)
...
### [134] [MUSIC: MUlti-Step Instruction Contrast for Multi-Turn Reward Models](https://arxiv.org/abs/2512.24693)
...
### [135] [BIOME-Bench: A Benchmark for Biomolecular Interaction Inference and Multi-Omics Pathway Mechanism Elucidation from Scientific Literature](https://arxiv.org/abs/2512.24733)
...
### [136] [Compute-Accuracy Pareto Frontiers for Open-Source Reasoning Large Language Models](https://arxiv.org/abs/2512.24776)
...
### [137] [Practising responsibility: Ethics in NLP as a hands-on course](https://arxiv.org/abs/2512.24825)
*Malvina Nissim,Viviana Patti,Beatrice Savoldi*

Main category: cs.CL

TL;DR: 该文章介绍了将伦理学融入自然语言处理（NLP）教育的课程设计与实践经验，强调了批判性思维的重要性。


<details>
  <summary>Details</summary>
Motivation: 随着自然语言处理系统的广泛应用，将其伦理考量纳入教育变得至关重要，文章旨在解决课程开发中的固有问题，并提出了一种新的教学方法。

Method: 课程设计基于主动学习理念，通过互动课程、实践活动和“教学中学”的方法来实现。课程已被四年内不同机构、不同教育水平和跨学科背景的学生所采用。

Result: 该课程锻炼了学生的批判性思维，提升了学生的社交影响意识，并产生了一系列可重复使用的教学材料和教育产品。

Conclusion: 文章通过分享实践经验，希望建立一种范式，供其他教育者参考以整合社会影响考虑。
...
### [138] [Triangulation as an Acceptance Rule for Multilingual Mechanistic Interpretability](https://arxiv.org/abs/2512.24842)
*Yanan Long*

Main category: cs.CL

TL;DR: 本文提出了一种新的方法来验证多语言语言模型的行为，通过引入参考家族和三角化法则，确保解释具有因果性和泛化性。


<details>
  <summary>Details</summary>
Motivation: 当前的多语言语言模型虽然表现强大但行为上常常不可预测。本文作者认为需要一种机制解释标准，能够在不同语言、字体和文化环境中验证这些模型的行为。

Method: 提出了参考家族的概念，即保持谓词不变的variants，并引入了三角化法则，要求必要性（断开电路会降低目标行为）、充分性（修补激活会转移行为）和不变性（在参考家族中这两个效果方向稳定且幅度足够）。作者还采用了自动电路发现，并通过三角化法则接受或拒绝候选子图。此外，文中还将其与因果抽象联系起来，将其视为变换得分的近似变换，连接到了语用可解释性议程，并提出了比较实验协议。

Result: 该研究提出的方法（三角化法则）能够筛选出跨语言不具有恒定性的假定电路，提供了一种可证实的机制性标准。

Conclusion: 本文提出的三角化法则为多语言语言模型的机制解释提供了一种新的评价标准，能够改善不同环境下的模型行为稳定性。
...
### [139] [Big AI is accelerating the metacrisis: What can we do?](https://arxiv.org/abs/2512.24863)
*Steven Bird*

Main category: cs.CL

TL;DR: 这篇文章呼吁探索替代方案，强调人类福祉与地球生命的未来，批评大型AI加速生态、意义与语言危机，并指出语言工程师在这一过程中扮演了关键角色。


<details>
  <summary>Details</summary>
Motivation: 文章认为目前的语言模型发展路径未能充分考虑人类福祉和地球生态环境，反而加剧了现有的危机，因此需要重新设计NLP的未来以实现人类的繁荣。

Method: 文章通过分析当前NLP技术的发展现状，指出现有技术存在的问题，进而提出新的发展方向。

Result: 文章未具体提及研究结果，而是提出了一个关于未来NLP技术发展应以人类繁荣和地球健康为核心的新方向。

Conclusion: 文章强调了重新设计NLP的必要性，旨在为人类创造一个积极的生活未来，强调需应用集体智慧来实现这一目标。
...
### [140] [Encyclo-K: Evaluating LLMs with Dynamically Composed Knowledge Statements](https://arxiv.org/abs/2512.24867)
...
TL;DR: Encyclo-K 提出了一种基于陈述的基准测试方法，通过从权威教科书中提取独立的知识陈述，并在测试时随机组合成评估问题，解决了现有基准测试的数据污染、单知识点评估和高成本专家注释限制。实验表明，Encyclo-K 对多个大型语言模型提出了实质性挑战，展示了不同模型在知识整合方面的能力差异。


<details>
  <summary>Details</summary>
Motivation: 现有的基准测试主要关注单个问题，存在数据污染、单知识点评估和高成本专家注释等问题，无法全面评估大型语言模型的多知识点理解能力。

Method: Encyclo-K 采用基于陈述的方法重新构建基准测试，从权威教科书中提取独立的知识陈述，测试时通过随机组合生成评估问题。

Result: 实验表明，Encyclo-K 对超过50个大型语言模型提出了实质性挑战。即使是最优秀的 OpenAI-GPT-5.1 准确率也只有 62.07%，不同模型在知识整合方面表现出明显的梯度分布。

Conclusion: Encyclo-K 作为一个可扩展的动态评估框架，能够有效评估大型语言模型在多个细粒度学科知识陈述方面的综合理解能力。
...
### [141] [mHC: Manifold-Constrained Hyper-Connections](https://arxiv.org/abs/2512.24880)
...
### [142] [BEDA: Belief Estimation as Probabilistic Constraints for Performing Strategic Dialogue Acts](https://arxiv.org/abs/2512.24885)
...
### [148] [HERO-Sign: Hierarchical Tuning and Efficient Compiler-Time GPU Optimizations for SPHINCS+ Signature Generation](https://arxiv.org/abs/2512.23969)
*Yaoyun Zhou,Qian Wang*

Main category: cs.AR

TL;DR: HERO Sign 是一种 GPU 加速的 SPHINCS+ 实现，它结合了层次化调优和高效的编译时优化。该方法通过重新评估 SPHINCS+ 组件（如 FORS、MSS 和 WOTS+）中的并行化机会，引入了树融合策略，并且实现了自动生成不同的 GPU 架构适配融合方案的算法。HERO Sign 通过自适应编译策略和任务图构建的内核级重叠优化，实现了性能显著提升。


<details>
  <summary>Details</summary>
Motivation: SPHINCS+ 尽管提供了强大的后量子安全性，但其签名生成过程因大量哈希计算而变得缓慢。为了加速签名过程，本文提出了一种名为 HERO Sign 的 GPU 加速实现方法，旨在充分利用 SPHINCS+ 的 Merkle 树结构的并行性，并提供细粒度的编译器级定制。

Method: HERO Sign 采用了多层次调优和高效的编译器级别优化策略。它重新评估了 SPHINCS+ 组件中的并行化机会，并引入了树融合策略，特别是在 FORS 中。树融合策略由一个自动化的树调节搜索算法驱动，该算法能够自动生成不同的 GPU 架构适配融合方案。HERO Sign 还利用了自适应编译策略，选择 PTX 和原生代码路径，以最大化性能。此外，通过任务图构造实现内核级重叠优化，减少了多流空闲时间和内核调用开销。

Result: HERO Sign 在 RTX 4090、A100、H100 和 GTX 2080 上的 SPHINCS+ 128f、192f 和 256f 参数集的批量签名生成中，实现了 1.28-3.13、1.28-2.92 和 1.24-2.60 的吞吐量提升。与最先进的 GPU 实现相比，HERO Sign 还显著降低了内核调用延迟。

Conclusion: HERO Sign 通过多层次的并行化探索和自适应编译策略，成功加速了 SPHINCS+ 的签名生成过程，提供了显著的性能提升和内核调用效率的优化。
...
TL;DR: 文章提出了DDFT测试框架，用于评估模型在语义压缩和对抗性伪造下的事实准确性，发现模型的稳健性与参数量和架构无关，而是由训练方法和验证机制决定。


<details>
  <summary>Details</summary>
Motivation: 现有评测基准无法区分模型在信息降级或对抗性攻击时核查机制失效的情况，这促使作者提出了DDFT测试框架，以评估模型的‘认知稳健性’。

Method: 作者构建了一个包含语义生成系统和知识验证器的认知模型，并通过评估九种领先模型在八个知识领域下的五种压缩级别来测试模型的认知稳健性。

Result: 实验结果表明，模型的认知稳健性与参数量和架构无关，而是主要取决于错误检测能力。尽管大规模模型通常更大，但并不意味着更可靠。

Conclusion: DDFT不仅提供了一个理论基础，还提供了一系列评估工具，为在关键应用部署模型前提供了一个新的评估标准。
...
### [150] [A Proof-of-Concept for Explainable Disease Diagnosis Using Large Language Models and Answer Set Programming](https://arxiv.org/abs/2512.23932)
*Ioanna Gemou,Evangelos Lamprou*

Main category: cs.AI

TL;DR: 提出了一个结合大语言模型和Answer Set Programming的框架McCoy，用于医学文献的转化和病人数据的处理，以实现疾病预测。


<details>
  <summary>Details</summary>
Motivation: 传统符号AI在医疗保健中的应用受限于构建高质量知识库的高成本，而McCoy框架通过整合大语言模型和Answer Set Programming来降低这一壁垒。

Method: McCoy框架使用大语言模型将医学文献转化为ASP代码，结合患者数据，并通过ASP求解器进行处理。

Result: 实验结果表明，McCoy在小型疾病诊断任务上表现出强劲的性能。

Conclusion: McCoy提供了一种能够充分利用两种不同范式优势的稳健且可解释的预测框架。
...
### [151] [CogRec: A Cognitive Recommender Agent Fusing Large Language Models and Soar for Explainable Recommendation](https://arxiv.org/abs/2512.24113)
...
### [152] [Graph-Based Exploration for ARC-AGI-3 Interactive Reasoning Tasks](https://arxiv.org/abs/2512.24156)
*Evgenii Rudakov,Jonathan Shock,Benjamin Ultan Cowley*

Main category: cs.AI

TL;DR: 本文提出了一种无需训练的基于图的方法，用于解决ARC-AGI-3基准中的互动推理任务，该方法通过分段视觉帧、基于视觉显著性的行动优先级以及有向图跟踪已探索状态和转移，实现有限反馈环境下的任务解决。


<details>
  <summary>Details</summary>
Motivation: 目前最先进的语言模型（LLMs）在ARC-AGI-3基准上的表现不佳，无法可靠解决游戏样任务，因此本文提出了一个不依赖学习的方法，以期提供更强的基准线方法。

Method: 该方法结合了基于视觉帧的帧处理和基于图结构的状态空间系统探索。具体包括：将视觉帧分段成有意义的组件；基于视觉显著性优先行动；维护一个记录已探索状态和转换的有向图结构，并根据最短路径优先级处理未测试的状态-行动对。

Result: 在ARC-AGI-3预挑战中，这种方法解决了6个游戏中大部分的52个关卡，取得了第3名的成绩，显著超越了前沿的基于语言模型的竞争对手。

Conclusion: 本文的工作表明，即使是无学习的图结构探索也可以为互动推理提供强有力的基准线，突出了系统状态跟踪和行动优先级在稀疏反馈环境中任务动态捕捉的重要性。
...
### [153] [Constrained Language Model Policy Optimization via Risk-aware Stepwise Alignment](https://arxiv.org/abs/2512.24263)
*Lijun Zhang,Lin Li,Wei Wei,Yajie Qi,Huizhong Song,Jun Wang,Yaodong Yang,Jiye Liang*

Main category: cs.AI

TL;DR: Risk-aware Stepwise Alignment (RSA) 是一种新颖的风险感知对齐方法，通过使用嵌套风险度量在政策优化过程中明确引入风险意识，从而在对齐语言模型以展现出期望行为时降低风险。


<details>
  <summary>Details</summary>
Motivation: 当前安全对齐方法如 Safe RLHF 和 SACPO 在处理偏离参考策略的风险和抑制潜在致命行为的脆弱性方面存在不足。RSA 方法旨在通过在其政策优化过程中引入风险意识来解决这些问题。

Method: RSA 通过以嵌套风险度量的形式将安全性对齐形式化为一个分层风险感知约束政策优化问题，并通过逐步对齐过程解决它，产生来自于嵌套风险度量的分层策略更新。

Result: RSA 方法不仅在政策优化中引入了风险意识，而且在降低由不必要模型偏移引起的风险方面表现出色，同时也抑制了低概率但高影响的有害行为，实验表明该方法能在确保安全的同时提供较高的实用性。

Conclusion: RSA 为细调预训练语言模型提供了一种新的途径，通过加入风险意识来控制风险，从而在确保安全性的同时提高了模型的帮助性。
...
TL;DR: 该研究通过JEPA-WMs模型探索了一类计划算法的关键组件，最终提出了一种新的模型，该模型在导航和操作任务中优于现有基线DINO-WM和V-JEPA-2-AC。


<details>
  <summary>Details</summary>
Motivation: 本文旨在解决AI领域中的长期挑战：开发能够解决广泛物理任务并泛化到未见过的任务和环境中的智能体。

Method: 研究针对候选家族中的计划算法模型进行了全面研究，由几个关键组件组成，包括模型架构、训练目标和规划算法的效果研究。

Result: 最终提出了一个JEPA-WMs模型，该模型在导航和操作任务中均优于两个现有的基线模型DINO-WM和V-JEPA-2-AC。

Conclusion: 研究表明，通过优化模型架构、训练目标和规划算法，可以提高智能体解决新任务的能力。
...
TL;DR: 本文提出了一种交互式评估框架，用于分析基础模型在符号地图环境中的探索、记忆和推理能力。研究发现，探索主要影响经验获取，而记忆表示在巩固空间体验中起重要作用。高级推理方案进一步影响存储的空间知识的使用。


<details>
  <summary>Details</summary>
Motivation: 现有的空间能力评估主要依赖于静态地图输入或文本查询，忽略了空间理解的交互性和经验驱动性质。因此，作者提出了一种新的交互式评估框架，旨在更好地理解和提升基础模型在符号地图环境中的空间能力。

Method: 作者构建了一个交互式评估框架，用于分析基础模型在探索、记忆和推理方面的能力。该框架包含了六个类型的空间任务，并通过系统地改变探索策略、记忆表示和推理方案，跨多个基础模型进行研究。

Result: 研究结果表明，探索主要影响经验获取，但对最终推理准确性的影响有限。记忆表示在巩固空间经验中起核心作用，特别是结构化的记忆表示，如顺序和图表示法，对于结构密集型任务（如路径规划）有显著提升作用。高级推理方案进一步影响存储空间知识的使用，高级提示支持更有效的多步骤推理。此外，空间推理性能在模型版本和规模达到一定能力阈值后趋于饱和。

Conclusion: 研究揭示了探索、记忆表示和推理方案在基础模型中不同类型任务上的不同功能作用，强调了对于基于地图的空间理解和推理，需要专门针对空间表示和推理设计的机制，而不是简单的模型扩大。
...
TL;DR: 该研究评估了大型语言模型（LLM）在未广泛研究的数学竞赛问题上的性能，特别是在微积分、解析几何和离散数学领域。DeepSeek-V3 在所有类别中表现最佳，而 GPT-4o-mini 和 Gemini 则在几何问题上表现相对较弱。


<details>
  <summary>Details</summary>
Motivation: 当前针对大型语言模型（LLM）在数学推理方面的研究主要依赖于相同的基准数据集，这限制了发现的普遍适用性。本研究旨在通过使用未被广泛研究的数学竞赛问题来更深入地理解 LLM 的限制和挑战。

Method: 研究者向三大领先的大语言模型，即 GPT-4o-mini、Gemini-2.0-Flash 和 DeepSeek-V3，提供了密苏里大学尔杰格尔（Missouri Collegiate Mathematics Competition）涉及微积分、解析几何和离散数学的竞赛题目。之后，研究者对比了 LLM 的回应与已知正确答案，并分析了 LLM 的推理过程。

Result: 实验证明 DeepSeek-V3 在所有研究的题目类别中表现最佳，而其他两个模型在几何问题上的表现相对较弱。DeepSeek-V3 的错误主要是由于计算和逻辑上的问题，GPT-4o-mini 经常表现出逻辑和方法上的错误，Gemini 则倾向于缺乏推理和仓促得出结论。

Conclusion: 本研究表明，评估大型语言模型在未广泛研究的数学竞赛数据集的表现可以提供他们不同错误模式的深入见解，并突显了许多领域中结构化推理的持续挑战，特别是在几何学领域。
...
TL;DR: 该论文提出了一种两阶段方法，将空间推理分解为原子构建块及其组合，通过监督微调基础空间变换，然后使用轻量级LoRA适配器在GRPO框架中学习策略来实现多层次规划。


<details>
  <summary>Details</summary>
Motivation: 鉴于大型语言模型（LLMs）在导航和规划中的广泛应用，但仍然难以应对空间变换和多步骤规划，研究团队提出了一种针对此类问题的方法。

Method: 第一步，通过监督微调使模型具备基本的空间物理知识；第二步，在冻结已具备空间意识的模型基础上，利用GRPO框架训练轻量级LoRA适配器来学习组成构件以进行多步骤规划的策略。同时，研究团队还设计了一种基于ASCII艺术的数据集及对应的强化学习环境来支持该流程。

Result: 实验结果显示，该方法在动态和静态环境中均优于基线模型，且收敛速度更快、训练更稳定。

Conclusion: 研究证实通过细致的空间变换训练和策略学习，模型的空间理解能力得到了提升，为解决复杂环境下的空间推理问题提供了一种创新思路。
...
### [158] [Recursive Language Models](https://arxiv.org/abs/2512.24601)
*Alex L. Zhang,Tim Kraska,Omar Khattab*

Main category: cs.AI

TL;DR: 该研究提出了一种名为递归语言模型（RLMs）的新颖策略，使大型语言模型能够处理任意长的输入提示，同时在多个任务上表现出色且成本效益高。


<details>
  <summary>Details</summary>
Motivation: 当前的大语言模型在处理长输入时受到模型上下文窗口的限制，限制了其实际应用的可能性，因此需要一种新的方法来克服这个限制。

Method: RLMs通过将长输入提示视为外部环境的一部分，并允许模型递归地对提示片段进行检查、分解和再次调用来处理任意长的输入。

Result: 实验结果显示，RLMs能够成功处理比模型上下文窗口大两个数量级的输入，对于较短的输入，其表现也大幅优于基线大语言模型和常见的长上下文架构，在四个不同的长上下文任务上取得了显著的成绩。

Conclusion: 该研究提出的RLMs在长输入处理方面表现出色，同时成本效益高，展示了其在实际应用中的潜力。
...
TL;DR: 本文提出了一种针对复杂推理任务的多智能体对话模型，该模型通过生成、验证和集成三个层级处理任务，并引入自我博弈机制和检索增强模块，实验结果显示该模型在HotpotQA、2WikiMultihopQA和MeetingBank数据集上取得了显著提升。


<details>
  <summary>Details</summary>
Motivation: 针对单一大型语言模型在复杂推理任务中的局限性，提出一种多智能体对话模型，旨在提高多步骤推理准确性和一致性。

Method: 该模型采用三层级架构，包括生成、验证和集成。引入自我博弈机制和检索增强模块，设计复合奖励函数并应用改进的近端策略优化策略进行协作训练。

Result: 实验结果表明，该模型在HotpotQA、2WikiMultihopQA和MeetingBank数据集上的多跳推理准确率分别提高了16.8%、14.3%和19.2%，一致性提高了21.5%。

Conclusion: 该模型有效地增强了多智能体方法在复杂推理任务中的推理效率，并提供了一种有效且稳定的解决方案。
...
### [160] [Multi-modal cross-domain mixed fusion model with dual disentanglement for fault diagnosis under unseen working conditions](https://arxiv.org/abs/2512.24679)
*Pengcheng Xia,Yixiang Huang,Chengjin Qin,Chengliang Liu*

Main category: cs.AI

TL;DR: 本文提出了一种双解耦的多模态跨域混合融合模型，用于故障诊断。该模型引入了双解耦框架以分解不同模态和域的信息，并设计了跨域混合融合策略以增加模态和域多样性。实验结果表明，该方法在常温和时变条件下均表现出色，且综合消融研究验证了每个组件的有效性。


<details>
  <summary>Details</summary>
Motivation: 当前的智能故障诊断方法和域适应方法在面对未见过的工作条件时存在性能下降的问题，且大多数研究依赖单一模态的传感信号，忽视了多模态信息对于模型泛化能力的提升。因此，需要提出一种新的方法来解决这些问题。

Method: 该研究提出了一个双解耦的多模态跨域混合融合模型。通过双解耦框架来分解模态不变和模态特定特征以及域不变和域特定表示，以实现全面的多模态表示学习和强大的领域泛化。设计了跨域混合融合策略以随机跨域混合模态信息，增加模态和域多样性。同时，引入了三模态融合机制以自适应地整合多模态异质信息。

Result: 在异步电动机故障诊断的实验中，该方法在未见过的常温和时变条件下均取得了优异的效果。全面的消融研究表明，每个提出组件均有效，多模态融合机制也证明了其有效。

Conclusion: 综上所述，该研究提出的方法有效且全面地解决了智能故障诊断中的问题，与最先进的方法相比有显著优势。
...
### [161] [Explaining Why Things Go Where They Go: Interpretable Constructs of Human Organizational Preferences](https://arxiv.org/abs/2512.24829)
*Emmanuel Fashae,Michael Burke,Leimin Tian,Lingheng Meng,Pamela Carreno-Medrano*

Main category: cs.AI

TL;DR: 本研究通过设计和验证一个自陈调查问卷，提出了一种四维度的物体排列偏好模型，并将其应用于Monte Carlo Tree Search (MCTS) 帮助机器人进行合理安排，结果表明此方法生成的布局与参与者生成的布局高度一致。


<details>
  <summary>Details</summary>
Motivation: 现有的物体排列模型虽然有效，但缺乏对人类决策背后可解释因素的理解。因此，本研究旨在提出一个紧凑且易于理解的物体排列偏好模型，并通过实验验证其实用性和解释力。

Method: 本研究通过在线调查设计了一个自陈调查问卷，以探索人的物体排列偏好，涵盖空间实用性、习惯便利性、语义一致性与常识适宜性四个维度。验证并测试了这些偏好在厨房和客厅场景下的应用。

Result: 研究结果表明这些偏好维度具有心理上的独特性和解释力，基于这些维度的Monte Carlo Tree Search (MCTS)规划器可以生成与人类偏好接近的合理布局。

Conclusion: 本研究通过一个紧凑且可解释的物体排列偏好模型，为机器人规划提供了新的方法，证明了偏好模型的实际操作性。
...
### [162] [GenZ: Foundational models as latent variable generators within traditional statistical models](https://arxiv.org/abs/2512.24834)
*Marko Jojic,Nebojsa Jojic*

Main category: cs.AI

TL;DR: GenZ是一种混合模型，通过解释性的语义特征连接基础模型和统计建模。该模型通过迭代过程发现语义特征描述，这使得它在预测任务中的表现优于依赖基础模型领域知识的模型。实验结果表明，GenZ在房价预测和电影推荐中的表现优于现有的基础模型。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然具备广泛的知识，但在捕捉特定数据集的模式方面存在不足，这些模式对于预测任务至关重要。GenZ通过结合统计建模和基础模型，利用解释性语义特征来克服这一缺点。

Method: GenZ的方法通过迭代过程发现语义特征描述，将统计建模中的错误来对比组，提出了一种广义EM算法，该算法联合优化了语义特征描述和统计模型参数。该方法促使已冻结的基础模型根据发现的特征进行分类，将这些判断视为对潜在二元特征（通过学习的统计关系预测实值目标）的噪音观察。

Result: GenZ在房价预测和电影推荐任务中表现突出。在房价预测上，采用发现的多模态列表数据中的语义特征，模型的中位相对误差为12%，显著优于GPT-5 basline（38%）。在Netflix电影嵌入预测中，仅从语义描述中获得了0.59的余弦相似度，这与通过传统协同过滤需要大约4000个用户评分相当。

Conclusion: 研究结论表明，通过将统计建模与基础模型相结合，GenZ能够在保持模型灵活性的同时，提取出能够更准确地揭示数据集特异性模式的解释性特征，从而在预测任务中取得更优的效果。
...
### [163] [A study on constraint extraction and exception exclusion in care worker scheduling](https://arxiv.org/abs/2512.24853)
*Koki Suenaga,Tomohiro Furuta,Satoshi Ono*

Main category: cs.AI

TL;DR: 该研究提出了一种方法，利用约束模板从护理管理人员提供的信息中自动生成定制化的工作班次安排约束条件，并通过约束编程解决器生成护理工人的班表。该方法减少了约束违反次数，特别排除了例外约束。


<details>
  <summary>Details</summary>
Motivation: 文中提到，在长期护理设施中，工作班次因设施而异，这种条件变化要求专门的研究方法以满足具体需求。现有的提取约束的方法无法满足这种需求，因此本文提出了专用的约束模板来更好地适应不同设施的具体情况。

Method: 该方法利用约束模板从管理人员提供的数据中提取多种约束条件，包括连续工作日的轮班模式和员工组合等，并通过一个机制排除了不常见的约束条件。提取出的约束能够被约束编程解决器利用，帮助生成满足全部硬约束和优化软约束的工作班表。

Result: 实验结果表明，该方法不仅能够满足所有硬约束，而且通过避免提取例外约束，有效减少了软约束的违反次数。

Conclusion: 该研究提出的方法能够有效地生成满足特定长期护理设施需求的工作班次安排，具有实用价值。
...
### [164] [Let It Flow: Agentic Crafting on Rock and Roll, Building the ROME Model within an Open Agentic Learning Ecosystem](https://arxiv.org/abs/2512.24873)
...
### [165] [Semi-Automated Data Annotation in Multisensor Datasets for Autonomous Vehicle Testing](https://arxiv.org/abs/2512.24896)
...
### [166] [Iterative Deployment Improves Planning Skills in LLMs](https://arxiv.org/abs/2512.24940)
...
### [167] [AMAP Agentic Planning Technical Report](https://arxiv.org/abs/2512.24957)
...
### [168] [Context-aware LLM-based AI Agents for Human-centered Energy Management Systems in Smart Buildings](https://arxiv.org/abs/2512.25055)
