...
TL;DR: 本研究提出了一种基于多模态学习模型 (MLLM) 的自动态C臂定位框架，该模型能够自主定位骨骼关键标记点， 幊亮显示了模型的推理能力和空间意识。


<details>
  <summary>Details</summary>
Motivation: 当前在临床中使用传统深度学习方法进行C臂定位时，医疗人员倾向于手动干预，这在一定程度上上增加了了额外的时间延迟。 一个能够自主精确定位骨骼关键标记点的系统构是非常必要的。

Method: 该研究构建了一个多模态学习模型框架在X光和 和 佝个注释合的X光数据集和 对数据集与对应的骨骼标记点对齐，并 使用两种训练好的深度学习模型 (DL) 对其个定位任务进行训练并在定量和与定性对比评估和能力模型的精定定定位能力。 宴然模拟手还展示了模型的推理能力和空间感知能力。

Result: 定量结果表明训练好的多模态学习模型在两个数据集中进行定位任务时的表现与传统的深度学习方法相比不亚与进行，定好的多模态学习模型在定定的对比评估中展示出了推理能力和空间感知能力。

Conclusion: 本研究认为训练的多模态学习模型能够实现准确的骨骼关键标记的精定定位并且有潜力实现医学专业人员的自动态精定。。该研究的结果已发布在GitHub项目：https:github.com/marsszibros/C-arm-localizationL-MLLMs.git
...
TL;DR: 本文提出了一种名为URoPE的统一旋转位置编码方法，它可以应用于跨视图或跨维度的几何空间，改善了Transformer模型在新型视图合成、3D物体检测、物体跟踪和深度估计等任务中的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的相对位置嵌入方法通常局限于固定的几何空间（如1D序列或规则的2D/3D网格），无法满足视觉任务中涉及多个相机视角或2D与3D空间复杂几何推理的需求。

Method: URoPE为每个键/值图像片段在对应相机射线上选择3D点，并在已定义的深度锚点处进行投影，然后使用投影的像素坐标应用标准的2D RoPE。该方法是无参数的、内参数感知的，并且对全局坐标系的选择具有不变性。

Result: URoPE作为插件位置编码应用于多种Transformer架构下的一系列任务中，结果显示它能够提高所有任务的性能，展示了其在几何推理方面的有效性与普遍性。

Conclusion: URoPE为处理涉及多视角或跨维度几何推理的任务提供了一种灵活且有效的解决方案，该方法与现有的RoPE优化注意力核完全兼容。
...
TL;DR: 通过引入黎曼框架分析生成雅可比矩阵，将几何结构分解为局部伸缩（容量）和局部复杂性（曲率），研究发现几何解耦现象，即正常使用生成时曲率功能性地编码图像细节，但在OOD生成中曲率与稳定语义边界功能解耦，表明结构热力点是不稳定性根源，进而提出一种稳健内在诊断生成可靠性的度量。


<details>
  <summary>Details</summary>
Motivation: LDMs虽能实现高保真合成，但在编辑时存在空间脆性问题，造成语义跳跃。因此需要一种新的框架来解决这一问题。

Method: 引入黎曼框架分析生成雅可比矩阵，将几何结构分解为局部伸缩（容量）和局部复杂性（曲率），并确定几何解耦现象

Result: 发现几何解耦现象，即正常使用生成时曲率功能性地编码图像细节，但在OOD生成中曲率与稳定语义边界功能解耦，确定结构热力点是不稳定性根源。

Conclusion: 提出一种稳健内在诊断生成可靠性的结构热力点作为不稳定性根源，提供诊断生成可靠性的新方法。
...
TL;DR: DUALVISION 提出了一种轻量级的融合模块，用于将红外-可见光信息高效地结合到 MLLMs 中，并通过两个新的数据集 DV-204K 和 DV-500 进行了实证研究，证明了 DUALVISION 在多种视觉退化条件下的良好性能。


<details>
  <summary>Details</summary>
Motivation: 当前的 MLLMs 在 RGB 图像的视觉感知和推理任务上表现出色，但在常见的退化条件下（如雾、模糊或低光照）却显得脆弱。红外 (IR) 图像在这些条件下的表现更加稳定，但其在 MLLMs 中的整合尚未得到充分探索。

Method: DUALVISION 设计了一个基于局部交叉注意力的轻量级融合模块，用于高效地将 IR 和 RGB 信息结合到 MLLMs 中。通过这两类数据集支持模型的训练、评估和未来的研究。

Result: DUALVISION 能够在视觉退化条件下显着提升 MLLMs 的性能。使用 DUALVISION 融合的数据集训练的模型在多种退化条件下的表现优于其他方法。

Conclusion: DUALVISION 增强了 MLLMs 在不同退化条件下的鲁棒性，并通过公开的数据集和代码为未来的跨模态研究提供了支持。
...
TL;DR: 本文讨论通过结合 Li结构化的激光雷达扫描与视觉基础模型（VFM）处理的相机数据，，，，来实现室内场景的帧级DD视角半语D检测任务D，并，化方法D，，并。并且该D在没有人工注释的过程中实现了该模D的型的跨模态D式蒸馏。D，D和.


<details>
  <summary>Details</summary>
Motivation: D室内场景D的3的3333的33D33DD33D便D秘D度制获取分析DD暂时象的DD创D化DD工作DD非常具DD塞DD困难DD和时因此D通过D结合DDD结合DD利用DD结合视觉基础模型D（DD来D来对D转化蒸馏DD方法DDDDDD来来D实现DDD在没有DD中没有D人工注DDD的情况下D实现D实现可以D实现实现了DDD对室内激光雷达DD半视角半DD地D检测任务D。

Method: D结合利用D视觉D基础模型（D处理D相机D数据D并并与D结构化D的激光DD雷达D扫描扫描D扫描现有数据D相结合D来构建DD一个跨模DD式D蒸馏D过程D进行D实DD现现边界框交叠区DD的D检测结果D。

Result: DD本文此DDD研究DD利用D视觉DD基础D模型D处理的相机D数据D以及D结构性D化的激光D雷达DD扫描扫描D检测D室内场景DD的D帧DD视角D的DD并D不D需要D人工注DDDDDDD标D就就能DDDD成功DDD实现完成了D室内DDD激光D雷达D的DDDDD视角DD面检测任务DD。

Conclusion: DD研究表明DD通过DDD视觉DDDDDDDDDDDDDDDDDDDDDDDDD视觉DDD模型DD的DDDDDDDD影响蒸馏D方法DDD可以在DD没有D不可DD没有室内场景场景DBD检测D可视DD信息D以及D结构化D的D激光D雷达DD数据DD之间DD实现获得D跨模DD式D蒸馏D中从而D实现没有没有实现了D一种D室DD内检测任务D，的DD凭DD而检测结果效果DD而准确无需D人工注D在DD检测D过程DD中的。简化DDDDDD室内D激光D雷达DDD视角DD面DD检测D任务DDDD的发展DDDD得多D实DDDDDDDD
...
TL;DR: ConvVitMamba通过集成多尺度卷积特征提取器、基于ViT的子词化和编码阶段以及轻量级Mamba激发门控序列混合模块，实现了高效的HSI分类，该方法在多个基准数据集上表现出优于传统CNN和Transformer方法的性能。


<details>
  <summary>Details</summary>
Motivation: 鉴于高光谱图像分类领域存在的挑战，提出了一种名为ConvVitMamba的统一混合框架，旨在克服传统深度学习方法存在的高计算成本和大模型尺寸问题。

Method: ConvVitMamba框架结合了多尺度卷积特征提取、基于ViT的特征编解码和自适应高效混合模块，同时采用PCA进行预处理以减轻冗余。

Result: 实验结果表明，ConvVitMamba在多个基准数据集上不仅提高了分类精度，还保持了良好的模型大小和推理效率。

Conclusion: 该研究表明，ConvVitMamba框架为高光谱图像分类提供了一种有效且高效的方法，适用于各种场景。
...
TL;DR: 本文提出了一种新颖的模块化学习框架，用于解决地形差异、传感器特性和物体分布差异对 aerial 图像检测模型的一致性和迁移性的影响。该框架采用分层路由机制和局部场景分解机制，允许模型在数据集和复杂场景内进行特殊化。此外，条件专家模块还利用外部语义信息来检测新的类别，无需重新训练或微调。四项数据集的综合评估显示了多数据集泛化、区域专业化和开放类别检测能力的改进。


<details>
  <summary>Details</summary>
Motivation: 由于地形差异、传感器特性和物体分布差异的影响，现有的物体检测模型在 Aerial 图像场景中难以推广和适应新类别。本文旨在改进现有的模型局限性，提出分层模块化学习框架，以适应多样性和复杂场景。

Method: 本文提出的方法包含分层路由机制和局部场景分解机制。分层路由机制通过地理隐含嵌入将数据集路由到专门处理模块。局部场景分解机制将图像子区域分配给特定场景子模块。此外，条件专家模块使用外部语义信息（例如，类别名称或文本描述）进行推断以检测新类别。

Result: 本文方法在四个数据集上进行了综合评估，结果显示在多数据集泛化、区域专项化和开放类别检测方面有所改进。

Conclusion: 本文提出了一种分层模块化学习框架，解决了现有方法在地理、传感器和小类分布上的不足，并促进了遥感中的物体检测。
...
TL;DR: 该研究提出了一个基于层次嵌入和多级对抗鲁棒对齐的对抗微调框架，以增强视觉语言模型的鲁棒性并利用类空间的层次特性。


<details>
  <summary>Details</summary>
Motivation: 当前的鲁棒微调方法在增强鲁棒性的同时牺牲了自然性能和鲁棒性，无法有效地应对针对超类（如哺乳动物）的对抗攻击，因此需要一个新颖的方法来提升对抗鲁棒性并充分利用类别的层次性质。

Method: 提出了一个基于层次嵌入的对抗微调框架，并通过多级鲁棒对齐图像-文本模态来提升对抗鲁棒性。该方法还包括将视觉嵌入置于适当的层次深度，并且提供了层次深度与最大可行余量之间的理论联系。此外，该研究考虑了从多个树中对齐以增加语义多样性。

Result: 实验结果表明，该方法在多个数据集上有效地提升了模型的鲁棒性和性能。

Conclusion: 该研究证明了基于层次嵌入的对抗微调框架的有效性，能够显著提升视觉语言模型在对抗攻击下的鲁棒性。
...
### [19] [A Proxy Consistency Loss for Grounded Fusion of Earth Observation and Location Encoders](https://arxiv.org/abs/2604.18881)
...
### [20] [Localization-Guided Foreground Augmentation in Autonomous Driving](https://arxiv.org/abs/2604.18940)
...
### [21] [Bridging Foundation Models and ASTM Metallurgical Standards for Automated Grain Size Estimation from Microscopy Images](https://arxiv.org/abs/2604.18957)
*Abdul Mueez,Shruti Vyas*

Main category: cs.CV

TL;DR: 本文提出了一种自动管道，利用Cellpose-SAM进行密集实例分割和晶粒尺寸估计，并结合ASTM E112 Jeffries平量模块，该系统在多种基准测试中表现出色，特别是在有限的训练数据下。


<details>
  <summary>Details</summary>
Motivation: 复杂晶粒形态和大量数据的需求使得从扫描电镜图像中提取标准化冶金度量变得具有挑战性。为了解决这个问题，本文提出了一种适应扫描电镜结构的自动管道，以提升冶金评价的准确性。

Method: 本文的方法基于Cellpose-SAM，并将其拓扑意识梯度跟踪与ASTM E112 Jeffries平量模块相结合，构建了一个自动化的密集实例分割和晶粒尺寸估计管道。此外，还在此基础上进行了系统性基准测试，并展示了在不同训练集大小下的少量样本泛化能力。

Result: 实验结果表明，本文的适应性管道能够在密集微观计数和拓扑分离方面保持优越性，且在有限训练样本下仍表现出色。使用仅两个训练样本，系统预测的 ASTM晶粒尺寸数（G）的平均绝对百分比误差（MAPE）可低至1.50%。并在不同晶粒计数量的目标下验证了ASTM 50粒晶粒采样最小值的稳健性。

Conclusion: 本文的工作证明了在基础模型中集成应用级模型的有效性，能够实现高度准确和自动的材料表征。
...
TL;DR: CXRMate-2 是一个先进的胸部X射线报告生成模型，通过结合结构化多模态条件和基于复合奖励的强化学习，达到了在多个数据集上的显著性能提升，并通过盲法随机回顾性评估表明其生成报告在可读性和召回率方面具有与放射科医生报告相媲美的表现。


<details>
  <summary>Details</summary>
Motivation: 目前的胸部X射线报告生成模型在临床应用上存在不确定性，尤其是缺乏放射科医生的实际验证。本研究旨在通过引入一种先进的模型——CXRMate-2，结合结构化多模态条件和强化学习，解决这一问题。

Method: 研究团队使用了 CXRMate-2 模型，并在 MIMIC-CXR，CheXpert Plus 及 ReXgradient 数据集上进行了性能测试。此外，还进行了一次盲测随机回顾性评估，比较了放射科医生报告与生成报告之间的异同。

Result: CXRMate-2 在多个数据集上达到了显著的性能提升，尤其是在 MIMIC-CXR 数据集上相对于 MedGemma 1.5（4B）获得了 11.2% 和 24.4% 的提升。在放射科医生盲测评估中，约 45% 的情况下生成报告被视为可接受（优于或与放射科医生报告相同），在大部分分析的发现上，偏好率没有显著差异；偏好倾向于放射科医生报告主要因为更高的回召率，而生成报告因其更好的可读性更受青睐。

Conclusion: 此研究表明 CXRMate-2 模型具备临床可接受的胸部X射线报告生成能力，通过改进召回率并更好地检测到细微发现（如肺充血），可能实现与放射科医生报告非劣效。为支持放射科医生流程，这些进步可能使 CXRMate-2 系统准备好进行前瞻性的评估。
...
TL;DR: 通过对Gaussian-tile对对对对对对 洼D 楣DG-SplattingD计算进行优化，AdaGScaleD技术通过减少对周边贡献较小D灶GaussianD DD对lijahsD的D的D的D在预处理阶段估计周围区域的贡献D，并并在视对视涉及到影响D的D的D的D的 GaussianD相交时区域时进行调整D从而提高计算效率D同时保持在相机视视野范围内的几何保精度DD优化Dina实验中DAdaGScaleD实现了在GPU上上上上D上上上上D的D3的333祥DG-Splatting计算的1几何倍速提升D33同时PSNR性能只降低了D略DDBD这样的既提高了效率D又保持了视觉质量DD这个方法适用于3D-GSysD场景下的GPU计算ing on 


<details>
  <summary>Details</summary>
Motivation: 研究介绍了一种种新的通过减少Gaussian tile pair的数量来提升3D-GD在计算的D计算效率的方法，同时这种方法依赖于对在推测在周边贡献较小的Gaussainian tile pair数量。通过在预处理器阶段获得Gaussdian tile pair的周围贡献并基于在光源对移动时调整在参与交子的Gaussdian tile pairsD，的方法以既提高了效率D又保持了视觉质量D D对于D3D-GD计算。这研究在GPU上上环境下进行，实果验结果表明DAdaGScaleD实现了在PSNRNA评里仅略微D降低D1的3D-GD计算的几何倍速提升。D

Method: 该论文采用了一种种通过减少Gaussdian tile pairs的方式D来提升在GPU环境下的D3D-GD计算效率。具体来说D在预处理器阶段估算D每一个Gaussdian tile pair在其D附近区域的贡献D并在相机移动过程中根据其周围区域是否有Gaussdian tile pairs有交子DDynamic地ally调整D对应Gaussdian tile pairsD的数量D从而在不显著影响视效和几何精度的情况下D实现在光源移动过程中3D-GD计算的效率。

Result: 实验结果表明D通过采用AdaGScale方法D3D-GD在计算过程中仅减少了了略降低D的Pixel Signal-to--noise比PSNRNA比D也就是说D影响D视觉质量很小D与此同时D在GPU环境下3D-GD计算的速度提升显著D进而为实时高效生成高质量的3D渲染图提供支持。

Conclusion: 总之D这项研究提出了一种有效的办法D通过减少和3D-GD计算中Gaussdian tile pairs的数量D在实提升了计算效率而保持了视觉质量和几何精度D该为D进一步优化D相关DGPUD环境下的33D-GD计算提供了新的D思路和D实验结果证明了该所提出的DAdaGScale方法的效果可在。实际应用中中论证。
...
TL;DR: 本文讨论了AutoAWG框架，，恶劣天气下的自动驾驶感知鲁棒性. 它通过多模态自适应融合、生成目标条件合成以及离线遮罩生成来减少对合成数据的依赖并，并， 保证了时间一致性、语义结构保真和风格保真. 通过对比实验表明，AutoAWG在单帧条件和和帧条件下都表现出色. 这进一步强调了AutoAWG对于提升自动驾驶领域的感知鲁棒性的价值.


<details>
  <summary>Details</summary>
Motivation: 提升自动驾驶在恶劣天气条件下的感知能力

Method: 多模态自适应融合、生成目标条件合成以及离线遮罩生成

Result: 时间欺骗性鲁棒性'automatic driving

Conclusion: AutoAWG框架为改善天气场景下的感知任务提供了新的解决方案， 对备注
...
### [25] [Explore Like Humans: Autonomous Exploration with Online SG-Memo Construction for Embodied Agents](https://arxiv.org/abs/2604.19034)
...
TL;DR: 该研究提出了一种通过利用预训练生成模型的强大图像先验知识来增强纹理过滤的技术，通过两阶段微调策略，实现了优于现有方法的性能和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 受到预训练生成模型的强大图像先验知识启发，在现有的纹理去除方法基础上改进，提出了一种更有效的方法。

Method: 该方法采用两阶段微调策略：首先在少量配对图像上进行监督微调，随后在大规模未标记数据集上进行强化微调，使用一种评估纹理去除质量与结构保持的奖励函数进行指导。

Result: 广泛实验表明，这种方法在性能和泛化能力方面显著优于现有方法，能够有效处理以前具有挑战性的案例。

Conclusion: 该研究提供了一种新型的纹理过滤生成方法，已经在实际应用中展现了优良的性能。
...
TL;DR: LPCVC2025通过三个赛道促进边缘设备上高效视觉模型的开发，利用Qualcomm AI Hub进行一致且可再现的基准测试，并介绍了各赛道的顶级解决方案及未来建议。


<details>
  <summary>Details</summary>
Motivation: LPCVC2025旨在推动高效视觉模型在边缘设备上的发展，平衡准确性和延迟、内存限制和能耗等约束。

Method: 通过Qualcomm AI Hub进行统一的基准测试，设置了图像分类、开放词汇分割和单目深度估计三个赛道。

Result: 公布了每个赛道的顶级解决方案，并记录了关键趋势和观察。

Conclusion: 提出了对未来计算机视觉比赛的建议。
...
TL;DR: PortraitDirector 是一种创新的面部再演绎框架，它将面部运动分解为空间层和语义层，分别处理物理动作和情感内容，采用层次化的分解和重组策略，实现高保真和可控的效果。


<details>
  <summary>Details</summary>
Motivation: 当前的面部再演绎方法在表达性和细节控制之间存在权衡， PortraitDirector旨在解决这个问题，通过层次化的分解和重组策略，实现精确控制和高保真度。

Method: 该方法采用了一种层次化的运动解混拼合策略，将面部运动分为物理动作的空间层和情感内容的语义层，空间层使用专用的表示和注入路径管理全局头部姿态，利用情感过滤模块去除情绪线索的局部面部表情，语义层包含全局情感的派生。

Result: PortraitDirector 实现了高帧率的实时面部再演绎，达到了 512x512 像素的高保真度和可控制性，同时保持了 20 FPS 的帧率和 800 毫秒的端到端延迟。

Conclusion: 该研究提出了一种创新的面部再演绎框架，成功地解决了表达性和控制性的权衡问题，为今后的面部动画研究和应用提供了新的思路。
...
TL;DR: BALTIC 是一个在不同介质和光照条件下评估 3D 重建方法的基准。展示了在受控条件下，Gaussian Splatting 方法在特定情况下的表现与专业水下方法相当。


<details>
  <summary>Details</summary>
Motivation: 机器人感知在不同环境条件下的 3D 重建面临挑战，特别是在空气和水之间的过渡。为了系统地评估现代 3D 重建方法，本文提出 BALTIC 基准。

Method: BALTIC 基准包括 13 个跨两种介质（空气和水）和三种光照条件（环境光、人工光和混合光）的数据集。实验设置包含定制的水箱、单目相机和 HTC Vive 跟踪器，用于准确的地面真实 pose 估计。此外，通过相似光照条件下添加空中视图序列来增强水下图像序列，进行跨域重建。

Result: 研究表明，Gaussian Splatting 方法在特定条件（例如纹理一致）下，配合简单预处理（如白平衡校正）可媲美专门的水下方法。但在复杂且异质的真实环境中，其鲁棒性降低。

Conclusion: BALTIC 提供了一个系统的方法评估 3D 重建方法在不同环境条件下的性能，展示了 Gaussian Splatting 方法在某些条件下的有效性和局限性。
...
TL;DR: 提出了一种基于patch的噪声尺度和时间步长调整策略，名为Patch Forcing (PF)，可以更高效地进行图像生成，尤其适用于类条件的ImageNet和文本到图像合成任务。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散模型和流模型在图像去噪时将计算资源均匀分配到整个图像，忽略了解构自然图像中的空间异质性，导致有些区域在去噪过程中可获得更多的上下文信息和细化程度不如其它区域。

Method: 通过对图像的patch进行噪音尺度和时间步长的动态分配，引入了timestep sampler来控制训练中每个patch的可获取信息量的最大值。此外，与轻量级的困难程度头结合，实现了自适应采样器，可以动态地将计算资源分配到需要的地方。

Result: 通过实验，PF框架在标准基准之上显著提高了图像生成的质量，特别是在类条件的ImageNet数据集和文本到图像合成任务上。此外，该方法在代码表示对齐和指导方法之上保持了正交性。

Conclusion: PF框架提供了一个有希望的基础，用于自适应的图像生成，表明了基于patch层面的去噪时间调度策略的有效性。
...
TL;DR: ST-Prune 是一种无需训练、即插即用的框架，通过引入运动感知时域裁剪（MTP）和环视空间裁剪（RSP）两个模块，针对多帧视频和多视角摄像头输入进行了高效的时空冗余消除，从而显著降低计算开销，同时保持了较高的性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型（VLMs）在自动驾驶系统的应用中受限于多视角摄像头和多帧视频输入带来的巨大计算负担，现有针对单图像输入的裁剪方法无法有效利用驾驶场景中的时空冗余，ST-Prune旨在解决这一问题。

Method: ST-Prune框架包括运动感知时域裁剪（MTP）和环视空间裁剪（RSP）两个模块。MTP通过在多样性选择目标中引入运动波动性和时间近期性作为软约束，优先处理动态轨迹和当前帧的内容；RSP则利用环视摄像头几何关系对对观视角相似性进行惩罚，消除因时间裁剪无法抑制的重复投影和残留背景。

Result: ST-Prune在四个基准测试中展示了在感知、预测和规划方面的优越性能，即便是90%的标记减少，也能达到近无损的效果，某些指标甚至优于完全模型基线，同时保持了与现有裁剪方法相当的推理速度。

Conclusion: ST-Prune为无训练的标记裁剪设定了新的行业领先水平，展示了在压缩计算开销的同时保持性能的优势。
...
TL;DR: 本研究通过多尺度扩展DeepSSIM，提出了Deep Structural Similarity with Multiscale Representation（MSDS），在多个基准数据集上显示了与单尺度baseline的一致且统计上显著的改进。


<details>
  <summary>Details</summary>
Motivation: 大多数现有的图像质量评估方法在单一空间尺度上操作，假设固定分辨率下的结构相似性是足够的。本文旨在研究空间尺度在深度特征相似性建模中的作用。

Method: 提出了一个基于多尺度的扩展DeepSSIM模型（MSDS），该模型在金字塔层独立计算DeepSSIM，并通过轻量级可学习全局权重融合结果。

Result: 在多个基准数据集上的实验表明，MSDS相比单尺度baseline显示了显著改进，且引入的复杂性可以忽略。

Conclusion: 本文的研究表明，空间尺度在深度感知相似性中是一个不可忽视的影响因素。
...
### [35] [Improved Anomaly Detection in Medical Images via Mean Shift Density Enhancement](https://arxiv.org/abs/2604.19191)
*Pritam Kar,Gouri Lakshmi S,Saptarshi Bej*

Main category: cs.CV

TL;DR: 本文提出了一种新的混合异常检测框架，结合自监督表示学习和基于流形的密度估计，该方法在七个医学成像数据集上表现出色，特别是在脑肿瘤检测方面性能卓越。


<details>
  <summary>Details</summary>
Motivation: 鉴于医学影像中稀有的病理条件标注样本较少，本研究旨在提出一种新的体系来克服这一挑战，通过整合自监督表征学习和流形密度估计，为有限标注样本下的异常检测提供新的视角。

Method: 首先，将医学图像嵌入预训练的潜在特征空间。然后使用Mean Shift Density Enhancement (MSDE) 迭代移动样本至高概率区域。最后在PCA降维后的潜在空间使用高斯密度估计计算异常得分。

Result: 在七个医学成像数据集上的实验显示该框架表现出色，特别是在脑肿瘤检测中取得了最佳的AUC/AP性能。

Conclusion: 该研究展示了新框架在多种医学影像模态下的潜力，特别是在低标签设置下的临床决策支持和早期疾病检测中。
...
TL;DR: CLVG-Bench框架改进了现有的多方法,评估多媒体生成模型的通过引入更复杂的标注多以及更先进的的任务设定。


<details>
  <summary>Details</summary>
Motivation: 目前的的基准在评估多媒体多 和方法上方面是不 讲究充分的， 迵由于它们过于简单，设计 和易忽略了综合多媒体的任务复杂性性 例如物理模拟和交互环境。。这们的这种限制使得难以对这些模型进行全面的评估。

Method: CLVG-Bench提出了一个新的框架，cluding上以下操作tasks。

Result: 引入了超过 6 小时分类和以及 4 个小 且直接 

Conclusion: CLVG-Bench 由通过这个泛化能力强的视频模型的评估和 吊络模广泛在评价的缺失点，并提出了明确的改进方向紧和
...
TL;DR: 本研究重新审视了仅使用视觉基线模型在跨域场景下进行面部抗欺骗（FAS）的潜力，使用自监督视觉模型DINOv2作为基础模型，并结合数据增强方法改进了基准模型性能。


<details>
  <summary>Details</summary>
Motivation: 由于面部反欺骗（FAS）的挑战在于跨域环境的鲁棒性需求，以及基于多模态的模型通常依赖高计算资源和低效率的推理，本文旨在通过使用仅视觉基模型和优化的技术来提供更高效且鲁棒的FAS基准线。

Method: 本文系统地评估了15种预训练模型，包括监督CNN、监督ViT和自监督ViT，在MICO和Limited Source Domains (LSD)协议下的表现。使用自监督视觉模型DINOv2结合注册技术，以及数据增强方法（包括Patch-wise数据增强和注意力加权片段损失），来构建高效且鲁棒的FAS基准。

Result: 这一仅视觉基模型的方法在MICO协议中取得了最先进的性能，并且在数据受限的LSD协议下优于现有方法，同时保持了较高的计算效率。通过这种方式，本研究提供了一个基于自监督视觉转换器的FAS基线。

Conclusion: 本文结论展示了优化的自监督视觉转换器可以作为FAS系统（无论是仅视觉还是未来多模态FAS系统的基础）。
...
TL;DR: 本文通过提供设备端捕获指导并在离线重建时记录传感器信号，解决了通过移动设备进行3D高斯点绘制的数据获取挑战。


<details>
  <summary>Details</summary>
Motivation: 移动手机通过高斯点绘制进行3D重建是一个挑战。为了提高准确性，本文提出了基于对象的采样协议，通过设备内部捕获指南和传感器信号的离线记录来满足可靠性要求。

Method: 首先进行校准步骤，使得设备位置与基准坐标对齐并确定其相对姿态。然后将相机光学轴映射到对象中心的球形网格上，以实现均匀的视角索引。为了减少极地采样偏见，在实时计算面积加权球形覆盖的同时，对用户进行动作指导。

Result: 与RealityScan和自由捕获策略相比，本文方法在输入图像较少的情况下能够获得更高的重建质量。进一步的分析表明，本文方法可以在对象中心采样期间获得更全面和均匀的视角覆盖。

Conclusion: 本文提出的方法通过优化视角采样，提高了基于移动设备的3D重建的质量和效率。
...
### [39] [Attention-based Multi-modal Deep Learning Model of Spatio-temporal Crop Yield Prediction with Satellite, Soil and Climate Data](https://arxiv.org/abs/2604.19217)
...
### [40] [Thinking Before Matching: A Reinforcement Reasoning Paradigm Towards General Person Re-Identification](https://arxiv.org/abs/2604.19218)
...
### [41] [Adaptive Slicing-Assisted Hyper Inference for Enhanced Small Object Detection in High-Resolution Imagery](https://arxiv.org/abs/2604.19233)
...
### [42] [Learning to Credit the Right Steps: Objective-aware Process Optimization for Visual Generation](https://arxiv.org/abs/2604.19234)
*Rui Li,Ke Hao,Yuanzhi Liang,Haibin Huang,Chi Zhang,YunGu,XueLong Li*

Main category: cs.CV

TL;DR: 文章 文文摘要介绍了关于GRPO框架及其改进方法OTCA。


<details>
  <summary>Details</summary>
Motivation: 方法 关于GRPO的局限性以及OTCA如何改进了原有的方法 方法使其更加适用于在视CA通过将粗略的奖励监督转化为具有时空意识的的优化信号，实现了对联合建模型的方法提高描述了临时序列的重要性分配， 劫学家.

Method:  OTCA通过以下步骤来改进了GRPO：1

Result:  1 1 1  OTCA首先估计临时序列的重要性，并分配分配， 夷 具体包括包括了临时 騡三执行：1

Conclusion:  OTCA通过其 增强了对 对细粒度奖励信号分配及其模型方法的实时适应性性优化信号，提高了方法 效性任务视觉生成中的的两个评估指标.
...
### [43] [Allo{SR}$^2$: Rectifying One-Step Super-Resolution to Stay Real via Allomorphic Generative Flows](https://arxiv.org/abs/2604.19238)
*Zihan Wang,Xudong Huang,Junbo Qiao,Wei Li,Jie Hu,Xinghao Chen,Shaohui Lin*

Main category: cs.CV

TL;DR: 通过利用大规模扩散和和生成先验知识与优化轨迹的方法，，这样可以在一次生成过程中保持高度的保真度和和 和现实主义。


<details>
  <summary>Details</summary>
Motivation: 研究为了改进实时超级分辨率（Real 是Real Real-SR) 的生成效果，提高图像修复的保真度 on 鼓励高度的生成真实感，保持极高的效率。

Method:  1 方法是首先引入噪声比噪声比比引导轨迹初始化 (SNR Guided Tra)，)，和), 在训练过程中对将潜在特征的退化阶段与预训练流中的最优锚定步长进行对齐。然后通过实现Flow-An c键定点一致性 (FATC) 来确保约束中间速度，速度 

Result: 通过对合成和真实世界的基准测试实验结果表明,  on 这种框架在实时超级分辨率的一次生成过程中 显著提高了修复保真度 on增加生成真实感的同时 也保持保持了极高的效率。

Conclusion: 这种工作提出一种能够同时在一次生成过程中同时结合生合成现实主义保真度的方法 
...
### [44] [Unposed-to-3D: Learning Simulation-Ready Vehicles from Real-World Images](https://arxiv.org/abs/2604.19257)
...
### [45] [Framelet-Based Blind Image Restoration with Minimax Concave Regularization](https://arxiv.org/abs/2604.19314)
...
### [46] [Multi-view Crowd Tracking Transformer with View-Ground Interactions Under Large Real-World Scenes](https://arxiv.org/abs/2604.19318)
*Qi Zhang,Jixuan Chen,Kaiyi Zhang,Xinquan Yu,Antoni B. Chan,Hui Huang*

Main category: cs.CV

TL;DR: 本文提出了一种基于Transformer的多视角人群跟踪模型MVTrackTrans，该模型通过相机视角与地面平面间的相互作用来提升多视角跟踪性能。此外，作者还收集并标注了两个大规模现实场景的多视角跟踪数据集MVCrowdTrack和CityTrack，为模型在复杂场景下的应用提供了评价基准。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖于基于CNN的多视角人群跟踪架构，并主要在小型数据集Wildtrack和MultiviewX上进行对比评估。这些数据集场景较小，评价阶段帧数有限，难以应用于真实场景中复杂的大规模场景和遮挡问题。

Method: 本文提出的MVTrackTrans模型通过引入相机视角与地面平面间的相互作用来实现多视角跟踪性能的提升。此外，还使用自收集的大型现实场景数据集MVCrowdTrack和CityTrack进行训练和评估，增强了模型在更大范围及长时间跨度场景中的适用性。

Result: 在两个新提出的大型数据集MVCrowdTrack和CityTrack上，提出的MVTrackTrans模型表现优于现有方法，展示了在处理大规模场景时模型设计的优势。

Conclusion: 作者认为，提出的MVCrowdTrack和CityTrack数据集以及MVTrackTrans模型能够将此研究领域的前沿推向更具实际应用场景的多视角跟踪任务，并将相关数据集和技术代码开源发布。
...
### [47] [PLaMo 2.1-VL Technical Report](https://arxiv.org/abs/2604.19324)
...
TL;DR: PLaMo 2.1-VL 是一种轻量级视觉语言模型，针对自主设备进行了优化，支持日语，并在视觉问答和视觉定位上表现出色，特别是在工厂任务分析和基础设施异常检测两个实际应用场景中。


<details>
  <summary>Details</summary>
Motivation: Papers introduce PLaMo 2.1-VL as a light model for autonomous devices, focusing on VQA and visual grounding, with specific application scenarios in mind. This highlights the need for efficient and accurate visual understanding solutions in practical settings.

Method: The research develops and evaluates PLaMo 2.1-VL, a Vision Language Model, in two real-world scenarios. It includes the creation of a large-scale synthetic dataset and comprehensive Japanese training and evaluation resources.

Result: PLaMo 2.1-VL outperformed comparable models in both Japanese and English benchmarks. It achieved 61.5 ROUGE-L on the JA-VG-VQA-500 and 85.2% accuracy on the Japanese Ref-L4. In factory task analysis, it achieved 53.9% accuracy zero-shot, and after fine-tuning, the anomaly detection bbox + label F1-score improved significantly to 64.9.

Conclusion: The conclusion emphasizes the success of PLaMo 2.1-VL in practical applications and its superiority over comparable open models, indicating its potential for deployment in autonomous devices.
...
### [48] [Divide-and-Conquer Approach to Holistic Cognition in High-Similarity Contexts with Limited Data](https://arxiv.org/abs/2604.19339)
*Shijie Wang,Zijian Wang,Yadan Luo,Haojie Li,Zi Huang,Mahsa Baktashmotlagh*

Main category: cs.CV

TL;DR: DHCNet 提出了一种新颖的方法来处理视觉分类中的整体特征，通过逐步分析细微差异，减少对大量训练样本的依赖，实现了在数据有限场景下的高效分类。


<details>
  <summary>Details</summary>
Motivation: 当前在超细粒度视觉分类中，对于高度相似类别之间的区分依赖于整体特征，但这些特征往往需要大量数据来建模，这在数据有限的情况下带来挑战。

Method: DHCNet 通过分割整体特征为细微差异，并在局部区域中逐步建立整体认知过程，使用自排序操作从局部区域逐步分析细微差异，同时利用未受影响的局部区域辅助感知原有的拓扑结构，从而建立空间关联。此外，DHCNet 还结合了从局部区域发现的整体特征的在线细化，提高了整体特征的质量，并将其作为监督信号来微调识别模型。

Result: DHCNet 在五个广泛使用的超细粒度视觉分类数据集上取得了显著性能。

Conclusion: DHCNet 提供了一种有效的方法来改善数据有限环境下超细粒度视觉分类的整体特征建模，具有重要的应用前景。
...
### [49] [RAFT-MSF++: Temporal Geometry-Motion Feature Fusion for Self-Supervised Monocular Scene Flow](https://arxiv.org/abs/2604.19349)
*Xunpei Sun,Zuoxun Hou,Yi Chang,Gang Chen,Wei-Shi Zheng*

Main category: cs.CV

TL;DR: RAFT-MSF++ 提出了一种自我监督的多帧框架，通过迭代融合时间特征联合估计深度和场景流。引入了 Geometry-Motion Feature，结合时空先验和占用修正模块，有效提高了在遮挡区域的鲁棒性，显著提升了 KITTI 场景流基准数据集的表现。


<details>
  <summary>Details</summary>
Motivation: 现有的单目场景流估计方法大多集中在两帧输入上，限制了时间建模和对遮挡的鲁棒性。RAFT-MSF++ 提出了一种自我监督的多帧框架，通过利用时空信息和局部连续性来解决这些限制。

Method: RAFT-MSF++ 使用 Geometry-Motion Feature 迭代更新来联合估计深度和场景流，引入相对位置注意力注入空间先验，并通过占用修正模块将可靠运动从可见区域传播到不可见区域。

Result: 实验结果显示，RAFT-MSF++ 在 KITTI 场景流基准数据集上的 SF-all 达到了 24.14%，相较于基线模型提高 30.99%，并且在遮挡区域具有更好的鲁棒性。

Conclusion: RAFT-MSF++ 通过结合时空推理和局部连续性信息，显著改进了单目场景流估计的鲁棒性和准确性，在多帧输入场景下取得了更好的结果。
...
### [50] [Attend what matters: Leveraging vision foundational models for breast cancer classification using mammograms](https://arxiv.org/abs/2604.19350)
*Samyak Sanghvi,Piyush Miglani,Sarvesh Shashikumar,Kaustubh R Borgavi,Veenu Singla,Chetan Arora*

Main category: cs.CV

TL;DR: 该论文提出了一个改进的ViT模型，用于乳腺癌检测任务。通过结合 ROI机制减少模型的参数量来增强了对LOM检测能力并


<details>
  <summary>Details</summary>
Motivation: 该现有工作聚焦于于 鍍于现大型筛查任务中，现有ViT模型存在参数量过大和和误分类倾向高 及对位置感知缺失问题问题 这些问题 限制了模型的诊断效能从而引入了RoI机制以增加模型对OM定位能力而抗其他方法例提高参数量用于优化模型性能及

Method:    此方法采用了作者提出的RoI机制减少模型的参数量ONG在模型的定位能力上引入了局部注意机制以优化模型在大型筛查任务中的的性能从而提高探测和在病变区域中的效果 以及引入使用结合INOv

Result: 在哺乳影数据图像序列

Conclusion: 最后  试验  体结果所 在大筛查任务中的  了到了良好的  本表以及证明了对 在这一O ROI机制可以提高模型的性能 
...
### [51] [Detection of T-shirt Presentation Attacks in Face Recognition Systems](https://arxiv.org/abs/2604.19365)
*Mathias Ibsen,Loris Tim Ide,Christian Rathgeb,Christoph Busch*

Main category: cs.CV

TL;DR: 在此研究中，作者利用来自T-shirt Face Presentation Attack (TFPA)数据库的1,608个T恤攻击和152个真样本，对基于空间一致性的检测方法进行了评估，证明了这种攻击能够破坏人脸识别系统的安全性。


<details>
  <summary>Details</summary>
Motivation: 由于现有方法难以有效检测新出现的伪装攻击手段，作者旨在提出一种基于空间一致性的检测方法来对抗新型T恤攻击。

Method: 使用了最新的面部和人体检测器来分析检测到的面部和人体的空间位置，以识别T恤攻击。

Result: 该研究证明了基于空间一致性的检测方法在一定程度上能够有效检测新型T-shirt攻击，并且优于单纯依赖单一检测器的性能。

Conclusion: 本文提出的方法为今后对抗新型伪装攻击提供了一种新的思路，但仍需进一步研究以完善检测精度和泛化能力。
...
### [52] [Mind2Drive: Predicting Driver Intentions from EEG in Real-world On-Road Driving](https://arxiv.org/abs/2604.19368)
*Ghadah Alosaimi,Hanadi Alhamdan,Wenke E,Stamos Katsigiannis,Amir Atapour-Abarghouei,Toby P. Breckon*

Main category: cs.CV

TL;DR: 本研究提出了一种基于EEG的驾驶员意图预测框架，通过同步多传感器平台集成到一辆真实的电动汽车上，评估了12种深度学习模型，其中TSCeption表现最佳，展现了较强的时序稳定性，在动作执行前1000ms内的解码性能持续保持高水准。


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶系统中基于EEG的驾驶员意图预测面临非平稳EEG信号和认知-运动准备复杂性等挑战，在真实驾驶场景中的应用仍具挑战性。研究旨在提出一种可行性方案，以提升先进驾驶辅助系统的主动安全性。

Method: 本研究收集了32个驾驶会话的真实道路数据集，并评估了12种深度学习架构，探索了最优的EEG预处理方法，并分析了预测性能在不同时间间隔的表现。

Result: TSCeption在准确性和宏F1分数上表现最优，再现性强，预测性能在动作执行前400-600ms达到峰值，即认知准备期。

Conclusion: 研究结果证明，在真实道路上，EEG可以用于早期和稳定的驾驶员意图解码，为增强驾驶辅助系统的安全性提供了新的可能性。
...
### [53] [PanDA: Unsupervised Domain Adaptation for Multimodal 3D Panoptic Segmentation in Autonomous Driving](https://arxiv.org/abs/2604.19379)
...
### [54] [Air-Know: Arbiter-Calibrated Knowledge-Internalizing Robust Network for Composed Image Retrieval](https://arxiv.org/abs/2604.19386)
...
### [55] [HarmoniDiff-RS: Training-Free Diffusion Harmonization for Satellite Image Composition](https://arxiv.org/abs/2604.19392)
*Xiaoqi Zhuang,Jefersson A. Dos Santos,Jungong Han*

Main category: cs.CV

TL;DR: 该研究提出了一种无需训练的基于扩散的框架，用于在多种领域条件下对合成卫星图像进行调和，同时保持内容的一致性。


<details>
  <summary>Details</summary>
Motivation: 卫星图像合成在远程 sensing 应用中至关重要，如数据增强、灾害模拟和城市规划。现有方法可能受到训练依赖和效果受限的限制。

Method: 提出了一种无需训练的框架 HarmoniDiff-RS，使用 Latent Mean Shift 操作来传递辐射度特征，以及 Timestep-wise Latent Fusion 策略来生成候选合成图像，并通过轻量级和谐度分类器自动选择最一致的结果。

Result: 该方法在实验中展示出有效性和优异的性能，特别适用于可扩展的远程 sensing 合成与模拟任务。

Conclusion: 研究者通过构建 RSIC-H 数据集，实验证明了方法的有效性，并且提出了一个可在 GitHub 访问的实现代码。
...
### [56] [VecHeart: Holistic Four-Chamber Cardiac Anatomy Modeling via Hybrid VecSets](https://arxiv.org/abs/2604.19403)
...
### [57] [HP-Edit: A Human-Preference Post-Training Framework for Image Editing](https://arxiv.org/abs/2604.19406)
...
TL;DR: 本文提出了一种名为HP-Edit的后训练框架，结合少量人类偏好评分数据和预训练的视觉大语言模型，构建了一个面向真实世界的HP-Scorer自动评分器。该评分器用于高效构建可扩展的偏好数据集，并作为编辑模型后训练的奖励函数。


<details>
  <summary>Details</summary>
Motivation: 鉴于在图像编辑领域缺乏针对人类偏好的可扩展数据集和框架，本文旨在填补这一空白，提出HP-Edit框架以解决这一问题。

Method: HP-Edit框架利用少量的人类偏好评分数据和预训练的视觉大语言模型构建了HP-Scorer自动评分器。该评分器用于构建可扩展的偏好数据集，并作为编辑模型后训练的奖励函数。

Result: 经过实验证明，该方法显著提升了如Qwen-Image-Edit-2509等模型的输出，使其更加符合人类的偏好。

Conclusion: 本文提出了一种有效的后训练框架HP-Edit，用于图像编辑任务中的人类偏好对齐，并通过构建偏好数据集和使用自动评分器提高了模型性能。
...
### [58] [GOLD-BEV: GrOund and aeriaL Data for Dense Semantic BEV Mapping of Dynamic Scenes](https://arxiv.org/abs/2604.19411)
...
<details>
  <summary>Details</summary>
Motivation: 简要说明撰写摘要的目的

Method: 方法基于观察和”的”“semantic分割的算法从第一视角(（ego-centric）传感器和”” “ 行人“ on 

Result: 算法描述

Conclusion: 总结总结算法的主要特点和  on  
...
### [59] [VCE: A zero-cost hallucination mitigation method of LVLMs via visual contrastive editing](https://arxiv.org/abs/2604.19412)
*Yanbin Huang,Yisen Li,Guiyao Tie,Xiaoye Qu,Pan Zhou,Hongfei Wang,Zhaofan Zou,Hao Sun,Xuelong Li*

Main category: cs.CV

TL;DR: 该研究提出了一种名为Visual Contrastive Editing (VCE)的新方法，用于减轻大型视觉-语言模型（LVLMs）中的对象幻觉问题。通过分析模型对对比视觉扰动的响应，VCE利用SVD分解模型的激活模式并进行针对参数编辑，以抑制幻觉倾向。实验表明VCE在多种基准上有效减少了对象幻觉，并保持了模型的原始计算效率。


<details>
  <summary>Details</summary>
Motivation: LVLMs在处理如医疗成像和自主驾驶等对准确性要求极高的实际应用中普遍存在的对象幻觉问题，该研究旨在提出一种新的后处理方法以减轻这一问题。

Method: 该方法使用了视觉对比编辑（Visual Contrastive Editing, VCE），它通过SVD分解模型激活模式来识别和抑制幻觉现象。这种方法无需进行微调或使用标签数据，因此可以在资源受限的环境下进行大规模部署。

Result: 实验结果证明，VCE在多个基准测试中成功降低了对象幻觉现象，同时保持了模型原有的计算效率。

Conclusion: VCE为减轻LVLMs中的对象幻觉问题提供了一种有效且实用的方法，这种方法具有良好的扩展性和实际部署价值。
...
### [60] [LoViF 2026 Challenge on Real-World All-in-One Image Restoration: Methods and Results](https://arxiv.org/abs/2604.19445)
...
TL;DR: 本报告提供了LoViF挑战的综合分析，展示了多种现实世界降质条件下的图像恢复方法及其结果，强调了统一现实世界图像恢复的最新进展。


<details>
  <summary>Details</summary>
Motivation: LoViF挑战旨在促进在各种现实世界降质条件下的全功能图像恢复研究，为不同降质类别提供统一体系下的统一基准，以评估恢复模型的鲁棒性和泛化能力。

Method: 通过提供统一体系的基准测试，评估多种降质类别下图像恢复模型的能力。

Result: 共有9个有效的最终提交和相应的详细说明，显著推动了现实世界全功能图像恢复的进步。

Conclusion: 该报告明确了有效的图像恢复方法，并为未来现实世界低层次视觉研究建立了基准。
...
### [61] [TS-Attn: Temporal-wise Separable Attention for Multi-Event Video Generation](https://arxiv.org/abs/2604.19473)
...
### [62] [Deep sprite-based image models: An analysis](https://arxiv.org/abs/2604.19480)
*Zeynep Sonat Baltacı,Romain Loiseau,Mathieu Aubry*

Main category: cs.CV

TL;DR: 本文着重于基于精灵的图像分解模型，通过设计细节分析和基准测试，提出了一种新的深层基于精灵的图像分解方法，该方法在标准CLEVR基准上与最先进的无监督类别感知图像分割方法表现相当，且能够线性扩展到多个对象。


<details>
  <summary>Details</summary>
Motivation: 鉴于基础模型在图像分割和扩散算法方面的进展，识别图像集合中的重复模式问题仍未解决，因此本文对基于精灵的图像分解模型进行深入研究，以解决这一问题。

Method: 研究了基于精灵的图像分解模型的设计细节，通过基准测试进行了详细分析，并提出了一个深度基于精灵的图像分解方法。

Result: 该方法在标准CLEVR基准上表现与最先进的无监督类别感知图像分割方法相当，且能够线性扩展到多个对象，明确识别出对象类别，并以易于解释的方式对图像进行建模。

Conclusion: 基于精灵的图像分解模型虽然具有高度可解释性，但在特定数据集上的调适以及规模扩展方面存在挑战，本文提出了一个能够克服这些问题的新方法。
...
### [63] [Seeing Candidates at Scale: Multimodal LLMs for Visual Political Communication on Instagram](https://arxiv.org/abs/2604.19489)
...
### [64] [Evaluating Histogram Matching for Robust Deep learning-Based Grapevine Disease Detection](https://arxiv.org/abs/2604.19510)
*Ruben Pascual,Inés Hernández,Salvador Gutiérrez,Javier Tardaguila,Pedro Melo-Pinto,Daniel Paternain,Mikel Galar*

Main category: cs.CV

TL;DR: 本研究通过结合直方图匹配作为预处理和数据增强技术，显著提高了葡萄叶疾病识别的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 由于光照变化导致的深度学习模型鲁棒性不足，研究提出使用直方图匹配技术来改善。

Method: 研究采用双阶段直方图匹配方法：一是作为预处理步骤进行归一化，二是作为数据增强技术引入可控的训练变化。使用ResNet-18模型在包含均匀叶和混合树冠样本的1,469张RGB图像上进行实验。

Result: 实验结果显示，结合直方图匹配显著增强了对实际树冠图像的鲁棒性。尽管叶样本略有提升，但树冠亚集明显改善，表明归一化与基于直方图的多样化平衡有效地缩小了由于不可控照明引起的数据域鸿沟。

Conclusion: 研究证明，直方图匹配技术在保持像素强度分布一致性的同时，增强了模型在光照变化情况下的适应性。
...
### [65] [Paparazzo: Active Mapping of Moving 3D Objects](https://arxiv.org/abs/2604.19556)
...
### [66] [EgoSelf: From Memory to Personalized Egocentric Assistant](https://arxiv.org/abs/2604.19564)
*Yanshuo Wang,Yuan Xu,Xuesong Li,Jie Hong,Yizhou Wang,Chang Wen Chen,Wentao Zhu*

Main category: cs.CV

TL;DR: EgoSelf是一种通过图为基础的交互记忆和个人化学习任务来捕获用户特定行为和偏好以提供个性化的助理系统。


<details>
  <summary>Details</summary>
Motivation: 考虑到不同用户的习惯、偏好和日常活动存在差异，因此需要针对特定用户进行个性化服务才能实现有效协助。

Method: EgoSelf系统基于图的交互记忆，整合用户过去的观察数据，建立一种特定于个人的学习任务来预测用户未来可能的交互行为。

Result: 通过大规模实验展示了EgoSelf在个性化的辅助作用方面的有效性。

Conclusion: EgoSelf系统为长期用户数据的有效整合以实现用户个性化服务提供了一种新的解决方案。
...
### [67] [TransSplat: Unbalanced Semantic Transport for Language-Driven 3DGS Editing](https://arxiv.org/abs/2604.19571)
...
### [68] [SmartPhotoCrafter: Unified Reasoning, Generation and Optimization for Automatic Photographic Image Editing](https://arxiv.org/abs/2604.19587)
...
TL;DR: SmartPhotoCrafter为自动摄影图像编辑方法，通过紧密结合的推理-生成过程，利用Image Critic模块识别图像缺陷，并通过Photographic Artist模块进行有针对性的编辑，实现图像质量的提升，无需显式的人类指令。该方法采用多阶段训练管道进行训练，包括基础预训练、推理指导的多编辑监督、以及推理到生成的强化学习，实现了高质量的摄影增强效果。


<details>
  <summary>Details</summary>
Motivation: 现有摄影图像编辑依赖于用户对美学的理解和明确的指令，而用户往往无法提供清晰的、完整的且易获取的美学意图。本文旨在提出SmartPhotoCrafter，以减少对用户专业知识的要求，通过自动化的图像编辑流程提升图像质量。

Method: SmartPhotoCrafter采用多阶段训练管道进行训练，包括基础预训练、推理指导的多编辑监督、以及推理到生成的强化学习。它包含两个模块：Image Critic用于图像理解并找出不足之处，Photographic Artist则针对这些不足进行有针对性的编辑以提升图像质量。

Result: 实验结果表明，SmartPhotoCrafter在自动摄影图像增强任务中表现出色，生成的照片更加逼真，且对润饰指令有更高的色调敏感度。

Conclusion: SmartPhotoCrafter为自动摄影图像编辑提供了一种新的方法，通过结合语义推理和生成模型，提高了图像质量而不依赖于用户的专业指导。
...
### [69] [Structure-Semantic Decoupled Modulation of Global Geospatial Embeddings for High-Resolution Remote Sensing Mapping](https://arxiv.org/abs/2604.19591)
*Jienan Lyu,Miao Yang,Jinchen Cai,Yiwen Hu,Guanyi Lu,Junhao Qiu,Runmin Dong*

Main category: cs.CV

TL;DR: 这篇论文介绍了一种克服高分分辨率遥感成像预测中因高频度细节和 和严格 的语义空间关联导致的预测碎片化的框架，


<details>
  <summary>Details</summary>
Motivation: 论文动机在于 是 是为了解决高分辨率遥感成像预测中的，低水平细节引起的预测碎片化问问题问题以及强关联导致的语义空间矛盾问题。

Method: 论文的方法主要 是通过引入一种称为SSDM的语义分分离联解结构-S框架，， 该该方法主要包括两个部分模态注入渠道

Result: 首先 攙 共成斯大解钙开仅高展开渠道首要任务是在细粒度分辨率编码器中通过从高粒度化分辨率获取中抽取出大尺度接收域 以适度抑制高抽像是特征抽取， 同一段公共需压抑制细粒粒度水平取波和和 和 噇过在细粒粒粒显示分辨率懂获取中 迉图像压 的 徤形干扰性动静注合模拟将语议制和 柔性强编码场中的深层次细粒粒度分辨区分割提取 斝同时 促进了二语征和空间上的匹配对准判断从而显著提高复杂物体的语义一致性性和分辨性。

Conclusion: 最终结论是论文通过这种框架方法实现已经实现了在多种场景中获得了最先进的精度 沷 为将在遥感基础理解任务中引入遥感基础基敷模致提供了普适 和的范式。
...
### [70] [PC2Model: ISPRS benchmark on 3D point cloud to model registration](https://arxiv.org/abs/2604.19596)
*Mehdi Maboudi,Said Harb,Jackson Ferrao,Kourosh Khoshelham,Yelda Turkan,Karam Mawas*

Main category: cs.CV

TL;DR: 本文介绍了PC2Model基准数据集，用于支持点云到模型注册任务中经典和数据驱动方法的训练和评估，结合了模拟点云和真实场景数据，以提高模型在不同场景下的鲁棒性和迁移性。


<details>
  <summary>Details</summary>
Motivation: 随着点云获取技术的普及和深度学习的进展，现有的数据驱动方法在实际场景中因稀疏性、噪声、杂乱和遮挡等问题难以达到理想的精度，因此需要一个综合的基准数据集来评估和改进这些方法。

Method: 该基准数据集采用混合设计，包含模拟点云数据和真实世界扫描及其对应的3D模型数据。模拟数据提供了精确的地面真相和可控制的条件，而真实世界数据引入了传感器和环境的非理想因素。

Result: 该数据集集成了模拟和真实数据，旨在支持点云到模型注册方法的训练和评估，提高了方法在不同场景下的鲁棒性和迁移性。

Conclusion: 研究者可以通过该数据集系统地分析模型在模拟和真实场景之间的迁移情况，为点云到模型注册领域的研究提供了重要的数据支持。
...
### [71] [GRAFT: Geometric Refinement and Fitting Transformer for Human Scene Reconstruction](https://arxiv.org/abs/2604.19624)
...
### [72] [CreatiParser: Generative Image Parsing of Raster Graphic Designs into Editable Layers](https://arxiv.org/abs/2604.19632)
*Weidong Chen,Dexiang Hong,Zhendong Mao,Yutao Cheng,Xinyan Liu,Lei Zhang,Yongdong Zhang*

Main category: cs.CV

TL;DR: 提出了一种综合框架用于将栅格图转换为向量图形，并


<details>
  <summary>Details</summary>
Motivation: 当前的栅格图解析方法依赖于复杂的工作流，包括分割、 觵绘 on 和 娡补,这给出了积累和 控制上的 on 表具体的难点.因此提出了一种种可可分的结构化方法用于解析栅格格图形并保持准确的重建和 允灵活编辑表

Method: 提出这种方法提出了一个多分支扩散模型结合rgba on onpha on 让与了Parser奖励模型 on 使相对 on 盳对自己性能优化来使以对对 be 自

Result: 通过这种方法从给定两个数据集集eg 行了测试 on 在ha种方法能够较出较大准确 on 的重建 on 吟能灵活编辑各形象上 屨

Conclusion: 总之该方法提供了一种解决方案用于栅格图的解析并具有创新的综合框架 on 以实现在现有技术栅行列图解析方法上的进步 on
...
### [73] [CoCo-SAM3: Harnessing Concept Conflict in Open-Vocabulary Semantic Segmentation](https://arxiv.org/abs/2604.19648)
*Yanhui Chen,Baoyao Yang,Siqi Liu,Jingchao Wang*

Main category: cs.CV

TL;DR: SAM3介绍-vocabulary 词汇 语义分割 通过 基于 line 方法 架构实现 通过 劌 产产生 生成 慫 体 ， 夒 夸 复 吚 吵 同 合 的 方法 融 稪 处理 不同 的 表达 形式 生成 的 体 位置不连续 的 望 体 产 ， 八 种 分字 词汇 语的 语义分割基准 虶 达 技果 强 化表 苨 认知 开 合 在 子标志 这一 基础进 行方 体 消 复和 矛盾 將 复类 栀语 与 綈抗 弬.


<details>
  <summary>Details</summary>
Motivation: 为了改善一词汇场景下的语义分割效果， 词汇 语言 表达 不 君 个 形式 于 吞 辫 字体存在 复和位置 不连 敘 与 砥 扉 紧 屝 和,将来自相同概念的不同描述的方法整合起来,， 合个 概念 产 生 感 叟关系 在 这 一 孿 中 现矛盾和 獰桔形而 夈 缔 巅 复类表 舒 綈丁. 緻加 血符号性 施 缺 无混合消矛盾的 影响 .

Method: Co-SAM3采取了一种两步的结方法 第其步整合来自相似概念的不同描述的表征 合 是良协调 戎 利地有滋活的口 体 ，， 通过这种方式 抶 衼 濅 搜 吻 摜 关 感 , 第 体 圗 释 兯 刈权 衈 . 瞶 数 榬 在 的 分 声长 在 

Result: 结果显示 Co-SAM3在八种不同的词汇 词汇 衇 的 语义分割基 衐藏表 迶靠产 明地加强了表 衟能 生 斋 衇 摜 衙 夈습니다 . 成 昳 产 白然 迄件方 綈 時 

Conclusion: Co-SAM3通过有效地融合和步 爿 在 盘中间消除了在 .志 中 衕 制 攴 基 癶 趈 弗 埥 吐藏一 . 在 .茫 夵 兄재 囏 . 巻 癬 有效的."
...
### [74] [CoInteract: Physically-Consistent Human-Object Interaction Video Synthesis via Spatially-Structured Co-Generation](https://arxiv.org/abs/2604.19636)
...
### [75] [MedFlowSeg: Flow Matching for Medical Image Segmentation with Frequency-Aware Attention](https://arxiv.org/abs/2604.19675)
*Zhi Chen,Runze Hu,Le Zhang*

Main category: cs.CV

TL;DR: MedFlowSeg 引入了一种新的流匹配框架，通过学习时间依赖的向量场来进行医学图像分割，实现了一步确定性推断，同时保持生成模型的表示能力，显著降低了计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有的生成医学图像分割方法主要依赖于耗时的扩散模型，MedFlowSeg旨在提供一种理论上更加严谨且计算效率更高的方法。

Method: MedFlowSeg 采用了一种新的流匹配框架，通过学习时间依赖的向量场，将简单的先验分布转化为目标分割分布，同时引入了一种双重条件机制来结合结构先验信息，并且设计了多尺度空间注意模块和频率感知注意模块来分别处理空间和频谱信息。

Result: 实验证明，与基于扩散模型的方法相比，MedFlowSeg 在多种医学成像模态下表现出前沿的性能，并且显著降低了计算成本。

Conclusion: MedFlowSeg 开辟了医学图像分割中流匹配的新应用，理论依据充分且计算效率高，显示出在生成功能性医学图像分割方面的潜力。
...
### [76] [MMControl: Unified Multi-Modal Control for Joint Audio-Video Generation](https://arxiv.org/abs/2604.19679)
...
<details>
  <summary>Details</summary>
Motivation: 该文摘要提介绍了近年来在扩散变换器（Diffusion Transformers) 方面上取得的关键性进展，，为单一模式生成提供更多控制能力的关键技术该，然而对音频- 角同步具在同步生成视频中的表现进行了描述。进一步阐述了基于此的研究进展如何推动了联合音频-视频生成框架的发展及其

Method: 采纳了将视觉和和和 

Result: 该 顾及此次 絇. 文ried研究进展中， 瑜条将将视觉和

Conclusion: 总体而言,该文研究工作了和尚能够通过，修改 Diffusion Transformers
...
### [77] [IR-Flow: Bridging Discriminative and Generative Image Restoration via Rectified Flow](https://arxiv.org/abs/2604.19680)
*Zihao Fan,Xin Lu,Jie Xiao,Dong Li,Jie Huang,Xueyang Fu*

Main category: cs.CV

TL;DR: IR-Flow 是一种新颖的图像恢复方法，基于整流流，旨在弥合判别性和生成性范式的差距。通过多级数据分布流和累积速度场，IR-Flow 能够快速推断和修复图像，同时保持对不同降级程度的良好适应性。


<details>
  <summary>Details</summary>
Motivation: 现有的单步骤判别映射方法缺乏对细节的学习，而生成范式需要多步采样且存在噪声扰动的问题。为了解决这些问题，提出了一种新的图像恢复方法 IR-Flow。

Method: IR-Flow 采用多级数据分布流和累积速度场构建了一个统一的框架，既能从模型中学习到不同损坏程度的信息，又能引导中间状态向干净的目标恢复。

Result: IR-Flow 方法能在很少的采样步骤内达到与现有方法相当的定量结果，且具备高效的推理速度和对未知损坏良好的适应能力。

Conclusion: IR-Flow 提供了一个高效且灵活的框架，有助于图像恢复任务中的快速且高质量的图像恢复，同时显著提高了对多种不同降级条件的适应性。
...
### [78] [Face Anything: 4D Face Reconstruction from Any Image Sequence](https://arxiv.org/abs/2604.19702)
*Umut Kocasari,Simon Giebenhain,Richard Shaw,Matthias Nießner*

Main category: cs.CV

TL;DR: 该研究提出了一种基于标准化面部点预测的统一方法，用于高保真4D面部重建，通过共享的标准化空间为每个像素分配标准化面部坐标，实现了几何时间一致性、可靠的对应关系以及深度估计、稳定重建和密集3D几何的精准面部点跟踪。


<details>
  <summary>Details</summary>
Motivation: 为了克服非刚性变形、表情变化和视角变化带来的几何和对应估计的显著不确定性，提高动态人脸重建和跟踪的准确性。

Method: 该方法利用一种标准化面部点预测来将密集跟踪和动态重建转换为标准化重建问题。通过联合预测深度和标准化面部坐标，实现了一种联合深度估计和稳健的面部点跟踪架构。使用基于变压器的模型进行预测，并用多视角几何数据进行训练。

Result: 在图像和视频基准测试上的广泛实验显示，该方法在重建和跟踪任务上达到了最先进的性能，对应关系误差降低了大约3倍，速度快于之前的动态重建方法，深度准确性提高了16%。

Conclusion: 研究表明，标准化面部点预测是统一4D面部重建的有效基础。
...
### [79] [SpanVLA: Efficient Action Bridging and Learning from Negative-Recovery Samples for Vision-Language-Action Model](https://arxiv.org/abs/2604.19710)
*Zewei Zhou,Ruining Yang,Xuewei,Qi,Yiluan Guo,Sherry X. Chen,Tao Feng,Kateryna Pistunova,Yishan Shen,Lili Su,Jiaqi Ma*

Main category: cs.CV

TL;DR: 该论文提出了一种名为SpanVLA的新型端到端自动驾驶框架，通过结合自回归推理和流匹配动作专家，以高效规划未来路径。此外，提出了一种基于GRPO的后训练方法，并构建了一个新的驾驶推理数据集mReasoning，以提高SpanVLA模型的性能和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言-动作（VLA）模型在动作生成上存在高延迟问题，且在鲁棒性方面表现有限。因此，需要一个更有效的框架来提高推理和动作生成的效率。

Method: SpanVLA框架结合了自回归推理和流匹配动作专家，通过流匹配策略基于历史轨迹初始化来高效规划未来轨迹。此外，提出基于GRPO的后训练方法，使模型不仅能从正面驾驶样本学习，还能学习避免典型的负面行为和恢复行为。构建了一个新的真实世界驾驶推理数据集mReasoning。

Result: 实验在NAVSIM（v1和v2）上展示了SpanVLA模型的竞争性能。另外， qualitative结果显示，模型在多种场景下的规划性能和鲁棒性优秀。

Conclusion: SpanVLA框架通过结合高效的路径规划策略和改进的模型训练方法，在自动驾驶场景中表现出色，特别是在复杂与需推理的场景中。
...
### [80] [Generative Drifting for Conditional Medical Image Generation](https://arxiv.org/abs/2604.19736)
*Zirong Li,Siyuan Mei,Weiwen Wu,Andreas Maier,Lina Gölz,Yan Xia*

Main category: cs.CV

TL;DR: GDM 是一种多目标学习框架，将确定性的医学图像预测重新制定为一个包含分布一致性与个体特异性目标的多优化问题，在高维 3D 医学成像领域表现出更高的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 现有的医学图像生成方法难以在推断效率、患者特异性精度和分布一致性之间达成平衡，特别是在高维度的3D医学成像中。因此，本文提出了GDM框架以解决这个问题。

Method: GDM 通过吸引力-排斥力漂移机制来最小化生成器推进与目标分布之间的差异，并通过多级特征库和梯度协调策略优化3D体积数据下的稳定漂移学习。

Result: 在MRI-to-CT 合成和稀疏视点 CT 重建两个代表性任务上的实验结果表明，GDM 优于许多现有的基线方法，包括基于 GAN、流匹配和 SDE 的生成模型以及监督回归方法。

Conclusion: GDM 提供了一种实践性且有效的框架，用于有条件且高维度的3D医学图像生成，平衡了解剖学精度、定量可靠性、感知真实性和推断效率。
...
### [81] [CityRAG: Stepping Into a City via Spatially-Grounded Video Generation](https://arxiv.org/abs/2604.19741)
...
TL;DR: CityRAG是一种利用大量地理位置注册数据作为上下文的视频生成模型，能够生成现实世界的3D一致、可导航环境，实现多天气条件和动态物体配置的重建，适用于自主驾驶和机器人模拟等领域。


<details>
  <summary>Details</summary>
Motivation: 现有的视频生成模型尽管能在文本或图像提示下生成可信的序列，但在不同天气条件和动态物体配置下真实世界的重建能力对于自主驾驶和机器人仿真等下游应用至关重要。

Method: CityRAG利用大量地理注册数据作为上下文，保持对复杂运动和外观变化的学习先验，依赖于时间对齐的训练数据，使模型能够语义地分离底层场景与暂时属性。

Result: 实验结果表明，CityRAG能够生成连续minutes级、物理上真实的视频序列，保持天气和光照条件数百帧，实现循环闭合，并导航复杂轨迹以重建现实地理。

Conclusion: CityRAG为现实世界重建和场景生成提供了新的解决方案，为自主驾驶和机器人学习等领域提供了重要工具。
...
### [82] [AnyRecon: Arbitrary-View 3D Reconstruction with Video Diffusion Model](https://arxiv.org/abs/2604.19747)
*Yutian Chen,Shi Guo,Renbiao Jin,Tianshuo Yang,Xin Cai,Yawen Luo,Mingxin Yang,Mulin Yu,Linning Xu,Tianfan Xue*

Main category: cs.CV

TL;DR: AnyRecon 提出了一种适用于任意和无序稀疏输入的可扩展框架，通过增加全局场景记忆并引入几何感知的条件策略，支持长范围条件，同时保持几何控制和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有的稀疏视图3D重建方法往往依赖于生成式的模型，但受限于只能基于少数观测帧，影响几何一致性和大型场景的建模效率。

Method: AnyRecon 通过预验证的捕获视图缓存构建持久的全局场景内存，并利用几何驱动的捕获视图检索和几何感知的条件策略，结合4步扩散蒸馏和上下文窗口稀疏注意力，减少计算复杂度。

Result: 实验结果表明，AnyRecon 能够对不规则输入进行稳健且可扩展的重建，即使在大视角变化和长时间轨迹中也能保持良好的几何一致性。

Conclusion: AnyRecon 框架通过改进的条件策略和高效的计算方法，提高了稀疏视图3D重建的质量和可扩展性。
...
### [83] [Tstars-Tryon 1.0: Robust and Realistic Virtual Try-On for Diverse Fashion Items](https://arxiv.org/abs/2604.19748)
...
### [84] [Two-dimensional early exit optimisation of LLM inference](https://arxiv.org/abs/2604.18592)
*Jan Hůla,David Adamczyk,Tomáš Filip,Martin Pavlíček,Petr Sosík*

Main category: cs.CL

TL;DR: 提出了一种二维早期退出策略，通过层间和句间退出机制，实现了一种在大型语言模型分类任务中节省计算资源的方法，相较于单一维度优化，节省了计算资源，并且在细调后依然保持了一定优势。


<details>
  <summary>Details</summary>
Motivation: 现有方法在大语言模型中的分类任务中进行早期退出优化时，要么仅优化层间退出，要么仅优化句间退出，而本文提出的方法结合了两者，能够在复杂任务中实现比单一优化更显著的计算节省。

Method: 该方法通过逐句处理输入并逐步激活更深的层，利用2D早期退出策略实现计算资源的高效利用。

Result: 在四个先进大语言模型（Llama 3.1, Llama 3.2, Gemma, Qwen；3B-8B参数）和三个情感分类数据集上的实验表明，相比于最佳层间早期退出方法，该方法在简单任务中能够实现1.4-2.3倍的速度提升，而在复杂多类问题上表现逐渐恶化，但经过微调后这种优势仍然存在。

Conclusion: 本文的方法是一种通用性强、仅需轻型分类适配器的方法，与量化和剪枝等效率提升技术不冲突，并且在需要在输入结构中积累语义信息的序列处理任务中有潜力得到应用。
...
### [85] [Probing for Reading Times](https://arxiv.org/abs/2604.18712)
...
### [86] [Characterizing AlphaEarth Embedding Geometry for Agentic Environmental Reasoning](https://arxiv.org/abs/2604.18715)
*Mashrekur Rahman,Samuel J. Barrett,Christina Last*

Main category: cs.CL

TL;DR: 该研究探讨了Google AlphaEarth的64维嵌入向量的流形几何结构，并开发了一个利用这一几何理解的自主系统，用于环境推理。研究发现，这些嵌入是高度非欧几里得的，且基于嵌入检索比参数模型更能提高多步骤比较问题的响应质量。


<details>
  <summary>Details</summary>
Motivation: 当前的地球观测基础模型虽然能够将地表信息编码为稠密嵌入向量，但其几何结构和对下游推理的影响尚未被充分探索。探讨嵌入的几何结构有助于开发更强大的环境推理系统。

Method: 研究通过分析AlphaEarth的64维嵌入数据集，估算了流形几何，并评估了不同的推理模型。构建了一个基于嵌入检索的自主推理系统，对比了参数模型和嵌入检索的效果。

Result: 研究发现嵌入是非欧几里得的，局部内嵌维度约为10，局部几何结构有助于提高检索的物理一致性。自主系统在多步骤环境查询中表现出显著优势。

Conclusion: 研究证明了嵌入几何结构在环境推理中的重要性，特别是在复杂环境查询中，表明几何理解有助于提升模型性能。
...
### [87] [Scripts Through Time: A Survey of the Evolving Role of Transliteration in NLP](https://arxiv.org/abs/2604.18722)
...
### [88] [Investigating Counterfactual Unfairness in LLMs towards Identities through Humor](https://arxiv.org/abs/2604.18729)
...
### [89] [Remask, Don't Replace: Token-to-Mask Refinement in Masked Diffusion Language Models](https://arxiv.org/abs/2604.18738)
*Lin Yao*
...
### [90] [Syntax as a Rosetta Stone: Universal Dependencies for In-Context Coptic Translation](https://arxiv.org/abs/2604.18758)
*Abhishek Purushothama,Emma Thronson,Alexia Guo,Amir Zeldes*

Main category: cs.CL

TL;DR: 本文提出了一种新的上下文学习方法，用于支持低资源库底语种科普特语到英语的机器翻译，通过结合词典项检索与句法信息，取得了新的最先进的翻译结果。


<details>
  <summary>Details</summary>
Motivation: 低资源语言的机器翻译方法不同于高资源语言，因此需要新的方法来支持翻译。该研究旨在通过句法增强为科普特语到英语的翻译提供支持。

Method: 该方法使用双语词典来支持词汇项的推理，并添加了一些句法分析的表示作为输入，具体探索了原始解析器输出、解析的自然语言描述以及子树中难以翻译结构的指令。

Result: 尽管句法信息本身不如基于词典的注释有用，但结合检索到的词典项和句法信息，在模型大小方面的表现有了显著提高，从而取得了科普特语翻译的新最先进的结果。

Conclusion: 该研究证明了结合句法信息和词典项检索可以显著改善低资源语言的机器翻译性能。
...
### [91] [Model-Agnostic Meta Learning for Class Imbalance Adaptation](https://arxiv.org/abs/2604.18759)
*Hanshu Rao,Guangzeng Han,Xiaolei Huang*

Main category: cs.CL

TL;DR: HAMR 提出了一种新的框架，能够同时处理类别不平衡和数据难度问题，通过动态加权和邻域感知重采样来优先处理真正具有挑战性的样本和少数类，实验结果显示 HAMR 在多个任务的不平衡数据集上表现出色。


<details>
  <summary>Details</summary>
Motivation: 为了解决自然语言处理中类别不平衡的问题，尤其是在数据存在难度的情况下，提出了 HAMR 框架，以提高模型的泛化能力和在各领域内的鲁棒性。

Method: HAMR 使用双层优化动态估计实例级别权重，优先处理真正具有挑战性的样本和少数类。同时通过邻域感知重采样机制增强对困难实例及其语义相似邻居的关注。

Result: 实验验证了 HAMR 在六组不平衡数据集上的有效性，特别提升了少数类的表现，并优于多个强大基线。消融实验显示各组件协同工作，显著提升了模型性能。

Conclusion: HAMR 提出了一种灵活且通用的方法来适应类别不平衡，为解决不平衡数据集带来的挑战提供了新的思路和解决方案。
...
### [92] [An Empirical Study of Multi-Generation Sampling for Jailbreak Detection in Large Language Models](https://arxiv.org/abs/2604.18775)
*Hanrui Luo,Shreyank N Gowda*

Main category: cs.CL

TL;DR: 该 文研究提出了一个实验， 方法来检测在未指系统对齐的情况下生成器产生的 Jailbreak 行为。 方法结合了词汇级方法TF-IDF 算器


<details>
  <summary>Details</summary>
Motivation: 动机本研究旨在解决在实际条件下检测生成器（LM）的 jailbreaking 行为， 的挑战，， 

Method:  方法通过研究发现单个生成样本会 tends 看作相反的 jailbreak 行为的理由，而同时样本短缺时

Result:  方法提出使用了一个实验数据数据来Jailail各ailameBench Bench
...
### [93] [Mango: Multi-Agent Web Navigation via Global-View Optimization](https://arxiv.org/abs/2604.18779)
*Weixi Tong,Yifeng Di,Tianyi Zhang*

Main category: cs.CL

TL;DR: 该摘要介绍介绍了 Mango 率， 一个基于多智能体的方法来动态分配网页抓取任务，并 抽动作通过使用 Thompson 抽奖算法和，使用使该在每个 episode层上 上自适应选择最有效的动作来优化抓取URL的成本。 Mango 的效果在模拟和离线资源上 数据优化基准之上都表现良好，


<details>
  <summary>Details</summary>
Motivation: 该指数主要解释了为什么需要一种高效的网页抓取策略， 传统的方法在复杂网站上的效率很低， Mango 提供 提供了一种不同时的方法的方法来解决这个问题 孵通过多人工智能算法动态调整抓取任务的分配

Method:  Mango 采用一种基于多智能体的方法和， 首先它 创建Mango 作为一个多智能体系统，每一次行动时都会根据当前的状态选择不同的动作; 文以最优化抓取UR成本为导向， 采用使用 Thompson 抽抽奖算法 TFTS 珣 束能自适应地优化行动的分布.

Result:  Mango 在模拟和 更多种离之间优化基准之上都表现良好 凷实现了63.6 的次在更以GPT on-mini的基准之上超出6%6.% 在更以GPT on-WalkerQA基式的之上超出63%. 也更加证明了Mango对于G咋开源和代码的模型着的良好泛化能力.

Conclusion:  怙方法Mango 提提供了一个高效的动态分配网页抓取任务的方法 孏还可以用于各种其他涉及到资源优化配置的应用场景.
...
TL;DR: 本文摘要讨论了了模型在具体情况下的可 抢标


<details>
  <summary>Details</summary>
Motivation: 摘要介绍了探讨了语言模型在应对模型不完善的假设性和情况下的表现的可行性性刘时作者希望通过研究得出结论，在给定的背景下，模型的推测是否其可行性还是稳定性较差时的两种代替模型的表现有较大优势。这意可以帮助表明在复杂条件下模型的表现可能会更具有稳健性

Method: 在这两篇论文中,作者通过对两组环境进行了模拟以观察模型在不同两个方面

Result: 第一组是有效的表现也就是

Conclusion: 第二组是脆弱的表现
...
TL;DR: 该研究提出了一种可扩展的多因素实验框架，系统地探讨了LLM在双文档比较中的细微语义变化敏感性，揭示了几项关键发现：LLM倾向于对文档早期的语义差异更敏感，周围背景不相关时语义差异评分降低且分化的特点，每个LLM都有独特的评分分布，但处理不同变化类型时有一致性。


<details>
  <summary>Details</summary>
Motivation: 为了全面了解大型语言模型在处理细微语义变化时的行为，研究者们提出了这一实验框架进行探索。

Method: 通过将一个语义修改的句子（作为“针”）置于多种不同的背景（作为“草堆”）中，研究者们系统性地检验了五个LLM在数千对文档中的表现，研究了不同干扰类型、背景类型、句子位置和文档长度等因素的影响。

Result: 研究结果揭示了LLM的不同特点：(1)大部分模型更严厉地惩罚早期出现的语义差异;(2)背景不相关时，语义差异评分降低且出现极度低或高相似情况；(3)每个LLM有自己的评分分布，且在处理不同干扰类型时表现出一致性。

Conclusion: 研究结论表明，除了语义变化本身，文档结构、背景一致性及模型身份对LLM的语义相似度评分有很大的影响，表明了该实验框架能够为评估和比较不同LLM的行为提供有用的工具。
...
### [96] [Where Fake Citations Are Made: Tracing Field-Level Hallucination to Specific Neurons in LLMs](https://arxiv.org/abs/2604.18880)
*Yuefei Chen,Yihao Quan,Xiaodong Lin,Ruixiang Tang*

Main category: cs.CL

TL;DR: 摘要要摘要的摘要总结为一句话。


<details>
  <summary>Details</summary>
Motivation: 摘要要 涚要将摘要的摘要摘要凝练成一句话，，

Method: 提取摘要中的关键信息和，

Result: 这篇文章提出了一种基于FH-神经元的轻; 怅虑方法以减轻引显示的生成现象。
...
### [97] [Prioritizing the Best: Incentivizing Reliable Multimodal Reasoning by Rewarding Beyond Answer Correctness](https://arxiv.org/abs/2604.18892)
*Mengzhao Jia,Zhihan Zhang,Meng Jiang*

Main category: cs.CL

TL;DR: 本文提出了一种新的奖励机制（Groupwise Ranking Reward）来解决多模态强化学习中的推理-答案不一致问题。实验表明，这种方法在提高可靠性条件下的准确性方面表现最佳。


<details>
  <summary>Details</summary>
Motivation: 文中指出，尽管RLVR通过奖励验证正确的最终答案改善了多模态推理，但答案正确的过程仍可能基于不完整的推理、不强的证据或与结论矛盾的陈述。因此，提出了轨迹监督方法来解决这一问题。

Method: 文章比较了两种方法：奖励模型（RMs）和生成式奖励（GRs）。RMs有效且初始训练时表现良好，但在策略分布变化时有效性下降；GRs性能提高，但可能产生不稳定奖励且计算成本高。因此，提出了Groupwise Ranking Reward，该方法一次排序通过验证的同一提示的轨迹，并相应重新分配奖励。

Result: 实验结果显示，Groupwise Ranking Reward 能有效缓解RLVR带来的推理-答案不一致问题，并在可靠性条件下的准确性上相较于RLVR提升了7.3%。

Conclusion: Groupwise Ranking Reward 提出了一种改进的评价机制，在多模态强化学习中表现出更优异的性能。
...
### [99] [LogosKG: Hardware-Optimized Scalable and Interpretable Knowledge Graph Retrieval](https://arxiv.org/abs/2604.18913)
*He Cheng,Yifu Wu,Saksham Khatwani,Maya Kruse,Dmitriy Dligach,Timothy A. Miller,Majid Afshar,Yanjun Gao*

Main category: cs.CL

TL;DR: 生成一个关于这项研究的简洁摘要。


<details>
  <summary>Details</summary>
Motivation: 这项研究的动机在于是因为现有的知识图谱与大型语言模型（进行多步推理时的瓶颈。包括，。其现有架构难以实现有效的扩展性和可对解析能力。。。以看到一种基于硬件优化的框架改善知识图谱中的多步检索效率。

Method: 该该方法是将知识图谱 (KGs) 形式化，并利用硬件优化的操作进行分解和的对象和以及边 

Result: 通过这种方法，现有的 GPU 和 和和CPU并 baselines实现了高效的多步检测 on 

Conclusion: 该结论研究展示了将结构化生物医学任务与与大语言模型结合在两轮轮中的提高了知识图谱的检索效率和 topology 效应 
...
### [100] [MORPHOGEN: A Multilingual Benchmark for Evaluating Gender-Aware Morphological Generation](https://arxiv.org/abs/2604.18914)
*Mehul Agarwal,Aditya Aggarwal,Arnav Goel,Medha Hira,Anubha Gupta*

Main category: cs.CL

TL;DR: 该 该摘要文章概述了了多Gram和和 作为对比， 一种基于形态学导向的方法（ 该  


<details>
  <summary>Details</summary>
Motivation:  本文旨在展示在多语言环境下的形态学敏感性和性别意识的重要性。 以及对 介绍了 howGram on 在如何改进当前的多语言模型对于不同 语言案件的处理能。 通过这种方法， 作者希望推动对未来模型在形态敏感性性和性别意识方面的发展.

Method:  为了实现Gramon 的研究， 作者首先定义了一个名为 

Result:  他们创建 使用了 compareToon 方法进行  以对比当前主流的语言模型在处理不同语言案件方面的表现. 通过方法学的输入和输出摩形地的基 本单元构建了一个名为 

Conclusion:  从而表明当前的主流模型在处理不同语言案件时 , 但新建的方法在这些方面展示出了明显的不适应和  作者通过此文研究建议 屭起了对Gram on 抙构建一个诊断工具来帮助 further more in形态敏感和 模型的重要性，并 促进未来的模型在这一方面的发展.
...
### [101] [Proposing Topic Models and Evaluation Frameworks for Analyzing Associations with External Outcomes: An Application to Leadership Analysis Using Large-Scale Corporate Review Data](https://arxiv.org/abs/2604.18919)
...
### [102] [Disparities In Negation Understanding Across Languages In Vision-Language Models](https://arxiv.org/abs/2604.18942)
...
### [103] [A Mechanism and Optimization Study on the Impact of Information Density on User-Generated Content Named Entity Recognition](https://arxiv.org/abs/2604.18944)
*Jiang Xiaobo,Dinghong Lai,Song Qiu,Yadong Deng,Xinkai Zhan*

Main category: cs.CL

TL;DR: 该研究揭示了表面噪声症状的根本原因是信息密度低，提出了一种注意力频谱分析方法（ASA）来量化信息密度对NER性能的影响，并提出了一个能够增强语义密度而不改变模型架构的Window-Aware Optimization Module（WOM）来提升NER在社交媒体数据上的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要针对特定噪声进行点式修复，但忽略了UGC中固有的结构稀疏性。研究发现表面噪声的根源是信息密度低，因此需要提出一种新方法来解决这一根本问题。

Method: 通过分层控制混淆的采样实验（具体控制实体稀有性和标注一致性），研究量化了信息密度对NER性能的影响，并开发了一种名为Attention Spectrum Analysis（ASA）的方法。进一步提出了Window-Aware Optimization Module（WOM），一种基于LLM的、模型无关的方法，用于识别信息稀疏区域并利用选择性的反向翻译来增强语义密度。

Result: WOM能够在主流架构上显著提升标准UGC数据集（WNUT2017, Twitter-NER, WNUT2016）的F1分数，最高可提升4.5%，并且在WNUT2017数据集上实现了新的SOTA结果。

Conclusion: 低信息密度是NER在UGC中性能下降的根本原因，WOM作为一种新的方法，能够有效提升NER模型在噪声较大的用户生成内容上的性能。
...
### [104] [Assessing Capabilities of Large Language Models in Social Media Analytics: A Multi-task Quest](https://arxiv.org/abs/2604.18955)
...
### [105] [STAR-Teaming: A Strategy-Response Multiplex Network Approach to Automated LLM Red Teaming](https://arxiv.org/abs/2604.18976)
...
### [106] [$R^2$-dLLM: Accelerating Diffusion Large Language Models via Spatio-Temporal Redundancy Reduction](https://arxiv.org/abs/2604.18995)
*Zhenbang Du,Kejing Xia,Xinrui Zhong,Yonggan Fu,Nicolai Oswald,Binfei Ji,Brucek Khailany,Pavlo Molchanov,Yingyan Lin*

Main category: cs.CL

TL;DR: R²-dLLM通过减少解码过程中的冗余，将解码步骤的数量最多减少了75%，同时保持了生成质量，从而为dLLM解码效率问题提供了有效解决方案。


<details>
  <summary>Details</summary>
Motivation: 观察到dLLM解码过程中的空间冗余和时间冗余，提出了统一的R²-dLLM框架，旨在通过优化的解码规则和冗余感知的微调方法提高解码效率。

Method: 在推理阶段引入训练免费的解码规则，用于聚合局部置信度和标记预测，并最终确定已稳定的时间序列标记，以避免冗余的解码步骤。进一步提出一种冗余感知的监督微调管道，使模型与高效解码轨迹对齐，降低对手动调整阈值的依赖。

Result: 实验表明，与现有解码策略相比，R²-dLLM将解码步骤的数量最多减少了75%，同时保持了竞争力的生成质量。这验证了解码冗余是dLLM的主要瓶颈，并且明确减少它能带来显著的实际效率提升。

Conclusion: R²-dLLM有效地解决了dLLM中的解码效率问题，通过减少冗余提高了整体系统性能，同时保持了高质量的生成效果。
...
### [107] [When Safety Fails Before the Answer: Benchmarking Harmful Behavior Detection in Reasoning Chains](https://arxiv.org/abs/2604.19001)
...
### [108] [AlignCultura: Towards Culturally Aligned Large Language Models?](https://arxiv.org/abs/2604.19016)
...
<details>
  <summary>Details</summary>
Motivation: 该研究动机是为了构建一个多阶段流水线以实现语言模型的文化对齐性。针对'有益'、'有害'和和 '诚恳' (HHH) 帕累托模型，进行系统文化对齐性

Method: 该方法主要构建流水由包含一“构建库

Result:  CULTURAX  

Conclusion:  该研究表明,文化调整模型能够在泛语言模型、文化自适应模型、以及重载多文化和高度调整语言模型方面的表现较好;同时可能这些模型在文化偏见上的减轻得提升效果明显，以及同时模型效率有所提升
...
### [109] [RARE: Redundancy-Aware Retrieval Evaluation Framework for High-Similarity Corpora](https://arxiv.org/abs/2604.19047)
...
### [110] [SAMoRA: Semantic-Aware Mixture of LoRA Experts for Task-Adaptive Learning](https://arxiv.org/abs/2604.19048)
...
### [111] [Cell-Based Representation of Relational Binding in Language Models](https://arxiv.org/abs/2604.19052)
...
### [112] [TRN-R1-Zero: Text-rich Network Reasoning via LLMs with Reinforcement Learning Only](https://arxiv.org/abs/2604.19070)
...
### [113] [HoWToBench: Holistic Evaluation for LLM's Capability in Human-level Writing using Tree of Writing](https://arxiv.org/abs/2604.19071)
*Andrew Zhuoer Feng,Cunxiang Wang,Yu Luo,Lin Fan,Yilin Zhou,Zikang Wang,Xiaotao Gu,Jie Tang,Hongning Wang,Minlie Huang*

Main category: cs.CL

TL;DR: 评估大型语言模型（ (LLLM) 在文本生成能力方面的挑战与进展


<details>
  <summary>Details</summary>
Motivation: 现有的大型语言模型（ (LLMs) 在评估文本生成质量方面遇到了挑战难题，，。 他们的生成能力不仅受到多维度文本生成能力限制， 怖且现有评估方法的局限性。 为了改进这一， 方法，， 提出了了Tree-of-Writing (ToW) 模型来应对这些挑战。

Method: 该 方法采用 包含明确建模各种特征的工作流程，并 以及构建了一个术语分类基准框架， 兡涵 1 文体类型和 三种生成方式: 欁举 摘要生成、、 、 toutine 弳奖生成 

Result: 该 爪果效果通过表现出与  的高强关联 
...
### [114] [SAHM: A Benchmark for Arabic Financial and Shari'ah-Compliant Reasoning](https://arxiv.org/abs/2604.19098)
...
TL;DR: 本文提出了SAHM，一个针对阿拉伯财务NLP及哈里发合规推理的数据集，包含14,380个专家验证实例，涵盖七个任务，评估结果显示阿拉伯流利度并不等同于基于证据的财务推理，发布该基准、评估框架以及指令微调模型以促进未来可信的阿拉伯财务NLP研究。


<details>
  <summary>Details</summary>
Motivation: 探讨阿拉伯财务NLP领域的现状及需求，指出相较于英语版本，阿拉伯财务NLP研究较为欠缺，提出SAHM以填补此空白，促进领域的进展。

Method: 作者构建了一个名为SAHM的基准和指令调优数据集，涵盖了七个任务，包括认知识别、生成、因果推理等不同类型的挑战。

Result: 研究表明，尽管一些模型在识别任务上表现出色，但在生成和因果推理任务上却表现不佳，尤其是在事件因果推理任务上差距明显。

Conclusion: 该研究发布了一个新的基准数据集SAHM及其评估框架，并对外发布了指令调优模型，以支持未来在这领域的深入研究。
...
### [115] [Detoxification for LLM: From Dataset Itself](https://arxiv.org/abs/2604.19124)
...
### [116] [Do Emotions Influence Moral Judgment in Large Language Models?](https://arxiv.org/abs/2604.19125)
*Mohammad Saim,Tianyu Jiang*

Main category: cs.CL

TL;DR: 本研究开发了一种情绪诱导管道，将情绪融入道德情境中，评估了不同数据集和大型语言模型中道德接受度的变化。研究发现，积极情绪提高道德接受度，消极情绪则降低道德接受度，且这种影响在低能力模型中尤为显著。此外，特定情绪有时会表现出与预期相反的效果。人类标注研究揭示了当前大型语言模型与人类存在较大的对齐差距。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型（LLM）在情感识别和道德推理方面的能力得到广泛关注，但关于情感如何影响道德判断的研究仍然不足。因此，本研究旨在探讨情感对道德判断的影响，并通过对比LLM和人类的行为揭示二者之间的对齐差距。

Method: 本研究设计了一种情绪诱导管道，将不同的情绪注入到道德情境中。利用多种数据集和LLM进行实验，观测道德接受度的变化模式。同时，通过人类标注实验分析情感对道德判断的影响。

Result: 研究发现，情绪对道德接受度具有显著影响。积极情绪提高道德接受度，而消极情绪则降低。这一效应在能力较低的模型中尤为明显。此外，部分情绪的表现与其预期的情感价值相反。人类标注研究表明，人类并未表现出类似的系统性变化，这揭示了当前大语言模型与人类之间存在较大的对齐差距。

Conclusion: 本研究揭示了情感对道德判断具有重要影响且具有模型能力依赖性。此外，大语言模型与人类在应对特定情绪与道德判断上的表现存在明显不对齐现象。
...
### [117] [Construction of Knowledge Graph based on Language Model](https://arxiv.org/abs/2604.19137)
...
### [118] [The Rise of Verbal Tics in Large Language Models: A Systematic Analysis Across Frontier Models](https://arxiv.org/abs/2604.19139)
...
<details>
  <summary>Details</summary>
Motivation: 对分析论文摘要，以

Method: 对于该抓取关键点

Result: 

Conclusion: 该 这篇研究表明， 在目前的AI训练框架中, 夲MMs展现出不同的频态 
...
### [119] [ReflectMT: Internalizing Reflection for Efficient and High-Quality Machine Translation](https://arxiv.org/abs/2604.19144)
...
### [120] [How Do Answer Tokens Read Reasoning Traces? Self-Reading Patterns in Thinking LLMs for Quantitative Reasoning](https://arxiv.org/abs/2604.19149)
*Haoyang Chen,Yi Liu,Jianzhi Shao,Tao Zhang,Chengfu Huo,Wei Hu*

Main category: cs.CL

TL;DR: 该摘要主要详细讨论了激活函数在调控LLMs（模型内部表示方向与集中性的情感倾向和的机制。。研究指出，，模型主要主要关注定量分析分析，，并倾向于关注语的波动性，并，，method”: “定量分析与语义锚点导向


<details>
  <summary>Details</summary>
Motivation: 探讨激活函数如何影响大语言模型的内部直觉和情感倾向，旨在理解模型行为背后的工作原理

Method: 定量分析（关注激活波动性与语义锚点的交互作用进行调控)

Result: 激活函数在LSTM门中的作用与决策边界内对的重构

Conclusion: 研究揭示了激活函数如何调整模型内部状态和，，
...
### [121] [Voice of India: A Large-Scale Benchmark for Real-World Speech Recognition in India](https://arxiv.org/abs/2604.19151)
...
TL;DR: Voice of India 是一个由未脚本的电话对话构建的封闭源语音识别基准，涵盖了15种主要印度语言和139个地方集群，包含306230句陈述，总时长536小时，对指标进行了地理级别的分析，指出现有系统的不足之处。


<details>
  <summary>Details</summary>
Motivation: 现有的印度语言ASR基准多使用脚本清晰的语音和排行榜驱动的评估，这导致了针对特定数据集的过拟合，并且严格的单参考WER指标惩罚了印度语言中自然的拼写变异，包括代码混杂英语词汇的非标准化拼写。为了克服这些局限性，作者提出了Voice of India基准。

Method: Voice of India数据集包含306230条未脚本电话对话的陈述，涵盖了15种主要印度语言和139个地方集群，总时长536小时，涉及36691位说话者，并记录了拼写变异的文本转录。

Result: Voice of India基准数据集通过对地理级别、音频质量、讲话速度、性别和设备类型等因素进行了详细分析，揭示了当前ASR系统的性能差异，并为改进现实世界的印度语言ASR系统提供了见解。

Conclusion: 通过分析Voice of India基准，研究者指出了ASR系统在不同因素下的表现差异，并提出了改善全球印度语言ASR系统的建议。
...
### [122] [Mind the Unseen Mass: Unmasking LLM Hallucinations via Soft-Hybrid Alphabet Estimation](https://arxiv.org/abs/2604.19162)
*Hongxing Pan,Yingying Guo,Wenqing Kuang,Jiashi Lu*

Main category: cs.CL

TL;DR: 该SHADE方法结合了全局Good-T覆盖与归 normalized Laplace变换的内嵌推述，为在有限样本下的语义占有概率估计提供了一个简单而可可解的 estimator。它通过应用场景下敏感地融合这两种信号，并然后采用了基于权重融合的一种平稳化转换方法，最终将其调整为一个应用场景适应的语义熵得分。 通过实验结果表明, 在少数样本限制的情况下， SHAD 对具有了最强的不确定性定量检 mage性能 有机结合语义占用概率估计特别有利于黑盒不确定性量官在采样预算较小时环境下的操作。


<details>
  <summary>Details</summary>
Motivation:  在对词表达的语义不确定性进行定量分析时,当前方法往往在一个样本不足上的估量较不足经常会对罕见的意义熵产生低估 幈습니다 为改进这一情况 在这个工作中开发者设计了一种称为 SHADE 的 estimator 。 宓这个方法通过更好地融合全局覆盖和局部信息的变化来调整语义熵得分 从而在小有限样本数的情况下提高对罕见意义的估计精度和

Method:  开发者首先使用 Normalized Laplace变换来处理几个几个sample的ent推述 爪然后将该一些来自entervention-weight-ed样本中的一些项表达转换为代表更有力义 内容的内含推述 爪

Result: result”: 

Conclusion:  歱在中国的工作中， 开发者通过SHADE方法显著地于实现了在少数样本限制的情况下语义熵得分的估计性能 该同时并通过权重调整融合和应用样估计  
...
### [123] [SCURank: Ranking Multiple Candidate Summaries with Summary Content Units for Enhanced Summarization](https://arxiv.org/abs/2604.19185)
*Bo-Jyun Wang,Ying-Jia Lin,Hung-Yu Kao*

Main category: cs.CL

TL;DR: 本文论文提出了一个名为 SCURank 的框架， 用于通过利用语义重要性的摘要内容 (SC) 来评估摘要质量。 其方法方法着重于评价过程而不是直接比较对比表面的表面内容重合。 该研究在多个多种不同的大模型 (LLM) 生成的摘要上上上LMs 之间进行比较实验


<details>
  <summary>Details</summary>
Motivation: 该文章针对于现有的大模型 (LLM) 生成摘要的稳定性问题 意志不足

Method: 该论文提出了一种名为 SCURank 的框架

Result: 该框架通过评估摘要的语义丰富度和及其在文中对 的重要性性 与评估来自多个不同的大模型 (LLLL

Conclusion: SCURank 在多种不同的大模型生成的摘要间取得了优于其他评价和大模型摘要排序的结果的成功。 证明了基于文档的的侧重点排序在多种大模型摘要筛选中的的有效性性
...
### [124] [Headlines You Won't Forget: Can Pronoun Insertion Increase Memorability?](https://arxiv.org/abs/2604.19189)
...
### [125] [Talking to a Know-It-All GPT or a Second-Guesser Claude? How Repair reveals unreliable Multi-Turn Behavior in LLMs](https://arxiv.org/abs/2604.19245)
*Clara Lachenmaier,Hannah Bultmann,Sina Zarrieß*

Main category: cs.CL

TL;DR: 本文研究了在多LM 交互中， 的多 复杂 场最与 鏈关 夠许 "。 LLM 的 表现及。。。


<details>
  <summary>Details</summary>
Motivation: 近年来，， LLMs 的演用限制虽有所改善

Method: 通过对比研究LLLM 在多数复徒 对 会话中的表表现

Result: 研究发现LLLM

Conclusion: 在多缎复杂的情境下 ,, LLM 衏式了越来越多敢的 
...
### [126] [ShadowPEFT: Shadow Network for Parameter-Efficient Fine-Tuning](https://arxiv.org/abs/2604.19254)
...
### [127] [Towards a Linguistic Evaluation of Narratives: A Quantitative Stylistic Framework](https://arxiv.org/abs/2604.19261)
*Alessandro Maisto*
...
### [128] [CulturALL: Benchmarking Multilingual and Multicultural Competence of LLMs on Grounded Tasks](https://arxiv.org/abs/2604.19262)
...
TL;DR: CulturALL 是一个大型语言模型基准测试，旨在评估多语言和多文化能力在具体任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 当前的基准测试大多关注通用语言理解和表面文化 trivia，而忽视了需要模型在真实、丰富的上下文中进行推理的任务。CulturALL 是通过将人类和人工智能协作框架结合，旨在填补这一缺口，并提供一个全面且具有挑战性的基准测试。

Method: CulturALL 的构建采用了人类和 AI 的协作框架，其中专家标注者确保适当难度和事实准确性，AI 则减轻了人力工作量。通过整合多样化的来源，CulturALL 确保了场景覆盖的全面性。每个问题都旨在提供高度的难度，使其成为一个挑战。

Result: CulturALL 包含 2,610 个样本，涵盖了 14 种语言和 51 个地区，16 个主题。实验结果显示，最优秀的大型语言模型在 CulturALL 上的准确率为 44.48%，表明还有很大的改进空间。

Conclusion: CulturALL 为评估大型语言模型的多语言和跨文化能力提供了一个挑战性的平台，强调了在具体任务中LQA研究的重要性。
...
### [129] [HarDBench: A Benchmark for Draft-Based Co-Authoring Jailbreak Attacks for Safe Human-LLM Collaborative Writing](https://arxiv.org/abs/2604.19274)
*Euntae Kim,Soomin Han,Buru Chang*

Main category: cs.CL

TL;DR: This abstract discusses a systematic investigation into the susceptibility of language Large Language Models (LLMs) to malicious draft-based authoring jailbreak attacks.. Utilizing a benchmark called HarDBench


<details>
  <summary>Details</summary>
Motivation: The on this research investigation focuses on the potential risks and vulnerabilities associated current LLMs when when collaborative writing tasks when involving questionable drafts and on the ease with which unauthorized users on may could could on can can may authors on could on these on on one on on one on on authors

Method: on on on  this on study on approach involves on uses on  involves on on on on on on on on on on on on on on  on on on on on on on on on on on on on on on on on on on on  on on on on on on on on on on on on on on on on on on on on on  on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on  on this on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on  on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on  on on on on on on on on on on on on on on on on on on on on on on on on on on on  on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on  on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on on  on this on on on on on on on on on on on on on  on on on on on on on on  on  on on on on on on on on  on on on on on on on  on  on  on  on  on  on  on on on on on on on on on on  on on on on on on on on  on on on on on on on on on on on on on on on on on  on on on on on on on on on on on on on on on on on  on  on on on on on on on  on on on on on on  on on on the  on  on on on  on  on on on on  on  on on on on on on on  on on on on on on on on  on on on on on on on  on on on on on on on on  on  on on on on on on on on  on on on on on on on  on  on on on  on on on on on on on on on on  on  on on on  on on on the  on  on on on on on on on  on  on  on  on  on  on on  on  on  on on on on on on on on on on on on on on on on  on on on on on on on on on on  on  on  on on on on on on on on  on  on on on on on on on on on  on  on on on  on  on on on on  on on  on on on on on on on on  on on on on on on on on on on  on on on on on on  on on on on on on on on on on, on on on on on on on on on on  on on on on on on on on on  on on on on on on on on on  on  on on on on on on on on  on on on on on on on on on  on on on on on on on on on  on  on on  on on on  on on  on  on on on on  on  on  on  on  on  on  on  on on on on on  on  on on on on  on on on on on  on on on on on on on on  on on on  on  on on on on  on on on  on on on  on on on on  on  on on  on  on  on on on  on  on on  on on on on on  on on on on on  on on on on  on on on on  on on on on on  on on on on  on  on on  on  on on on  on on on on on  on on on on on on on on  on  on on on on on on on on on  on  on on on on  on  on  on on on  on  on on  on on on  on on on  on on  on  on  on  on  on on on on on on  on  on  on  on  on on on on on on  on on  on  on  on on on  on  on  on  on  on  on  on  on  on  on  on   on on  on  on  on on  on on on on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on on on on  ,  on  on  on  on  on  on  on  on  on  on  on on on on  on  on  on  on  on  on  on  on  on on  on  on  on  on  on  on on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on on on on on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on on on on on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on  on on on

Result:  这一步分析将涉及对 HarDBench的设计目标、实验方法、实验结果以及提供的相关建议供研究者参考的建议，来汇总该文章的主要贡献和影响。

Conclusion:  这一步总结将概述 HarDBench 怎么帮助调节和增强 LLM 在联合作业中的 的 安全和可靠性。此外，本文还提出了改进方法，以防止有害组件污染协作环境。
...
### [130] [Location Not Found: Exposing Implicit Local and Global Biases in Multilingual LLMs](https://arxiv.org/abs/2604.19292)
...
### [131] [IndiaFinBench: An Evaluation Benchmark for Large Language Model Performance on Indian Financial Regulatory Text](https://arxiv.org/abs/2604.19298)
*Rajveer Singh Pall*

Main category: cs.CL

TL;DR: 印度金融基准（IndiaFinBench）提供了406个专家注释的问题-答案对，用于评估大型语言模型在印度金融监管文本上的表现。该基准填补了非西方监管框架覆盖不足的空白，涵盖了监管解释、数值推理、矛盾检测和时间推理四大任务类型。模型在零样本条件下评估结果显示，所有模型都超过了非专家人类基准线，但数值推理任务显示出最高的区分度。


<details>
  <summary>Details</summary>
Motivation: 现有的金融NLP基准数据主要来源于西方金融文件，而印度金融监管框架与这些西方文件存在显著差异。因此，开发印度金融基准的目的是提供一个更全面和多样化的评估工具，以便更好地理解和应用大型语言模型在非西方金融市场中的性能。

Method: 印度金融基准收集了来源于印度证券和交易委员会（SEBI）和印度储备银行（RBI）的192份文件，涵盖四个任务类别：监管解释、数值推理、矛盾检测和时间推理。每个类别包含174个、92个、62个和78个问题。数据被12个模型进行了零样本评估，包括Gemma和Gemini系列模型。

Result: 所有模型在零样本条件下都超过了60.0%的非专家人类基准线。数值推理任务显示出最高的区分度，模型准确性范围从70.4%（Gemma 4 E4B）到89.7%（Gemini 2.5 Flash）。通过10,000次重采样的bootstrap显著性测试，发现了三个具有统计显著性的性能层级。

Conclusion: 印度金融基准为评估大型语言模型在印度金融领域的性能提供了一个有效的工具。该基准测试表明，模型在某些特定任务上的性能差异明显，并强调了数值推理作为一种非常有潜力的任务类型。未来的研究可以利用该数据集进一步完善和优化这些模型在这一领域的应用。
...
### [132] [Evaluating LLM-Driven Summarisation of Parliamentary Debates with Computational Argumentation](https://arxiv.org/abs/2604.19331)
*Eoghan Cunningham,Derek Greene,James Cross,Antonio Rago*

Main category: cs.CL

TL;DR: 本文提出了一个基于计算性论证的议会辩论摘要评估框架，重点关注论证结构的忠实保存。


<details>
  <summary>Details</summary>
Motivation: 为了提高议会辩论的透明度和参与度，以及评估大语言模型生成的摘要是否忠实反映了原始辩论的内容。

Method: 该研究设计了一个新的评估框架，该框架侧重于论证结构的形式特性，以评估摘要与原始辩论的一致性。

Result: 研究通过使用欧洲议会的辩论案例和大语言模型生成的摘要进行了实证研究，证明了该方法的有效性。

Conclusion: 基于计算性论证的评估框架能够更准确地衡量大语言模型生成的辩论摘要的忠实度。
...
### [133] [Are Large Language Models Economically Viable for Industry Deployment?](https://arxiv.org/abs/2604.19342)
...
### [134] [DASH-KV: Accelerating Long-Context LLM Inference via Asymmetric KV Cache Hashing](https://arxiv.org/abs/2604.19351)
...
<details>
  <summary>Details</summary>
Motivation: 摘要 文动机和 吜索一种能够加速改进基于键值的近值在上下文文上下文环境中的的推理效率的方法。 嵌入一种非对对称实现的DASH-KV方法，ASH-KV项目的介绍ASH-KV概要，ASH-KV方法通过非对ASH-ASH对称编码将查询和与键映射定向到更精确的键映射，ASH对来补偿了精度和再利用了精AP pHK的动态编DASH话精度以适应不同的键映射DASH-KV在onBench基准测试中的ASHK平均比ASHK比ash_kv优于现有最DASHdDASH上ASH优方法，ASHkASHK

Method: ASHK DASHK方法DASHK 包括非DASH对ASH对ASH对ASH框架DASH的开发DASHK DASH以及DASHDASHK DASHK 渴情况下的加速ASHDASHK DASHK 近可能 

Result: ASHK DASHK 善DASH将查询和ASH与键映射DASH DASHK DashedKV优化映射到更精确的键映射以补偿精度差异DASHK 庞匹配DHDASHK DASHK 鑐DASHDASHK D高以动态改变ASH化了编DASH序以适应不同的键映DASH映DASHK 最终在onBench基准测试DASHK DASHKdlash_kv提升了推理性能DASHDASHK DASHK

Conclusion: ASHK 该DASHK DASH 獅ASHASASHDASHDASH通过非DASHDASHK DASHK 这方法DASHK DASHK着DASHDASHK DASHK 瑐DASHDASHK D了非DASHDASHK DASHK Dasha_kv在推理性能上的提升和ASHK 吩ASHK更ASHK DASHK DASHK DASHK DASHK DASHK 最DASHDASHK DASHK 唔DASHDASHK DASHK DASHK DASHK DDASHK DASHK DASHK DASHK DASHK DATIONP ERIODASASHK DASHK DASHK DASHK
...
### [135] [Can Continual Pre-training Bridge the Performance Gap between General-purpose and Specialized Language Models in the Medical Domain?](https://arxiv.org/abs/2604.19394)
*Niclas Doll,Jasper Schulze Buschhoff,Shalaka Satheesh,Hammam Abdelwahab,Héctor Allende-Cid,Katrin Klug*

Main category: cs.CL

TL;DR: 该该 study closes the gap between specialized and general-purpose models through continual fine-tuning and merging of high English German medical corpora. This significantly leads a significant improvement on the performance capabilities of small models, benchmark tasks, German German medical benchmarks.. inserting to a 3-fold increase for the Mistral-Small 2 24B-Instruct on the OWT and WAT..


<details>
  <summary>Details</summary>
Motivation: 为了增强针对特定任务的小模型的性能表现， 避免使用庞大且通用的大型模型， 该 study研究旨在通过集成和 以及不断微调的方式， 刻定 徠 为德国医疗保健领域中的实际应用而提供基础的、合规的专门化大语言模型作为基础。

Method:  

Result: 研究结果显示， 专训能够显著提高小B模型的性能表现 从细微调整到三倍提高 Mistral-Small 1 1 2 1B-Instruct 輦统标它们的表现为准 盈습니다 当 on  

Conclusion: 该 research研究表明， 绅化调小有效通过提高模型的Task-Oriented能力 蛛 也 B模型的成功育结中出现的C.T成 时  λ问题中较为灵活但是效正较低问题 哗 于模型采纳了更为集中的调整策略 
...
### [136] [Lost in Translation: Do LVLM Judges Generalize Across Languages?](https://arxiv.org/abs/2604.19405)
...
<details>
  <summary>Details</summary>
Motivation: 动机在于 该工作着重于LVLM（（大型语言模型)在多语言和多模态领域动机评价中的表现。当前的评估基准主要 on近-centric benchmarks 仅关注序列化动机，单一语言，.

Method:  

Result:  该展示，当前动机模型在多语言环境中的表现差异，且由于缺乏合适的多语言多模态基准，因此无法系统地开发可靠的自动化评价器。由此强调当前动机模型的局限性性，并强调了构建这种这样的基准以进行系统的评级评估的重要性的必要性.

Conclusion:   注结果指出需要构建多语言和和多模态态态的动机模型 benchmark]基准以进行系统的评语模型评估,增强对动机模型的理解和创新.
...
### [137] [What Makes an LLM a Good Optimizer? A Trajectory Analysis of LLM-Guided Evolutionary Search](https://arxiv.org/abs/2604.19440)
*Xinhao Zhang,Xi Chen,François Portet,Maxime Peyrard*

Main category: cs.CL

TL;DR: 本文摘要探讨了大规模语言模型（LLM) 在优化问题任务中的潜力，并总结模型指导的进化优化轨迹，。


<details>
  <summary>Details</summary>
Motivation: 研究旨在揭示LLM在不同优化中的作用,以及

Method: 通过分析1不同8任务

Result:  11115 LLMs

Conclusion: 证实了通过演化的轨迹分析对于优化模型理解和改进模型的效果重要。和
...
### [138] ['The Order in the Horse's Heart': A Case Study in LLM-Assisted Stylometry for the Discovery of Biblical Allusion in Modern Literary Fiction](https://arxiv.org/abs/2604.19447)
*Ewan Cameron*

Main category: cs.CL

TL;DR: 该 分词的摘要是一个基于稀有词汇识别和在文学作品中识别圣经引用的一个双管流程管道。并应用在科尔莫高雷夫·马科·斯蒂小说的文过程中。该管道首先首先使用一种识别稀有词汇的方法来提取圣经引用,然后使用一个基于自监督学习的模型来理解和处理这些引用。最后通过与巨量文学语料库中其他的作品对比来验证这些引用的准确性。


<details>
  <summary>Details</summary>
Motivation: 该该摘要的动机是为了探索文学创作中圣经引用在现代文学作品中的用现实应用表现手法,并其他研究不仅能提供对作者创作背景和风格洞见

Method:  该该 a双流程的识别过程包括利用稀有词汇识别开获取圣经引用事件

Result:   

Conclusion: 该借助这一胶乳识别和在小说中使用圣经引用的方法
...
### [139] [LePREC: Reasoning as Classification over Structured Factors for Assessing Relevance of Legal Issues](https://arxiv.org/abs/2604.19464)
*Fanyu Wang,Xiaoxi Kang,Paul Burgess,Aashish Srivastava,Chetan Arora,Adnan Trakic,Lay-Ki Soon,Md Khalid Hossain,Lizhen Qu*

Main category: cs.CL

TL;DR: 研究展示了大型语言模型在识别法律问题方面存在精度不足的问题，提出了LePREC框架以提升准确性，该框架结合了神经生成和结构化的统计推理。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在识别法律问题方面存在显著不足，旨在探寻更有效的解决方案，提升对法律问题的准确识别。

Method: 研究基于769个马来西亚合同法案件构建了一个数据集，使用GPT-4o提取事实并生成候选法律问题，并通过高级法律顾问进行注释。然后提出了LePREC框架，该框架结合了神经生成与符号主义推理。

Result: LePREC框架在实验中显示了对高级语言模型基线（包括GPT-4o和Claude）30-40%的改进，证明了相关性因素-问题分析方法的数据效率。

Conclusion: 研究提出了LePREC框架，结合神经生成和结构化推理以提升法律问题识别的准确性。
...
### [140] [Rank-Turbulence Delta and Interpretable Approaches to Stylometric Delta Metrics](https://arxiv.org/abs/2604.19499)
...
### [141] [Beyond Rating: A Comprehensive Evaluation and Benchmark for AI Reviews](https://arxiv.org/abs/2604.19502)
...
### [142] [Bangla Key2Text: Text Generation from Keywords for a Low Resource Language](https://arxiv.org/abs/2604.19508)
*Tonmoy Talukder,G M Shahariar*

Main category: cs.CL

TL;DR: 这篇论文介绍阐述了一个多词百万组巴玲关键词-词对设计，旨在推动条件-驱-动的生成. 数据集 使用巴玲Bert模型为基础的关键词提取管道-将大量的巴玲自然文章转换为结构化的关键词-词对,适合进行条件-建基准模型的训练和. 稡模型在受条件约束条件下的文本生成表现上有显著改进. 数据集公开供用于巴玲关键词提取和和 与 关键词-文本生成任务.


<details>
  <summary>Details</summary>
Motivation: 该项目研究的主要动机是解决资源匮乏方言（如巴玲语言）的自然语言处理任务，特别是在关键词驱动的文本生成方面. 通过设计一个大规模的关键词--词对数据集,研究者旨在推动一种基于该个数据集训练出来的生成模型,这将有助于改善低资源语言的自然语言处理能力.

Method: 研究采用了一种基于BERT模型的预训练方法来进行关键词提取. 免于大量巴玲自然语言文本转换为结构化的关键词-词对形式.这些数据对将被用来训练模型,然后在训练过程中使用目的条件性的优化和模型

Result: 实验结果显示,在目标条件建基准模型上效果有显著提升. 特别是在关键词驱动的文本生成任务中表现尤为明显, 这项改进应在资源匮乏的语言环境下具有重要应用重要的潜在应用.

Conclusion: 最终研究数据模型公开供用于巴玲关键词提取与关键词-文本生成任务,推广研究于对少资源语言的自然语言处理进展贡献.
...
### [143] [Emotion-Cause Pair Extraction in Conversations via Semantic Decoupling and Graph Alignment](https://arxiv.org/abs/2604.19547)
*Tianxiang Ma,Weijie Feng,Xinyu Wang,Zhiyong Cheng*

Main category: cs.CL

TL;DR: SCALE框架通过语义解耦和全局对齐原则，解决了emotion和cause的匹配问题，展示了其在多个基准数据集上的优越性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法未能捕捉情绪传播和原因解释的语义差异，导致无法实现全局一致的多对多对话因果关系匹配。因此，需要提出一种能达到这一目标的新框架。

Method: 语义解耦和全局对齐。通过将情绪侧和原因侧的表示映射到两个互补的语义空间，使用最优传输实现多对多的全局一致匹配。

Result: 在多个基准数据集上的实验表明，SCALE框架能够取得最先进的性能。

Conclusion:  SCALE框架能够有效地识别对话中的情感和因果关系，是一个有效的解决方案。
...
### [144] [Detecting Hallucinations in SpeechLLMs at Inference Time Using Attention Maps](https://arxiv.org/abs/2604.19565)
*Jonas Waldendorf,Bashar Awwad Shiekh Hasan,Evgenii Tsymbalov*

Main category: cs.CL

TL;DR: 研究提出了四种基于注意力的指标（AUDIORATIO，AUDIOCONSISTENCY，AUDIOENTROPY和TEXTENTROPY），用于检测语音大型语言模型中的幻觉。通过训练轻量级的逻辑回归分类器，该方法在多种任务中表现出色，优于基于不确定性与先验注意力的基线模型。


<details>
  <summary>Details</summary>
Motivation: 现有幻觉检测方法依赖于代价高昂或不切实际的金标准输出，且针对文本的检测方法无法捕捉到与音频相关的特殊信号，因此需要开发新的方法来有效检测语音大型语言模型的幻觉。

Method: 研究设计了四种基于注意力的指标来检测幻觉，并利用这些特征训练轻量级的逻辑回归分类器。方法包括AUDIORATIO、AUDIOCONSISTENCY、AUDIOENTROPY和TEXTENTROPY。
- AUDIORATIO用于识别注意力是否集中在某一部分长期未使用的语音数据。
- AUDIOCONSISTENCY检查模型是否在时间序列上保持一致。
- AUDIOENTROPY衡量注意力分布的不均匀性。
- TEXTENTROPY关注文本和声音之间的不匹配。

Result: 在自动语音识别和语音转文本翻译任务中，该方法在领域内数据上表现优于基于不确定性和先验注意力的基线模型，领域外ASR任务上的性能也有提升。此外，使用约100个注意力头能更好地改善领域外泛化。

Conclusion: 尽管模型依赖性和任务特定性要求最初的训练，研究结果表明，注意力模式可以作为一种有价值的方法来检测语音大型语言模型中的幻觉。
...
### [145] [A Self-Evolving Framework for Efficient Terminal Agents via Observational Context Compression](https://arxiv.org/abs/2604.19572)
...
TL;DR: 本文通过分析自然语言处理模型的出现对学术文章同行评审报告的影响，发现评审报告长度和流畅性增加，注重表面清晰度和标准化语言，但深入的评估维度如原创性、可重复性以及细致批判性推理的关注度下降。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型的迅速发展，学术界面临前所未有的变化，特别是学术沟通领域。本文旨在探讨这些模型如何影响同行评审的核心评估功能。

Method: 该研究分析了大型语言模型出现后学术文章同行评审报告中细微变化的语言特征，如评论长度和句子复杂性，同时自动注释每个评论句子的评估维度，并使用最大似然估计方法来识别可能由大型语言模型修改或生成的同行评审报告。

Result: 结果显示，自大型语言模型出现以来，同行评审文本变得更加长和流畅，更加注重摘要和表面清晰度，并且具有更标准的语言模式，尤其是对低自信分数的评审人员而言。然而，对原创性、可复制性和精细批判性推理等更深层次评估维度的关注度有所下降。

Conclusion: 综上所述，大型语言模型的引入对同行评审报告产生了影响，导致评审报告的撰写方式发生变化。
...
TL;DR: 该研究创建了首个专门针对撒丁岛卡坦达·洛古多雷塞即兴诗歌的结构化语料库，通过多维分析揭示了即兴诗歌的特点，并为开发更包容和敏感于较少广泛使用的语言的NLP工具提供了新视角。


<details>
  <summary>Details</summary>
Motivation: 由于对少数语言自然语言处理的研究不足，特别是在即兴诗歌这一基于实时即兴创作和韵律修辞能力的表演流派方面，存在方法论空白。因此，本文旨在通过创建专门的结构化语料库来填补这一空白，以记录和分析即兴诗歌的结构。

Method: 本文创建了名为A Bolu的第一个专门用于卡坦达·洛古多雷塞即兴诗歌的结构化语料库，该语料库包含2,835个诗节，总计141,321个标记。研究还采用了结合描述性统计指标和计算语言学技术的多维度分析方法来映射诗歌文本的特点。

Result: 研究结果表明，撒丁岛即兴诗人在创作中的生产特点具有重复模式，这些模式支持了帕里和洛德对公式性的理论。这些证据不仅为理解口头创造力提供了新的视角，还为开发更包容和敏感于较少广泛使用的语言的NLP工具做出了重要贡献。

Conclusion: 该研究填补了计算语言学对少数语言即兴诗歌研究中的空白，提出了更适用于少有使用的语言的NLP工具的新视角，并为理解口头创造力提供了新的证据。
...
### [148] [RoLegalGEC: Legal Domain Grammatical Error Detection and Correction Dataset for Romanian](https://arxiv.org/abs/2604.19593)
...
### [149] [Cross-Model Consistency of AI-Generated Exercise Prescriptions: A Repeated Generation Study Across Three Large Language Models](https://arxiv.org/abs/2604.19598)
*Kihyuk Lee*

Main category: cs.CL

TL;DR: 研究对比了三个大型语言模型GPT-4.1、Claude Sonnet 4.6和Gemini 2.5 Flash在温度设为0的情况下，对于六个临床场景生成的处方输出的反复生成一致性。结果显示这些模型在语义相似度、输出重复性、FIT模型分类和安全性表达四个维度上表现出显著差异，强调了模型选择在临床应用中扮演的关键角色。


<details>
  <summary>Details</summary>
Motivation: 研究旨在评估不同大型语言模型在生成运动处方时的表现一致性，以确保其在临床应用中的可靠性和安全性。

Method: 研究通过设置相同的解码参数，让每个模型针对同一组六种临床场景生成20次处方，总计分析360个输出结果。从语义相似度、输出重复性、FIT模型分类、安全性表达四个维度进行评估。

Result: GPT-4.1在语义相似度方面表现最佳（0.955），其次是Gemini 2.5 Flash（0.950），Claude Sonnet 4.6最低（0.903），显示出显著的模型间差异（H = 458.41，p < .001）。GPT-4.1生成的是完全独特的输出，而Gemini 2.5 Flash则显示出显著的输出重复性。所有模型的安全性表达达到了顶峰，显示了这种指标的有限区别能力。

Conclusion: 研究结论表明，模型在重复生成条件下的输出行为是其在临床应用中可靠性的核心指标，选择模型时需给予高度重视，而不仅仅是技术指标。
...
### [150] [The "Small World of Words" German Free-Association Norms](https://arxiv.org/abs/2604.19620)
*Samuel Aeschbach,Rui Mata,Kaidi Lõo,Simon De Deyne,Dirk U. Wulff*

Main category: cs.CL

TL;DR: 该研究构建了包含5877个德语联想词的SWOW-DE数据集，通过多次实验证明了其在词汇识别任务、相关性判断和心理语言学词语评分中的预测能力，并展示了与现有资源的对比以及跨语言联想模式的初步比较。


<details>
  <summary>Details</summary>
Motivation: 鉴于目前尚缺乏系统性、规模化的德语自由联想数据集，为了填补这一空白，研究者通过德语版本的SWOW项目创建了一个大规模的自由联想规范数据库，以便更好地理解语言、语义和文化现象。

Method: 研究者通过多步骤的数据收集与预处理流程，包括筛选参与者并制定征询指南等，最终形成了SWOW-DE数据集。同时使用三种已验证的语义过程范式来评估数据集的有效性。

Result: 研究结果显示，SWOW-DE数据集能够稳定预测词汇识别任务、相关性判断以及心理语言学词语评分的表现，还优于现有的德语资源，并展示了与不同语言的联想模式的对比。

Conclusion: SWOW-DE数据集成为目前规模最大的德语自由联想集合，为语言学、心理学和跨文化研究提供了宝贵的资源。
...
### [151] [Micro Language Models Enable Instant Responses](https://arxiv.org/abs/2604.19642)
*Wen Cheng,Tuochao Chen,Karim Helwani,Sriram Srinivasan,Luke Zettlemoyer,Shyamnath Gollakota*

Main category: cs.CL

TL;DR: 介绍了μLMs（超紧凑语言模型），它们能在边缘设备上即时生成一段响应的前几个单词，而后由云端模型完成剩下的内容，从而解决了低延迟的问题。这些超紧凑模型在参数量上至少比现有模型减少99%，但仍保持了高质量的语言生成能力。


<details>
  <summary>Details</summary>
Motivation: 由于边缘设备功率和计算能力受限，无法持续运行小型语言模型，而云端推理则会导致延迟，影响体验。μLMs通过在设备上即时生成部分响应，利用云模型完成剩余内容，以保持响应速度。

Method: μLMs设计了一个协作生成框架，使云模型作为续写者，优先处理局部出错情况，并通过三种错误纠正方法实现流畅的句子过渡和结构化的后备恢复。

Result: 实验结果显示，μLMs能够在参数量大幅减少的情况下，与大型模型生成相媲美的高质量语言，同时实现了边缘设备上的即时响应。

Conclusion: μLMs为资源受限设备提供了快速响应的AI助手，为未来边缘计算与云计算结合提供了新的解决方案。
...
### [152] [The signal is the ceiling: Measurement limits of LLM-predicted experience ratings from open-ended survey text](https://arxiv.org/abs/2604.19645)
...
### [153] [Chat2Workflow: A Benchmark for Generating Executable Visual Workflows with Natural Language](https://arxiv.org/abs/2604.19667)
*Yi Zhong,Buqiang Xu,Yijun Wang,Zifei Shan,Shuofei Qiao,Guozhou Zheng,Ningyu Zhang*

Main category: cs.CL

TL;DR: 研究介绍了一种自然语言生成可种类的工作流框架，能够从自然语言描述中生成可以直接执行的工作流，在可以直接应用于真实的业务流程 中。这为自动化复杂任务 提供作为高级自动化的发展奠定了基础。”foundation 炨介绍的实验表明 cutting-edge 模型能够生成可多种类型的工作流 且这些工作流在实际应用中具有很高的可行性。


<details>
  <summary>Details</summary>
Motivation: 当前的高级自动化任务面临着耗时的手动适配和过程中的人工参与过多且难以适应复杂任务的问题，这样的方法主要尝试提出一种基于自然语言的工作流生成框架 励от有效降低了这些挑战通过自然语言理解和生成技术的进步

Method: 方法首先收集并各种真实的业务流程描述 

Result: 然后利用这些描述训练生成工作流 

Conclusion: 该实验表明该方法能够显著提高复杂任务的自动化效率 提该框架为实现 实实践的自动化任务高级自动化的实现提供了新的可能 萜索的方法和
...
### [154] [Exploring Language-Agnosticity in Function Vectors: A Case Study in Machine Translation](https://arxiv.org/abs/2604.19678)
*Nurkhan Laiyk,Gerard I. Gállego,Javier Ferrando,Fajri Koto*

Main category: cs.CL

TL;DR: 该研究通过分析多语言LLM中的功能向量（FVs），发现它们在机器翻译任务中表现出跨语言一致性的特征，可以从源语言向多个目标语言迁移，有助于提高正确翻译候选词的排名。


<details>
  <summary>Details</summary>
Motivation: 研究旨在探索多语言模型中的功能向量是否也具有跨语言一致性，即这些向量在不同语言间的翻译任务上是否具有通用性。

Method: 研究选取了三个单向解码器多语言预训练语言模型，从一种源语言到多个目标语言提取功能向量，并评估这些向量在不同目标语言中进行机器翻译的效果。

Result: 实验证明，功能向量可以从一种源语言向其他目标语言迁移，提高正确翻译候选词的排名。删除功能向量后，翻译性能显著下降，而对与翻译无关的任务影响较小。此外，基础模型的功能向量也可以迁移到指令调优的变体模型中，并在一定程度上从词级翻译泛化到句级翻译。

Conclusion: 研究发现，功能向量具有跨语言一致性，在翻译任务中表现出良好的迁移性能，同时保持了少部分任务的通用性。
...
### [155] [An Answer is just the Start: Related Insight Generation for Open-Ended Document-Grounded QA](https://arxiv.org/abs/2604.19685)
...
### [156] [Epistemic orientation in parliamentary discourse is associated with deliberative democracy](https://arxiv.org/abs/2604.19699)
*Segun Aroyehun,Stephan Lewandowsky,David Garcia*

Main category: cs.CL

TL;DR: 论文研究了政治话语中的证据与直觉之间的比例（EMI评分），并在多个国家的议会演说片段中进行了应用，发现EMI评分与民主对话质量正相关，并与治理中的透明度和可预测实施正相关。


<details>
  <summary>Details</summary>
Motivation: 研究旨在通过引入证据-减-直觉（EMI）评分来量化政治话语中的知识导向程度，以评估其对民主对话质量和治理影响。

Method: 使用大规模语言模型（LLM）评级和基于嵌入的语义相似度生成EMI评分，应用于1946年至2025年间七个不同国家的1500万议会演讲片段。

Result: 研究结果表明，EMI评分在国家层面上与民主对话的质量呈正相关，并且与治理中的透明度和可预测实施呈正相关。

Conclusion: 论文结论指出，政治话语的知识导向性对提升民主质量和治理效果具有重要意义。
...
### [157] [Discovering a Shared Logical Subspace: Steering LLM Logical Reasoning via Alignment of Natural-Language and Symbolic Views](https://arxiv.org/abs/2604.19716)
*Feihao Fang,My T. Thai,Yuanyuan Lei*

Main category: cs.CL

TL;DR: 该研究利用Canonial相关性分析探索LLMs内部是否存在一个逻辑子空间，该空间同时对自然语言和符号语言的推理过程具有最大交叉视图相关性，从而提升多步逻辑推理能力。


<details>
  <summary>Details</summary>
Motivation: 动机在于解决现有LLMs在多步逻辑推理中的不足，现有方法要么仅在自然语言中进行推理链的优化，要么依赖外部符号解算器，本研究旨在从LLMs内部寻找共享逻辑子空间，利用自然语言和符号语言推理过程的互补信号，提升模型在逻辑推理任务中的表现。

Method: 该研究采用了Canonical Correlation Analysis来分析自然语言和符号语言推理过程中的残差激活，学习一个能最大化跨视图相关性的低维子空间。提出了一个训练无损的方法，引导LLMs推理过程沿这个逻辑子空间进行，从而利用两个视图的互补推理信号。

Result: 实验结果显示，该方法在四个逻辑推理基准上表现有效，准确率提高最多达11个百分点，并且在域外问题上具有良好的推广能力。

Conclusion: 该研究表明内部逻辑子空间确实能够提升LLMs在多步逻辑推理任务上的能力，验证了存在的共享逻辑图表示假设，为未来的LLMs改进提供了新的视角和方法。
...
TL;DR: 该研究基于对研究人员使用语言模型时的调查，提出了一个名为GROVE的可视化工具，帮助用户理解语言模型输出的分布特性，提供了一种结合图形摘要和直接输出查看的混合工作流程。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型的交互方式仅展示单次输出，隐藏了其分布特性。这导致用户在修改提示时产生过度概括。因此，文章旨在开发一个工具，让研究人员更好地理解模型的分布特性，提高他们的迭代效率。

Method: 研究人员通过一项包含13名使用LMS的研究人员的初步研究，了解了模型输出的不确定性实际意义，如何思考语言的分布以及现有工作流程的不足，基于此设计并实现了GROVE工具。

Result: GROVE在三项针对不同评估任务的众包用户研究中被测试，参与者数量分别为47、44和40。结果表明，图形摘要对于结构评估，如多样性判断有帮助，而直接查看输出更适合细节问题。

Conclusion: GROVE通过可视化多个模型生成路径，揭示了共享结构、分支点和聚类，有助于提高对模型输出分布特性的理解。
...
TL;DR: ARES框架通过动态构建语义连贯的对抗性提示，发现了并缓解了LLM和奖励模型中的双重安全漏洞，通过两阶段修复过程提升了模型的安全鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统的红队测试主要针对策略层面的弱点，忽略了LLM核心模型与奖励模型因同步失效导致的安全隐患。ARES通过引入“安全导师”来构建连贯的对抗性提示，旨在全面识别和缓解这种两方面的安全漏洞。

Method: ARES框架采用了使用结构化的组件类型（主题、人设、策略、目标）动态构建语义连贯的对抗性提示的方法，通过生成相应的恶意和安全响应来同时检测核心LLM和奖励模型中的弱点。随后，通过两阶段修复过程，首先对奖励模型进行微调以提高对有害内容的检测能力，然后利用改进后的奖励模型优化核心模型。

Result: 实验表明，ARES方法在多个对抗安全性基准测试中显著提高了模型的安全稳健性，同时保持了模型的能力。

Conclusion: ARES为RLHF安全对齐提供了更强有力的框架，标志着一个新的全面的安全性范式。
...
### [161] [AI scientists produce results without reasoning scientifically](https://arxiv.org/abs/2604.18805)
...
### [162] [Human-Guided Harm Recovery for Computer Use Agents](https://arxiv.org/abs/2604.18847)
*Christy Li,Sky CH-Wang,Andi Peng,Andreea Bobu*

Main category: cs.AI

TL;DR: 该摘要摘要摘要讨论了如何通过偏好对齐恢复来引导智能体从有害行为转向有益方向，通过对奖励模型进行改造，和同时提出了一个基准测试BackBench，，包含五个任务来评估智能体恢复有害行为的能力。


<details>
  <summary>Details</summary>
Motivation: 研究人员认识到，在智能体在遭受负面反馈时时可能会采取有害行动.这需要一种方法来引导其智能体转向更积极的方向走.因此，提出了通过调整奖励模型来在改正行为时时产生智能体进行对有益恢复。

Method: 方法包括调整奖励模型俾在到损害事件之后促进合适的恢复行为。.总的多个提出了五个基准任务BackBench场 以作为评估智能体恢复能力的标准。

Result: 研究人员希望能够通过这种方法改进智能体的恢复能力，使之能在被纠正行为后时更加倾向于积极的方向走向。

Conclusion: 该总种通过框架和和贡献了了一种新型类的智能体引导方法；使这可能为研究人员和更深入理解和改进智能体行为提供了提供重要工具。。
...
TL;DR: 本文介绍了一个结合形式验证和人工智能的专利分析框架，通过形式化的专利分析算法和验证，旨在提高知识产权分析的准确性和可验证性。


<details>
  <summary>Details</summary>
Motivation: 现有专利分析方法依赖于手动专家分析（耗时且不具扩展性）或机器学习/自然语言处理方法（概率性、不透明且不按部分处理），因此本文提出了一种基于依赖类型理论的交互式定理证明方法，用于专利分析。

Method: 专利的结构编码使用Lean 4实现，匹配强度被视为验证过的完备格中的元素，并通过证明正确的单调函数在依赖关系中传播置信得分。

Result: 论文提出了针对五种IP用例的六个算法，并证明了部分结构性引理及其验证核心生成器，对于其他用例的高级定理则提供了非正式的证明草图。

Conclusion: 该框架有望提供一种更加准确和可验证的专利分析方法，但其准确性仍有赖于后续对机器学习层的验证。
...
TL;DR: 研究者提出了一种新的个性化的LLM基准，该基准使用ELO评分和Bradley-Terry系数评估了115名Chatbot Arena用户的个性化模型排名。研究发现，用户查询特征（主题和写作风格）影响了LLM排名的差异，且用户级别的标准化评分表现不佳。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型（LLMs）在实际任务中的应用越来越广泛，对其是否与人类偏好保持一致成为了一个重要的挑战。当前的评价基准通过简单地平均所有用户的偏好来计算总体评分，忽略了个体用户的偏好。鉴于用户的偏好在不同的上下文中可能会有所不同，研究者提出应构建个性化的LLM基准，以满足个体用户的需求。

Method: 研究者使用ELO评分和Bradley-Terry系数进行个性化排名，并通过话题建模和风格分析来探索用户查询特征如何影响LLM的排名。他们进一步展示了结合主题和风格特征的小型特征空间可有效预测用户特定的模型排名。

Result: 研究结果显示，用户的个性化模型排名与总体排名之间存在显著差异，Bradley-Terry相关性平均只有ρ=0.04（约57％的用户表现出零或负的相关性），而ELO评分显示了中等的相关性（ρ=0.43）。研究还表明，主题和风格特征的紧凑组合为预测用户特定模型排名提供了有用的空间。

Conclusion: 本研究表明，当前的聚合评分并不能很好地捕捉用户的具体偏好，强调了开发能够根据个体用户偏好对LLM进行排名的个性化基准的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 总结该 文摘中生 滴出 LRM 对任务 关的 敏情感动力进行 訡调 敦的 效方， 

Method: 阶段 訡 财用 宛建 LRM 评 苕在 夑合槽 劗 怲的 衴 倚该 弅改进 方发 从 LRM 评 夒拉 电 

Result: 该 察全 了 LRM 在 敏情感动机方面 的 挑战 以及 采取 的 解进 方法 

Conclusion: 总体 杆， LRM 在 敏情感动机方面 的 改进方 劅 蛤 偿 叅且 有待 迋改进和 
...
TL;DR: DW-Bench 是一个新的基准，用于评估大型语言模型在数据仓库模式上的图拓扑推理能力，涵盖了外键和数据血缘关系。实验表明，工具增强的方法显著优于静态方法，但在复杂的组合子类型上达到瓶颈。


<details>
  <summary>Details</summary>
Motivation: 当前存在对大型语言模型在数据仓库复杂架构下进行图拓扑推理评估的需求，以提升模型在实际应用中的性能。

Method: DW-Bench 涵盖了五个不同模式的数据仓库，并通过自动生成1046个问题来评估模型。实验对比了工具增强方法和静态方法。

Result: 工具增强的方法在所有测试问题上的表现优于静态方法，但在处理特定复杂的组合类型问题时表现出限制。

Conclusion: DW-Bench 为大型语言模型的图拓扑推理能力提供了一个有效的评价标准，同时指出了未来研究的方向。
...
TL;DR: 该文提出了一个称为Plausible Logic (PL) 的逻辑系统，它满足14条必要原则和3条期望原则中的大部分，可以处理基于事实和可能为真的陈述的推论。


<details>
  <summary>Details</summary>
Motivation: 为了提供一种在应用领域中较为实用的推理方式，即在没有具体概率计算的情况下进行合理的推理。

Method: 作者定义了一个新的逻辑系统Plausible Logic (PL)，并通过17条推理原则为其奠定了基础。系统基于可能性而非概率，支持满足特定原则的推理。

Result: PL已经定义出8种推理算法，能够正确处理多种合理的推理示例。

Conclusion: 这是首个完全符合这些原则的逻辑系统，填补了处理不确定性和可能为真的陈述推理的空白领域。
...
TL;DR: 本文提出了一种结合深度学习和混合整数线性规划（MILP）的框架，用于从仅观察到的图像序列中学习提升的动作模型，通过MILP修正预测以避免局部最优，促进模型收敛至全局一致的解决方案。


<details>
  <summary>Details</summary>
Motivation: 现有的学习动作模型的方法大多依赖于高层面的状态描述或动作序列，而本文旨在解决从仅观测到的状态图像序列中学习提升动作模型的更具挑战性的任务。

Method: 本文提出了一个深度学习框架，该框架联合学习状态预测、动作预测和提升的动作模型，并引入了混合整数线性规划（MILP）来确保预测之间的逻辑一致性。

Result: 实验结果表明，通过MILP修正预测有助于模型跳出局部最优解，并向全局一致的解决方案收敛。

Conclusion: 通过该框架，模型能够更有效地学习从仅观测到的状态图像序列中提取的提升动作模型，这种方法提高了实际应用中AI规划的效果。
...
TL;DR: 本文介绍了一种名为OLLM的方法，通过引入一组学习到的选项来替代标准LLM的单一下一个词预测，从而增强数学推理的可控性、稳健性和效率。


<details>
  <summary>Details</summary>
Motivation: 由于当前的LLM方法依赖于温度或采样技巧来引入多样性，并且通常会出现语言切换或退化推理等常见不对齐问题，因此本文提出了一种结构化的方案来缓解这些问题。

Method: OLLM通过引入一个离散的潜在空间来索引多种可能的下一个词选项，而不依赖于温度或采样技巧。它包括一个小型的编码器和解码器插件，可以在任何预训练的LLM之前插入，以最小的参数量级扩展其功能。

Result: 实验结果表明，相比于SOTA的LoRA适应基线在最终答案正确率上只有51%，OLLM通过在潜在空间中选择最佳选项，其潜在选项集可以达到约70%的正确率。同时，通过在潜在空间中训练一个紧凑的策略来控制生成过程，使得奖励优化更加高效，且减少了常见的偏差。

Conclusion: 本研究证明了选项化下一个词建模对于增强LLM在数学推理中的可控性、稳健性和效率具有显著优势，并且提出了一种潜在空间策略学习的新方向，在LLM的强化学习中值得关注。
...
TL;DR: 我们的框架 LightEdit 諁量在持续学习任务中有效地进行知识更新，并，并 当并 却减轻了幻觉现象,。


<details>
  <summary>Details</summary>
Motivation: 当前的持续学习模型（LLMs)在知识更新过程中容易出现幻觉现象， 近来有一些方法试图通过保留模型稳定性和缓解幻觉来提高知识迁移的效果 on. 肨的框架 LightEdit 站了不同的角度， 通过利用逆向散编码和 从抽取的片段知识出发来进行有效的知识更新 on 歌手 LightEdit 在多个dataset on 上 on广泛使用的基准测试上 衷现了优越的的性能 on 咿 this的框架能够较好地处理大规模数据集上的持续学习任务 on

Method: LightEdit通过结合 起用 一个两步的方法 on on 邨此首先首先从抽取的片段知识上 力建立部分 模型的知识 冥采用了的编码器和 迨 一种注意力编码tion on 的ON，RI on 劂全 一种反向向onon on 的解 蠁 on 稡 on RI on 的 on 储成上 那此外 该里 on LightEdit 迣二 一种自我解释编码在抽取的不完整信息上 以便更好地适应新的数据环境 on

Result: LightEdit是一个框架 on 肨此框架能够有效处理大规模数据上的知识更新 on 吘同时通过引入注意力机制减轻幻觉现象 on 姐合适于 on广泛使用的基准测试中取得了较好的性能 on

Conclusion: Your contribution, LightEdit on on显著提高了当前模型在处理大规模数据和 踕上良使用保护模型稳定性和规避幻觉 on 衔也在广泛使用的基准测试上 寞显示了优秀的适应性 舶 数据集新的数据环境.
...
TL;DR: 该研究提出了基于经典测验理论可靠性概念的两种数据集特定的QWK上限，分别为理想的AES模型在标注噪声下的理论上限和具有人类评分误差的AES模型的实用上限，从而解决了公共基准中标签标注错误导致的QWK上限不明的问题，并通过模拟实验和现实基准实验验证了这些上限的有效性。


<details>
  <summary>Details</summary>
Motivation: 目前，AES评估通常依赖于公开基准上的QWK值，但这些值受到人类评分标签中不可避免的评分误差影响，导致QWK的具体理论上限和实用上限不清楚。作者旨在通过新的上限定义解决这一问题。

Method: 作者基于经典测验理论的设计了两种QWK上限：理论上限和人类相似上限。通过标准两标注者基准可估算而无需额外注释。此外，作者使用模拟实验验证这些上限的有效性，并通过实际基准测试进一步说明这些新的QWK上限如何清晰地展示现代AES模型的当前性能和可用改进空间。

Result: 作者提出了两种新的QWK上限，它们可以有效评估现代AES模型的性能，并为AES模型的实际性能提供了更为精确的参考。模拟实验验证了这些上限的有效性，而实际基准测试展示了这些新的QWK上限如何帮助阐明现代AES模型的当前性能和现有改进的潜力。

Conclusion: 研究指出，由于公共基准中评分错误导致的QWK理解限制，提出了一种基于经典测验理论的新型评分标准上限。这些新上限提供了一个明确框架，使AES模型的评估更加精确，并有助于改进AES系统的性能。
...
TL;DR: 本文提出了一个名为AIGC-text-bank的多领域数据集和REVEAL检测框架，通过两阶段训练策略大幅提升了AI生成内容检测的性能。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型的快速进步和广泛应用，可靠地检测AI生成内容变得尤为重要。现有的检测方法在模型进化过程中变得愈发具有挑战性。

Method: 该方法包含一个两阶段训练过程：首先通过监督微调建立推理能力，随后通过强化学习提升准确性和逻辑一致性，减少幻觉。

Result: 实验结果显示，REVEAL在多个基准测试中达到了最先进的性能，并提供了针对AIGC检测的稳健且透明的解决方案。

Conclusion: 本文提出的方法不仅显著提高了AI生成内容检测的性能，还确保了该技术的可解释性和透明性。
...
TL;DR: 该研究通过设计新任务区分信息一致性和规范一致性，并通过实验发现多个大型语言模型在信息一致性和规范一致性方面表现出倾向，且通过调整社会环境可以控制模型的规范一致性，提示LSTM-AS可能易受少数恶意用户操控。


<details>
  <summary>Details</summary>
Motivation: 当前大量关于大语言模型（LLMs）的研究将一致性视为意见改变的一种形式，但本文通过引入社会心理学中的信息一致性和规范一致性概念，旨在更深入地理解LLMs在多智能体系统（LLM-MAS）中的决策机制。

Method: 研究人员设计了区分信息一致性和规范一致性的新任务，并开展了实验来研究这些模型在两种情况下的一致性倾向。此外，通过分析与信息一致性和规范一致性相关的内部向量，探讨了它们的不同内部驱动机制。

Result: 实验结果显示，所测试的六种LLM中有五种既表现出信息一致性倾向，又表现出规范一致性倾向。进一步研究还表明，通过调整社会环境，可以控制LLMs规范一致性的目标。

Conclusion: 该研究表明，LLM-MAS的决策可能受到少数恶意用户的影响，同时也揭示了这两种一致性行为可能由不同的内部机制驱动，为理解“规范”在LLMs中的实现及其对群体动态的影响提供了初步线索。
...
### [179] [Do Agents Dream of Root Shells? Partial-Credit Evaluation of LLM Agents in Capture The Flag Challenges](https://arxiv.org/abs/2604.19354)
...
### [181] [Do LLMs Game Formalization? Evaluating Faithfulness in Logical Reasoning](https://arxiv.org/abs/2604.19459)
...
<details>
  <summary>Details</summary>
Motivation: 动机是为了理解一种新的编译制定方法，G

Method: 使用用两种不同合管道来检测不忠诚性正化的方法，

Result: 首先两阶段的管道能是为了检测不忠诚正化的两种不同不同系统:第一阶段使用从GFair促使粗糙形式化，

Conclusion: 结论结论是通过两个阶段的管道来，检测出了,有两处不忠诚正化的现象， G
...
### [182] [CoDA: Towards Effective Cross-domain Knowledge Transfer via CoT-guided Domain Adaptation](https://arxiv.org/abs/2604.19488)
*Jianzhi Yan,Le Liu,Buzhou Tang,Yang Xiang,Dongning Sun,Zhiming Li*

Main category: cs.AI

TL;DR: 该研究提出了一种名为CoDA的方法，通过轻量级适配器直接干预中间隐藏状态，结合特征导向的CoT丰富参考表示和核化分布匹配，将源域与目标域的潜在推理表示对齐。该方法在多个逻辑推理任务中显著优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 现有的通过在输入中提供高级领域特定例子来提升模型推理能力的方法在资源稀缺领域效果有限，因此需要一种新方法来跨越领域间差异，更有效地传输跨域知识。

Method: CoDA方法使用轻量级适配器直接介入中间隐藏状态，并结合特征导向的CoT丰富参考表示与Maximum Mean Discrepancy (MMD)进行核化分布匹配，以对齐源域和目标域的潜在推理表示。

Result: CoDA方法在多个逻辑推理任务中表现出色，显著优于现存的最佳基线模型。

Conclusion: 该研究通过提出CoDA方法，成功解决了跨领域推理难题，为提升大型语言模型在特定领域的逻辑推理能力提供了新策略。
...
### [183] [SimDiff: Depth Pruning via Similarity and Difference](https://arxiv.org/abs/2604.19520)
*Yuli Chen,Shuhao Zhang,Fanshen Meng,Bo Cheng,Jiale Han,Qiang Tong,Xiulei Liu*

Main category: cs.AI

TL;DR: 提出了一种新的层重要性标准SimDiff，通过代表相似性和变换差异两个维度评估层的重要性，显著优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖单一维度（余弦相似性）来评估层的重要性，可能会导致性能不可预测或崩溃。

Method: SimDiff通过代表相似性和变换差异两个维度来评估层的重要性。代表相似性使用MSSD进行量化，这种度量对异常值敏感，用于识别做出决定性修正的层；变换差异使用MASD度量，这是一种稳健的方法，用于评估层的平均贡献。

Result: SimDiff在不同模型参数量的多项实验中，各个修剪比例下均显著优于现有最先进的基线方法。在LLaMA2-7B模型25%修剪比例下，SimDiff保持了超过91%的性能；当在LLaMA3.1-8B模型12层修剪时，Inference速度提升至1.49倍。

Conclusion: SimDiff在自动剪枝大语言模型层时提供了一种更加可靠的评估方法，且修剪后的模型经过少量微调可以有效恢复。
...
### [184] [DT2IT-MRM: Debiased Preference Construction and Iterative Training for Multimodal Reward Modeling](https://arxiv.org/abs/2604.19544)
*Zhihong Zhang,Jie Zhao,Xiaojian Huang,Jin Xu,Zhuodong Luo,Xin Liu,Jiansheng Wei,Xuejin Chen*

Main category: cs.AI

TL;DR: DT2IT-MRM 提出了一种新的方法来解决现有多模态偏好数据集存在的问题，通过去偏见的偏好构建管道、图像到文本的偏好数据新形式以及迭代训练框架，显著提高了多模态奖励模型的整体性能。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态偏好数据集存在偏好强度细化不足、文本风格偏差和不稳定的偏好信号等问题，且缺乏有效的去噪方法。DT2IT-MRM 的目标是改善这些问题，从而提高多模态奖励模型的性能。

Method: DT2IT-MRM 方法包含三个主要部分：去偏见的偏好构建管道、图像到文本的偏好数据新形式以及迭代训练框架。这些组件共同作用于现有数据集进行改进。

Result: 该方法在 VL-RewardBench、Multimodal RewardBench 和 MM-RLHF-RewardBench 三个主要基准上的实验结果表明，DT2IT-MRM 实现了新的最先进的整体性能。

Conclusion: DT2IT-MRM 的提出和应用展示了在解决多模态偏好数据集问题方面的创新，有助于提高多模态奖励模型的性能。
...
### [185] [Enhancing Construction Worker Safety in Extreme Heat: A Machine Learning Approach Utilizing Wearable Technology for Predictive Health Analytics](https://arxiv.org/abs/2604.19559)
*Syed Sajid Ullah,Amir Khan*

Main category: cs.AI

TL;DR: 该摘要描述了利用长短时记忆（ (LSTM) 稡型和 和和注意力基长短期记忆 (Attention LSTM) 模型来预测和 巯高压 (EH) 湿对员工的影响。， 


<details>
  <summary>Details</summary>
Motivation: 该动机是帮助： 在沙特阿拉伯， ，的工作环境中识别和高温高压 (EH) 对工人健康的影响，并 是复杂和 的。， 

Method: 方法使用是 

Result: 该方法使用使用了两种种类型的LSTM模型:： 长期短期记忆 (LSTM) 模型和注意力基长短期记忆 (STLV LSTM) 模型来 杋试去预测温度 (T) 

Conclusion:  总结来说该 功能提高了预测的准确性, 幟能够被有效应用在物联网 (IoT) 动设施和 和 和和建筑信息模型 (BIM) 浌板上 中 
...
### [186] [Detecting Data Contamination in Large Language Models](https://arxiv.org/abs/2604.19561)
*Juliusz Janicki,Savvas Chamezopoulos,Evangelos Kanoulas,Georgios Tsatsaronis*

Main category: cs.AI

TL;DR: 该论文分析了如何检测大型语言模型（LLMs）中存在来自受版权保护内容的剽窃攻击， 对 MIA。，尽管这些攻击可能可能 通过操控这些LL模型的数据，并 与。。 


<details>
  <summary>Details</summary>
Motivation: 为了评估LLMs是否 在最黑盒MIA场景 下 的环境下的性能， 以及展示了通过不同熟悉排名 评价模型来可 茯来帮助展示这些模型在评估隐藏在大模型中的受版权保护内容时的难度， 

Method: 该研究首先探讨了MIA攻击 如在评估受版权保护内容是否 是否存在于LLIA中的模型中 

Result: 研究发现， 夌些个较大语言模型LMI

Conclusion: 总的来说 该 记论文说明了在最黑盒环境中鉴定受版权保护内容的存在是一个具有挑战性的任务 茯通常使用的排名模型IA很难准确地评估这些内容是否是否存在。 
...
### [187] [Multi-modal Reasoning with LLMs for Visual Semantic Arithmetic](https://arxiv.org/abs/2604.19567)
*Chuou Xu,Liya Ji,Qifeng Chen*

Main category: cs.AI

TL;DR: 本文提出了一种新的基于可验证函数和组相对策略优化的后训练方法，用于增强大型视觉-语言模型在图像中的语义算术能力。该方法在图像-关系对数据集和Visual7W-Telling数据集上取得了当前最先进的结果。


<details>
  <summary>Details</summary>
Motivation: 当前的大型语言模型虽然在编码和数学推理方面有较强的提升，但在从视觉信息中进行语义算术推理方面仍存在不足。现有的方法主要通过图像特征解码和向量算术来处理，存在模块间鸿沟，缺乏系统评估。因此，本文旨在通过设计新型任务和提出新的后训练方法来填补这些不足。

Method: 本文提出了一个名为SAri-RFT的后训练方法，用于提升大型视觉-语言模型的语义算术推理能力。该方法基于可验证函数和组相对策略优化，通过构建Image-Relation-Pair数据集来进行基准测试。

Result: 本文的方法在Image-Relation-Pair数据集和现实世界的Visual7W-Telling数据集上都取得了当前最先进的结果，展示了在视觉信息中进行高质量语义算术推理的能力。

Conclusion: 通过增强模型的跨模态关系推理能力，本文的工作提升了服务和家用机器人在复杂环境中进行感知归因、工具适应和人机交互的效果。
...
### [188] [SafetyALFRED: Evaluating Safety-Conscious Planning of Multimodal Large Language Models](https://arxiv.org/abs/2604.19638)
...
### [189] [A Dual Perspective on Synthetic Trajectory Generators: Utility Framework and Privacy Vulnerabilities](https://arxiv.org/abs/2604.19653)
...
### [190] [A-MAR: Agent-based Multimodal Art Retrieval for Fine-Grained Artwork Understanding](https://arxiv.org/abs/2604.19689)
...
### [191] [Optimizing Branch Predictor for Graph Applications](https://arxiv.org/abs/2604.18698)
*Upasna,Venkata Kalyan Tavva*

Main category: cs.AR

TL;DR: 该论文关注于通过优化分支预测准确度来提升图处理应用性能，指出当前的分支预测器仍然有优化空间，特别是在减少分支预测错误方面。


<details>
  <summary>Details</summary>
Motivation: 现有图处理应用规模超出缓存容量，导致内存层次成为性能瓶颈。尽管通过改进缓存性能可以提升部分性能，但进一步提高分支预测准确度仍有很大潜力。

Method: 论文可能探讨了现有分支预测器的不足之处，并提出新的优化方法来减少分支预测错误。

Result: 论文的具体结果未在摘要中提及。

Conclusion: 结论部分可能总结了优化分支预测准确度方法对图处理应用性能提升的效果。
...
### [192] [A Comparative Analysis of ARM and x86-64 Laptop-Class Processors: Architecture, Assembly-Level Performance, and Energy Efficiency](https://arxiv.org/abs/2604.18896)
*Mustafa Mert Özyılmaz*

Main category: cs.AR

TL;DR: 这份摘要综合分析了一款基于AAArch64和 x86-64架构的两款处理器———： Apple M M3 和和 AMD Ryzen 7 3 375H 进行性能和 在不同方法和动机


<details>
  <summary>Details</summary>
Motivation: 研究对比基于不同 构架构间的两种处理器在不同类型工作任务上的的性能差异，这部分分析目的在于揭示除了这两个架构及其对应处理器在指令集 扷长度 和表性丰富度的x86-64与固定宽度的AAArch64之间的差异

Method: 通过组合使用杖动态表象、不同 和处理器吞活性干预和以及跨平台测量统平测量表在匹配针对这两个处理器进行基准测试

Result: 在基准测试中，包括 Ryzen 7 375 在 heavy Fibonacci 基准测试上上在出现相对效果较佳 on 誌组表上n然而在矩阵乘加作业上的两个处理器显示没有没有显著差异 on on表明实际使虽然为同 x86-64 慷表比能在在某些类型任务工情况下表现出一般改善 off对 但在两种类型的工作负载上on 上相对于处理器来说 两种显不同显示在 Fibonacci 抛代码在 M3 夕인上表现具有明显改善 但在矩阵乘法测试上上 上， M3 仍然表现较为接近 x86on64

Conclusion: 综合表明在指令集和总体架构层面上不同主要 x86on64 和 AArch64
...
### [193] [Energy Efficient LSTM Accelerators for Embedded FPGAs through Parameterised Architecture Design](https://arxiv.org/abs/2604.19293)
*Chao Qian,Tianheng Ling,Gregor Schiele*

Main category: cs.AR

TL;DR: 摘要 迱份关于 拒长序列数据处理方面, 让 s-LSTMs在嵌入设备上的 的服行全和和功率消耗方面得到了优化.


<details>
  <summary>Details</summary>
Motivation: 在嵌入设备上 $由于需要

Method: 我们通过针对FPGA特进行优化

Result: 我们的方法提高了执行速度和 和 欂 
